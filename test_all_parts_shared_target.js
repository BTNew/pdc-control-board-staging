...
assert.ok(app.includes('function authenticatedEmailPartsTarget(') && app.includes('async function authenticatedPartsTarget('), 'Parts actions have shared authenticated target resolvers');
for (const name of ['markVehiclePartsOrdered', 'markVehiclePartsComplete', 'updateVehiclePartsWorstEta']) {
  ...
  assert.ok(start >= 0 && body.includes('authenticatedPartsTarget(key, vehicle)'), `${name} resolves the canonical shared vehicle target`);
