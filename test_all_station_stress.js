...
const workItems=[];
for(let i=0;i<500;i++){
  workItems.push({vehicle_id:`v${i}`,work_key:stationCodes[i%8],required:true,completed:false});
  workItems.push({vehicle_id:`v${i}`,work_key:stationCodes[(i+1)%8],required:true,completed:false});
}
const bookings=vehicles.map((v,i)=>({vehicle_id:v.id,stage_code:stationCodes[i%8],status:'planned'}));
...
assert.strictEqual(evaluations,800);
assert(candidates>0);
...
console.log(JSON.stringify({vehicles:500,work_items:1000,bookings:500,planners:8,evaluations,elapsed_ms:Number(elapsed.toFixed(2)),heap_delta_bytes:heapDelta,sublet_rejected:true}));
