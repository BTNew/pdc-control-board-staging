...
ok(/function qcPageVehicles[\s\S]*vehicleInQualityControlGate\(vehicle\)/.test(app), 'QC page filters only vehicles requiring QC');
ok(app.includes('data-qc-vehicle-key=') && app.includes('data-qc-open-vehicle='), 'QC page renders selectable vehicle cards');
ok(app.includes('data-qc-work-check=') && app.includes("'is-complete'") && app.includes('qc-work-item'), 'QC checklist supports green completed states');
...
ok(app.includes('data-qc-signoff=') && app.includes('completeVehicleQualityControl('), 'QC sign-off uses the existing authoritative QC-to-RFT action');
ok(app.includes('notification_has_recipient'), 'QC sign-off preserves salesperson notification result handling');
