...
MIGRATION = ROOT / "supabase/staging_only/20260830237000_816_historical_importer_sender_binding_successor.sql"


class HistoricalImporterSender816ContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_append_only_814_predecessor_and_parse(self):
        ...
        for marker in (
            ...
            "20260830237000",
            "816_historical_importer_sender_binding_successor",
            "pdc_816_current_head_or_importer_prestate_failed",
            "pdc_816_historical_importer_sender_postcondition_failed",
        ):
            self.assertIn(marker.lower(), self.lower)

    def test_both_historical_fallbacks_use_locked_intake_sender(self):
        body = self.lower.split("as $function$", 1)[1].split("revoke all on function", 1)[0]
        self.assertEqual(body.count("h.sender_email=lower(coalesce(v_intake.sender_email,''))"), 2)
        self.assertNotIn("h.sender_email=v_sender", body)
        self.assertIn("pdc_historical_provider_observations_778", body)
        self.assertIn("pdc_historical_writer_authorized_773", body)
        self.assertIn("pdc_monitor_authenticated_active_scope_673", body)

    def test_security_and_atomic_import_contract_remain(self):
        for marker in (
