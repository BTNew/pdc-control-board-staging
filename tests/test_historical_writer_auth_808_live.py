...
RUN_LIVE = os.environ.get("PDC_RUN_HISTORICAL_WRITER_AUTH_808_LIVE") == "1"
...
@unittest.skipUnless(RUN_LIVE, "set PDC_RUN_HISTORICAL_WRITER_AUTH_808_LIVE=1 for authorised STAGING regression")
class HistoricalWriterAuth808LiveTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        bootstrap = load_module(boot, "pdc_808_writer_bootstrap")
        access = json.loads(bootstrap.unprotect(secret.read_bytes()).decode())
        cls.conn = psycopg2.connect(access["PDC_STAGING_DATABASE_URL"], sslmode="verify-full", sslrootcert=access["PDC_STAGING_SSLROOTCERT"], application_name="pdc-808-writer-live")
        cls.conn.autocommit = False
        cls.caller = load_module(CALLER, "pdc_808_writer_caller")
    ...
    def test_all_five_notice_and_replay_paths_rollback_only(self):
        before = self.snapshot()
        for uid in UIDS:
            request = self.request(uid)
            self.cur.execute("savepoint pdc808_notice")
            result = self.call_public(request)
            self.assertNotEqual(result.get("code"), "pdc_778_exact_authorization_failed", (uid, result))
            self.assertNotEqual(result.get("code"), "historical_wrapper_preflight_failed", (uid, result))
            self.assertNotEqual(result.get("code"), "unauthorized", (uid, result))
            ...
            self.cur.execute("rollback to savepoint pdc808_notice")
            self.cur.execute("release savepoint pdc808_notice")
    ...
    def test_malformed_auth_and_wrong_actor_are_denied_without_drift(self):
        ...
        self.cur.execute(
            ...
            (request["parent_source_hash"], request["evidence_hash"], request["provider_uid"], request["sender_email"], json.dumps(bad_auth), request["stock_number"], json.dumps(request["observations"])),
        )
