...
class StorageIdempotency20260845Tests(unittest.TestCase):
    ...
    def test_other_http_400_failures_are_not_replay_success(self):
        for body in (
            ...
            {"code": "KeyAlreadyExists", "statusCode": "410"},
        ):
    ...
    def test_enqueue_projection_binds_each_attachment_to_a_stable_mime_ordinal(self):
        intake = bridge.IntakeMessage(
            graph_message_id="imap:<message@example.invalid>",
            source_hash="b" * 64,
            provider_uid="imap_uid:673",
            attachments=[
                bridge.AttachmentRecord(
                    filename=f"image-{index}.jpg", content_type="image/jpeg", size_bytes=7,
                    source_hash=hashlib.sha256(f"bytes-{index}".encode()).hexdigest(),
                    validation_status="verified", local_path=f"C:/ignored/{index}",
                ) for index in range(1, 3)
            ],
        )
        response = self._response(200, b'{"ok":true,"intake_id":"intake-1"}')
        with patch.object(bridge, "supabase_scoped_client", return_value=(
                "https://cdsmnqxtyyoeoznmbidd.supabase.co", "anon", "token")), \
             patch.object(bridge, "_upload_attachment", side_effect=lambda _base, _anon, _token, attachment: (
                 "pdc-email-intake-private/" + attachment.source_hash + "/" + attachment.filename)), \
             patch.object(urllib.request, "urlopen", return_value=response) as request:
            bridge.post_to_supabase(intake)
        payload = json.loads(request.call_args.args[0].data.decode("utf-8"))
        self.assertEqual(
            [item["graph_attachment_id"] for item in payload["p_attachments"]],
            ["imap_uid:673:1", "imap_uid:673:2"],
        )
