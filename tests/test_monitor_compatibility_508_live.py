...
RUN_LIVE = os.environ.get("PDC_RUN_508_COMPATIBILITY_LIVE_TESTS") == "1"


@unittest.skipUnless(RUN_LIVE, "set PDC_RUN_508_COMPATIBILITY_LIVE_TESTS=1 for authorised staging integration")
class MonitorCompatibility508LiveTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.conn = psycopg2.connect(
            ...
            connect_timeout=15,
            application_name="pdc508_code_compatibility_live_test",
        )
    ...
    def test_exact_contained_sales_reader_returns_legacy_code_and_terminal(self):
        ...
        self.assertEqual(result["receipt_kind"], "staging_commissioning")
        self.assertEqual(result["receipt_source"], "logical_507_exact_terminal_receipt")
    ...
    def test_wrong_actor_and_event_remain_denied(self):
    ...
    def test_508_control_and_history_are_private_and_singleton(self):
        with self.conn.cursor() as cur:
            cur.execute(
                """select count(*) from public.pdc_uid514_receipt_code_compatibility_controls_508
                   where singleton and enabled and receipt_id in (
                     select receipt_id from public.pdc_uid514_staging_commissioning_terminal_receipts_507
                     where recovery_event_id=25751401)
                     and recovery_event_id=25751401 and actor_id='df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b'
                     and response_code='uid514_receipt_terminal'
                     and receipt_kind='staging_commissioning'
                     and receipt_source='logical_507_exact_terminal_receipt'
            ...
            cur.execute("select count(*) from public.pdc_uid514_receipt_code_compatibility_history_508 where event_kind='forward_code_compatibility'")
            history_count = cur.fetchone()[0]
            cur.execute(
                """select (select relrowsecurity and relforcerowsecurity from pg_class where oid='public.pdc_uid514_receipt_code_compatibility_controls_508'::regclass),
                    (select relrowsecurity and relforcerowsecurity from pg_class where oid='public.pdc_uid514_receipt_code_compatibility_history_508'::regclass),
                    has_table_privilege('authenticated','public.pdc_uid514_receipt_code_compatibility_controls_508','select'),
                    has_table_privilege('authenticated','public.pdc_uid514_receipt_code_compatibility_history_508','select'),
