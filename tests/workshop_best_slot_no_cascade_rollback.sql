...
DO $checks$
DECLARE a uuid; v uuid; bay uuid; otherbay uuid; later uuid; other uuid; n integer; st timestamptz; r jsonb; rid uuid:=gen_random_uuid(); ver integer; before_rows jsonb;
BEGIN
 st:=((SELECT friday FROM ou_context)+3)::timestamp AT TIME ZONE 'Australia/Perth'+interval '7 hours';
 bay:=pg_temp.ou_bay('best-slot','FABRICATION'); otherbay:=pg_temp.ou_bay('other-station','FITTING');
 UPDATE public.workshop_bays SET bay_number=(SELECT coalesce(max(bay_number),0)+1 FROM public.workshop_bays WHERE stage_id=(SELECT stage_id FROM public.workshop_bays WHERE id=bay)) WHERE id=bay RETURNING bay_number INTO n;
 a:=pg_temp.ou_vehicle('arriving'); PERFORM pg_temp.ou_operation(a,'FABRICATION',2);
 v:=pg_temp.ou_vehicle('later'); PERFORM pg_temp.ou_operation(v,'FABRICATION',1); PERFORM pg_temp.ou_operation(v,'FITTING',1,2);
 later:=pg_temp.ou_booking('later-fab',v,'FABRICATION',bay,st+interval '2 hours');
 other:=pg_temp.ou_booking('later-fitting',v,'FITTING',otherbay,st+interval '4 hours');
 SELECT jsonb_agg(to_jsonb(b) ORDER BY id) INTO before_rows FROM public.workshop_bookings b WHERE id IN(later,other);
 SELECT version INTO ver FROM public.vehicles WHERE id=a;
 BEGIN
 r:=public.cascade_workshop_schedule('insert',a,ver,'FABRICATION',n,st,120,NULL,120,NULL,'{"source":"best_slot_rollback"}');
 EXCEPTION WHEN SQLSTATE '22023' THEN
  IF SQLERRM NOT LIKE '%vehicle_overlap%' THEN RAISE; END IF;
  r:=jsonb_build_object('ok',false,'error','vehicle_overlap');
 END;
 PERFORM pg_temp.ou_assert(r->>'ok'='false' AND r->>'error'='vehicle_overlap','Old cascading save reproduces the reported vehicle-overlap error',r);
 PERFORM pg_temp.ou_assert((SELECT jsonb_agg(to_jsonb(b) ORDER BY id)=before_rows FROM public.workshop_bookings b WHERE id IN(later,other)),'Rejected cascade leaves following jobs unchanged');
 r:=public.schedule_vehicle_work(a,ver,'FABRICATION',n,st,120,NULL,NULL,'{"source":"best_slot_rollback"}');
 PERFORM pg_temp.ou_assert(r->>'ok'='true','Best slot books the free space successfully',r);
 PERFORM pg_temp.ou_assert(EXISTS(SELECT 1 FROM public.workshop_bookings WHERE vehicle_id=a AND scheduled_start_at=st AND scheduled_end_at=st+interval '2 hours'),'New booking keeps its selected time and correct duration');
 PERFORM pg_temp.ou_assert((SELECT jsonb_agg(to_jsonb(b) ORDER BY id)=before_rows FROM public.workshop_bookings b WHERE id IN(later,other)),'Best slot does not move later bookings or their other stations');
END $checks$;
SELECT pg_temp.ou_assert(NOT EXISTS(SELECT 1 FROM ou_original_bookings x LEFT JOIN public.workshop_bookings b ON b.id=x.id WHERE to_jsonb(b) IS DISTINCT FROM x.row_data),'Real bookings unchanged');
SELECT pg_temp.ou_assert(NOT EXISTS(SELECT 1 FROM ou_original_vehicles x LEFT JOIN public.vehicles v ON v.id=x.id WHERE to_jsonb(v) IS DISTINCT FROM x.row_data),'Real vehicles unchanged');
