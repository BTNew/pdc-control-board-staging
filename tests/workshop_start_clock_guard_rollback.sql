...
CREATE FUNCTION pg_temp.clock_fixture_advance(bid uuid, recorded_at timestamptz DEFAULT clock_timestamp()) RETURNS void LANGUAGE plpgsql AS $fn$
DECLARE x jsonb; y jsonb; bay uuid;
BEGIN
 PERFORM set_config('TimeZone','UTC',true);
 SELECT to_jsonb(b),b.bay_id INTO x,bay FROM public.workshop_bookings b WHERE id=bid;
 ...
 version=version+1,updated_at=clock_timestamp(),updated_by=auth.uid() WHERE id=bid;
 ...
 FROM public.workshop_bookings b WHERE b.id=bid AND a.booking_id=b.id AND a.released_at IS NULL;
 SELECT to_jsonb(b) INTO y FROM public.workshop_bookings b WHERE id=bid;
 INSERT INTO public.workshop_clock_history(booking_id,bay_id,clock_at,before_data,after_data,created_at)
 VALUES(bid,bay,recorded_at,x,y,recorded_at);
 PERFORM set_config('TimeZone','Australia/Perth',true);
END $fn$;
DO $checks$
DECLARE t timestamptz:=date_trunc('minute',statement_timestamp()); v uuid; bay uuid; b uuid; tech uuid:=gen_random_uuid(); tech2 uuid:=gen_random_uuid();
 d jsonb; r jsonb; ver integer; scenario text; decision integer;
BEGIN
 INSERT INTO public.workshop_technicians(id,name,role_type,active) VALUES(tech,'Clock guard rollback','technician',true),(tech2,'Clock assignment rollback','technician',true);
 PERFORM pg_temp.ou_assert(NOT has_function_privilege('authenticated','pdc_fitter_private.clock_start_version(uuid,integer)','EXECUTE')
 AND NOT has_function_privilege('anon','pdc_fitter_private.clock_start_version(uuid,integer)','EXECUTE'),'Clock proof remains private');
 FOREACH scenario IN ARRAY ARRAY['direct','manual','protected','assignment','expired','catalog','other-action'] LOOP
  v:=pg_temp.ou_vehicle('clock-'||scenario);bay:=pg_temp.ou_bay('clock-'||scenario,'FITTING');
  PERFORM pg_temp.ou_operation(v,'FITTING',1,1);
  b:=pg_temp.ou_booking('clock-'||scenario,v,'FITTING',bay,public.workshop_admin_next_operational_minute(t+interval '1 minute'));
  -- Explicit assignment is created at the same fixture baseline timestamp.
  ...
  d:=public.get_fitter_job(tech,b);ver:=(d->>'version')::int;
  IF scenario='manual' THEN UPDATE public.workshop_bookings SET version=version+1 WHERE id=b; END IF;
  PERFORM pg_temp.clock_fixture_advance(b,CASE WHEN scenario='expired' THEN clock_timestamp()-interval '11 minutes' ELSE clock_timestamp() END);
  IF scenario='direct' THEN
   PERFORM pg_temp.clock_fixture_advance(b);
   r:=public.start_workshop_work(b,ver,NULL,'{}');
   PERFORM pg_temp.ou_assert((r->>'ok')::boolean AND (r->>'clock_rebased')::boolean,'Planner Start accepts a continuous two-clock chain serialized in UTC',r);
   -- Free the fixture mechanic for the next independent fixture, using canonical completion.
   UPDATE public.workshop_booking_assignments SET released_at=clock_timestamp() WHERE booking_id=b;
  ELSIF scenario='protected' THEN
   UPDATE public.workshop_bookings SET metadata=metadata||'{"manual_note":"changed"}'::jsonb WHERE id=b;
   PERFORM pg_temp.ou_assert(pdc_fitter_private.clock_start_version(b,ver) IS NULL,'Unversioned protected booking edits cannot pass the clock proof');
  ELSIF scenario='assignment' THEN
   UPDATE public.workshop_booking_assignments SET released_at=clock_timestamp() WHERE booking_id=b;
   INSERT INTO public.workshop_booking_assignments(booking_id,technician_id,assignment_type,assigned_by,assigned_at,scheduled_start_at,scheduled_end_at)
   SELECT id,tech2,'primary',auth.uid(),clock_timestamp(),scheduled_start_at,scheduled_end_at FROM public.workshop_bookings WHERE id=b;
   PERFORM pg_temp.ou_assert(pdc_fitter_private.clock_start_version(b,ver) IS NULL,'Changed assignment without a booking version still rejects stale Start');
  ELSIF scenario='catalog' THEN
   r:=public.fitter_job_command(tech,b,ver,'incorrect-display-scope',gen_random_uuid(),'start');
   PERFORM pg_temp.ou_assert(r->>'error'='scope_changed','Stale fitter Start must match its displayed operation catalogue',r);
  ELSIF scenario='other-action' THEN
   r:=public.fitter_job_command(tech,b,ver,d->>'catalog_hash',gen_random_uuid(),'stop',NULL,NULL,'Synthetic stop');
   PERFORM pg_temp.ou_assert(r->>'error'='version_conflict','Other fitter actions retain strict version checks',r);
  ELSE
   r:=public.start_workshop_work(b,ver,NULL,'{}');
   PERFORM pg_temp.ou_assert(r->>'error'='version_conflict',CASE WHEN scenario='manual' THEN 'A non-clock version gap is never rebased' ELSE 'Clock evidence older than ten minutes is rejected' END,r);
  END IF;
  IF scenario<>'direct' THEN
   PERFORM pg_temp.ou_assert((SELECT status='planned' AND actual_start_at IS NULL FROM public.workshop_bookings WHERE id=b),'Rejected '||scenario||' fixture remains unstarted');
  END IF;
  -- No active fixture assignment is carried into another independent case.
  UPDATE public.workshop_booking_assignments SET released_at=coalesce(released_at,clock_timestamp()) WHERE booking_id=b;
 END LOOP;
END $checks$;
