...
class MonitorSuccessor20260849ContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.active_bootstrap = (SCRIPTS / "pdc_monitor_active_bootstrap_20260849.ps1").read_text(encoding="utf-8").lower()
        cls.active_dispatch = (SCRIPTS / "pdc_monitor_active_dispatch_20260849.ps1").read_text(encoding="utf-8").lower()
        cls.verify_bootstrap = (SCRIPTS / "pdc_monitor_verifyonly_bootstrap_20260849.ps1").read_text(encoding="utf-8").lower()
        cls.verify_runner = (SCRIPTS / "pdc_monitor_verifyonly_runner_20260849.ps1").read_text(encoding="utf-8").lower()
        cls.installer = (SCRIPTS / "install_pdc_monitor_successor_20260849.ps1").read_text(encoding="utf-8").lower()
        cls.rollback = (SCRIPTS / "rollback_pdc_monitor_successor_20260849.ps1").read_text(encoding="utf-8").lower()

    def test_successor_artifacts_are_present_and_hashable(self):
        for path in (
            SCRIPTS / "build_pdc_monitor_successor_20260849.py",
            SCRIPTS / "verify_pdc_monitor_successor_20260849.py",
            ROOT / "backend/imap_bridge_successor_20260849.py",
    ...
    def test_storage_response_is_exact_and_fail_closed(self):
        bridge = (ROOT / "backend/imap_bridge_successor_20260849.py").read_text(encoding="utf-8")
    ...
    def test_successor_is_parent_bound_and_staging_only(self):
        for source in (self.active_dispatch, self.verify_runner, self.installer, self.rollback):
            for marker in ("2026.08.49", "2026.08.44", "production"):
        ...
        self.assertIn("backups\\20260849", self.installer)
