...
class ExactSuccessfulReplayMigrationTests(unittest.TestCase):
    def test_migration_resolves_only_exact_successful_replay_before_current_validation(self):
        ...
        patch_targets = re.search(r"FOREACH v_signature IN ARRAY ARRAY\[(.*?)\] LOOP", sql, re.S)
        self.assertIsNotNone(patch_targets)
        self.assertIn("'public.apply_pdc_email_ai_typed_action_surface_20260901_strict(jsonb)'", patch_targets.group(1))
        self.assertNotIn("'public.apply_pdc_email_ai_typed_action_surface_20260901(jsonb)'", patch_targets.group(1))
