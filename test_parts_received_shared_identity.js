...
assert.ok(service.includes("const PDC_PARTS_COMPLETE_RPC = 'mark_pdc_parts_complete';"), 'Parts Received uses the staging canonical RPC');
assert.ok(service.includes("body: JSON.stringify({ p_vehicle_id: vehicleId, p_expected_version: Number(expectedVersion) || 0 })"), 'Parts Received sends only canonical UUID and expected version');
