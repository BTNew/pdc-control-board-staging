...
assert.match(select, /workshopSelectPlanForDetail\(button\.dataset\.workshopSelectPlan\)/);
assert.doesNotMatch(select, /renderWorkshopPlanner\(\)/, 'single click preserves the card DOM so double-click can open the job');
assert.match(select, /classList.toggle\('is-selected'/);
assert.match(source, /addEventListener\('dblclick'[\s\S]*?openWorkshopVehicleJob/);
