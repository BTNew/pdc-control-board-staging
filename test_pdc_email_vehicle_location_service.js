...
(async () => {
  ...
  const server = { id: 's1', permanent_vehicle_id: 'p1', version: 12, stock_number: 'S-100', vin: 'VIN100', customer_name: 'Server customer', vehicle_description: 'Server vehicle', current_location: 'Other', visible_on_board: true, work_items: [{ work_key: 'tint', required: true, completed: false }, { work_key: 'sublet', required: true, completed: false }, { work_key: 'pit_inspection', required: true, completed: true, completed_at: '2026-07-25T01:00:00Z', completed_by: 'staff' }], operation_lines: [{ operation_line_id: '11111111-1111-4111-8111-111111111111', operation_no: 'OP1', work_key: 'tint', description: 'AI estimated tint work', estimated_hours: 1.5, estimated_hours_source: 'ai_estimate', source_uid: 'message-1' }], parts_required: true, parts_completed: false, parts_update: { parts_ordered: true, parts_stoppage: false, worst_eta: '2026-08-12', previous_worst_eta: '2026-08-01', updated_at: '2026-07-27T01:00:00Z' }, sublet_booking: { provider: 'Provider A', booking_date: '2026-08-04', email_sent: false, version: 3 } };
  ...
  assert(mapped.pdcEmailOperationLines.length === 1 && mapped.pdcEmailOperationLines[0].estimatedHoursSource === 'ai_estimate', 'Authenticated operation hour provenance must survive the projected-row mapper');
