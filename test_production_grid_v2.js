...
const expectedVersion = (app.match(/const APP_VERSION = '([^']+)'/) || [])[1];

assert.ok(expectedVersion, 'app.js must define APP_VERSION');
...
assert.doesNotMatch(app, /data-pmb-work-transfer-key/);
assert.doesNotMatch(app, /function bindPmbWorkTransferSelects\(/);
assert.doesNotMatch(app, /movePmbVehicleToStage\(select\.dataset\.pmbWorkTransferKey, stage\)/);
...
assert.match(app, /<th>Key<\/th><th>Stock<\/th><th>JC<\/th><th>Vehicle \/ customer<\/th><th>Parts status<\/th><th>Parts ETA<\/th><th>ETA counter<\/th><th>JITA<\/th><th>Outstanding station work<\/th><th>Stoppage reason<\/th><th>Actions<\/th>/);
