#!/usr/bin/env python3
"""Build a deterministic .61-derived staging runtime successor.
...
def main() -> int:
    ...
    parser.add_argument("--jobcard-source", type=Path, required=True)
    ...
    processor = args.processor_source.resolve(strict=True)
    jobcard = args.jobcard_source.resolve(strict=True)
    ...
    if not processor.is_file() or processor.is_symlink():
        raise SystemExit("successor processor must be one regular file")
    if not jobcard.is_file() or jobcard.is_symlink():
        raise SystemExit("successor jobcard client must be one regular file")
    if output.exists():
    ...
    shutil.copyfile(jobcard, output / "backend" / "pdc_jobcard_runtime_client.py")
    manifest = json.loads(parent_manifest.read_text(encoding="utf-8"))
    manifest.update({
        "release_series": "pdc-monitor-staging-m502-successor",
        "release_name": "pdc-monitor-staging-m502-2026.08.64",
        "release_version": "2026.08.64",
        ...
        "successor_patch": {
            ...
            "source_path": "backend/imap_bridge_successor_20260864.py",
            "sha256": sha(bridge),
            "contract": "HTTP 400 body code KeyAlreadyExists and statusCode 409 only",
        },
        "successor_processor": {
            "path": "backend/email_intake_processor.py",
            "source_path": "backend/email_intake_processor_successor_20260864.py",
            "sha256": sha(processor),
            "contract": "authenticated private object readback and bound source hash verification",
        },
        "successor_jobcard_client": {
            "path": "backend/pdc_jobcard_runtime_client.py",
            "source_path": "backend/pdc_jobcard_runtime_client_successor_20260864.py",
            "sha256": sha(jobcard),
            "contract": "authenticated monitor jobcard RPC client dependency closure",
        },
        ...
        "migration_head": "20260829010000",
        "storage_path_contract": "pdc-email-intake-private/<lowercase-source-sha256>/<sanitized-filename>; exact three components; no URL, traversal or alternate bucket",
        "storage_reconciliation_contract": "append-only receipt-bound historical path reconciliation; original evidence is never rewritten",
        "admin_requeue_contract": "admin_requeue_pdc_email_intake_735 exact two-target audited custody; Monitor remains non-administrator",
    })
    ...
    receipt = {
        ...
        "successor_processor_sha256": sha(processor),
        "successor_jobcard_sha256": sha(jobcard),
        "files": len(inventory),
    }
