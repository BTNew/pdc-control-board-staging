...
try {
 ...
 $acl=(icacls.exe $release 2>&1 | Out-String);if($acl -notmatch '(?:S-1-5-19|NT AUTHORITY\\LOCAL SERVICE):(?:\(OI\)\(CI\))?\(RX\)'){Fail 'PDC_SUCCESSOR_RELEASE_ACL_FAILED'}
 if(-not(Test-Path -LiteralPath $Observer -PathType Leaf)){Fail 'PDC_SUCCESSOR_OBSERVER_MISSING'}
 if(-not(Test-Path -LiteralPath $Python -PathType Leaf)){Fail 'PDC_SUCCESSOR_PYTHON_RUNTIME_MISSING'}
 $observerOut=& $Python $Observer 2>&1;$observerExit=$LASTEXITCODE;if($observerExit -ne 0){Fail 'PDC_SUCCESSOR_AI_RUNTIME_OBSERVER_FAILED'};Set-Content -LiteralPath $Report -Value ($observerOut -join [Environment]::NewLine) -Encoding utf8
