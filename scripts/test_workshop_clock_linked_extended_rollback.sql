...
create temp table clock_test_work as select id,to_jsonb(w) data from public.vehicle_work_items w;
create temp table clock_test_admin as select id,to_jsonb(a) data from public.workshop_admin_blocks a;
create temp table clock_test_assignments as select id,to_jsonb(a) data from public.workshop_booking_assignments a;
...
begin
 ...
 r:=public.workshop_clock_tick(true,test_at+interval '1 minute'); insert into clock_test_results values('next_minute',r);
 if r->>'ok'<>'true' then raise exception 'Next minute rejected: %',r->'issues'; end if;
 if exists(select 1 from public.vehicle_work_items w join clock_test_work o using(id) where to_jsonb(w) is distinct from o.data) then raise exception 'Work state changed'; end if;
 if exists(select 1 from public.workshop_admin_blocks a join clock_test_admin o using(id) where to_jsonb(a) is distinct from o.data) then raise exception 'Admin reservation changed'; end if;
 if exists(select 1 from public.workshop_booking_assignments a join clock_test_assignments o using(id) where (to_jsonb(a)-array['scheduled_start_at','scheduled_end_at','updated_at']) is distinct from (o.data-array['scheduled_start_at','scheduled_end_at','updated_at'])) then raise exception 'Technician assignment identity changed'; end if;
