...
ROOT=Path(__file__).resolve().parents[1]; MIGRATION=ROOT/'supabase/staging_only/20260901236000_pdc_email_ai_v2_canonical_nested_eta_history_guard_20260901.sql'; BOOT=Path(r'C:/Users/nwmgr/AppData/Local/hermes/staging-bootstrap/pdc_staging_bootstrap.py'); SEC=Path(r'C:/Users/nwmgr/AppData/Local/hermes/staging-secrets/pdc-staging.dpapi'); REF='cdsmnqxtyyoeoznmbidd'; PROD='vjdtsswhroyguxyfjdkt'; PRE=('20260901235000','pdc_email_ai_v2_canonical_eta_history_capability_20260901'); TARGET=('20260901236000','pdc_email_ai_v2_canonical_nested_eta_history_guard_20260901'); ENV='PDC_APPROVE_STAGING_MIGRATION_20260901236000'
...
def bundle():
 ...
 if REF not in u or PROD in u:raise RuntimeError('PDC_NESTED_ETA_NON_STAGING_TARGET')
 return d
...
def main():
 digest=hashlib.sha256(MIGRATION.read_bytes()).hexdigest();expected=f'apply migration 20260901236000 pdc email ai v2 canonical nested eta history guard source {digest}'
 if os.environ.get(ENV)!=expected:raise RuntimeError('PDC_NESTED_ETA_APPROVAL_MISSING_OR_HASH_MISMATCH')
 import psycopg2
 d=bundle();c=psycopg2.connect(d['PDC_STAGING_DATABASE_URL'],sslmode='verify-full',sslrootcert=d['PDC_STAGING_SSLROOTCERT'],application_name='pdc-email-ai-v2-canonical-nested-eta-staging-controller');c.autocommit=False
 try:
  cur=c.cursor();before=state(cur)
  if before['head'] not in {PRE,TARGET}:raise RuntimeError(f'PDC_NESTED_ETA_UNEXPECTED_HEAD:{before["head"]}')
  if before['production']:raise RuntimeError('PDC_NESTED_ETA_PRODUCTION_SENTINEL_PRESENT')
