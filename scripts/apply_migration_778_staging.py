...
def values() -> dict[str, str]:
    spec = importlib.util.spec_from_file_location("pdc_778_bootstrap", BOOTSTRAP)
    ...
    if REF not in dsn or PROD in dsn:
        raise RuntimeError("PDC_778_NON_STAGING_DATABASE_TARGET")
    return data
...
def main() -> None:
    ...
    expected = f"apply migration 778 source {digest}"
    ...
    import psycopg2
    v = values()
    conn = psycopg2.connect(v["PDC_STAGING_DATABASE_URL"], sslmode="verify-full", sslrootcert=v["PDC_STAGING_SSLROOTCERT"], application_name="pdc-website-development-778-controller")
    ...
    try:
        ...
        if head != "(20260830166000,777_historical_reconciliation_canonical_adapter_repair2)":
            raise RuntimeError(f"PDC_778_UNEXPECTED_LIVE_HEAD:{head}")
        if scalar(cur, "select to_regclass('public.pdc_production_environment_sentinel') is not null"):
            raise RuntimeError("PDC_778_PRODUCTION_SENTINEL_PRESENT")
        cur.execute(MIGRATION.read_text(encoding="utf-8"))
        conn.commit()
        ledger = scalar(cur, "select (version,name)::text from supabase_migrations.schema_migrations where version='20260830170000'")
        result = {
            "ok": ledger == "(20260830170000,778_historical_reconciliation_enqueue_adapter)",
            "environment": "staging",
            "project_ref": REF,
            "migration_sha256": digest,
            "ledger": ledger,
            "authorization_rows": scalar(cur, "select count(*) from public.pdc_historical_reconciliation_writer_authorizations_773"),
        ...
        if not result["ok"] or result["authorization_rows"] != 15 or result["reference_stock_rows"] != 0 or result["provider_observation_rows"] != 0 or result["reconciliation_receipt_rows"] != 0 or result["auth_rls_forced"] is not True or result["receipt_rls_forced"] is not True or result["rpc_exists"] is not True or result["production_sentinel_present"] is not False:
            raise RuntimeError("PDC_778_POST_APPLY_READBACK_FAILED:" + json.dumps(result, sort_keys=True, default=str))
