...
EXPECTED_HEAD = "(20260830170000,778_historical_reconciliation_enqueue_adapter)"
...
def main() -> None:
    ...
    expected = f"apply migration 778 successor 1710 source {digest}"
    ...
    try:
        ...
        if head != EXPECTED_HEAD:
            raise RuntimeError(f"PDC_778_1710_UNEXPECTED_LIVE_HEAD:{head}")
        if scalar(cur, "select to_regclass('public.pdc_production_environment_sentinel') is not null"):
            raise RuntimeError("PDC_778_1710_PRODUCTION_SENTINEL_PRESENT")
        cur.execute(MIGRATION.read_text(encoding="utf-8"))
        conn.commit()
        result = {
            "ok": scalar(cur, "select count(*)=1 from supabase_migrations.schema_migrations where version='20260830171000' and name='778_historical_reconciliation_security_successor'"),
        ...
        if not result["ok"] or result["provider_observation_rows"] != 0 or result["reconciliation_receipt_rows"] != 0 or result["reference_stock_rows"] != 0 or result["auth_rls_forced"] is not True or result["receipt_rls_forced"] is not True or result["rpc_exists"] is not True or result["production_sentinel_present"] is not False:
            raise RuntimeError("PDC_778_1710_POST_APPLY_READBACK_FAILED:" + json.dumps(result, sort_keys=True, default=str))
