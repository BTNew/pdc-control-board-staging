...
def _normalize_claim(claim, label):
    ...
    if scoped and not isinstance(claim.get("source_system"), str):
        raise VehicleReferenceArtifactError(f"{label} scoped identifier lacks source_system")
    if not scoped and claim.get("source_system") is not None:
...
def _normalize_item(item):
    ...
    if isinstance(item.get("version"), bool) or not isinstance(item.get("version"), int) or item["version"] < 1:
    ...
    claims = sorted((_normalize_claim(row, f"vehicle {vehicle_id}") for row in item["identifiers"]), key=_claim_key)
...
def _normalize_conflict(conflict):
    ...
    clean_candidates.sort(key=_canonical_json)
    return {
...
def _validate_completion(completion, items):
    ...
    if "terminal_cursor" not in completion:
        raise VehicleReferenceArtifactError("terminal cursor is missing")
...
def validate_vehicle_reference_artifact(artifact, *, expected_resolver_revision):
    ...
    if isinstance(expected_resolver_revision, bool) or not isinstance(expected_resolver_revision, int):
    ...
    conflicts = sorted((_normalize_conflict(row) for row in artifact["conflicts"]), key=_canonical_json)
...
def build_vehicle_reference_artifact(export_data, *, generated_at, source_environment):
    ...
    artifact["conflicts"] = sorted((_normalize_conflict(row) for row in artifact["conflicts"]), key=_canonical_json)
