...
$ErrorActionPreference='Stop';$ExpectedVersion='2026.08.47';$control=Join-Path $InstallRoot ("control\"+$ExpectedVersion);$trust=Join-Path $InstallRoot ("trust\"+$ExpectedVersion);$dispatch=Join-Path $control 'run-current-active.ps1';$anchor=Join-Path $trust 'ACTIVE_DISPATCH_SHA256'
