...
def main():
 ...
 def complete_work(label,no,key,*,changed_probe=False,stale_probe=False):
  ...
  if not item or not item["required"] or not item["completed"] or not item.get("completed_at") or not item.get("completed_by"): raise RuntimeError(f"{label} explicit synthetic completion evidence")
  ...
  if stale_probe:
   ...
   stale_rejected=True
  actions.append({"label":label,"scenario_no":no,"kind":"work_states","action":"explicit_synthetic_completion","work_key":key,"expected_ok":True,"http_status":status,"receipt_id":response.get("receipt_id"),"version_before":v["version"],"version_after":after["vehicle"]["version"],"replay_verified":True,"changed_payload_rejected":changed_rejected,"stale_version_rejected":stale_rejected,"physical_work_claimed":False,"pre_action_migration_head":pre["database"]["migration_head"]})
  return after
 ...
 if not (v13["current_location"]=="RFT" and v13["lifecycle_state"]=="rft" and v13.get("qc_completed_at") and v13.get("rft_transferred_at") and v13.get("collected_at") is None): raise RuntimeError("013 RFT separation")
 if not (v14["lifecycle_state"]=="collected" and v14.get("qc_completed_at") and v14.get("rft_transferred_at") and v14.get("collected_at")): raise RuntimeError(f"014 Completed separation {json.dumps(v14)[:1000]}")
