...
def main():
 ...
 def schedule(label,scenario_no,stage,bay,start,duration,expect_success,conflict_kind=None):
  ...
  http_rejection=status!=200
  if expect_success and (status!=200 or response.get("replay") not in (False,True) or response.get("notification_delta")!=0):raise RuntimeError(f"{label} wrapper failure {status} {json.dumps(response)[:1200]}")
  if not expect_success and status not in (200,400):raise RuntimeError(f"{label} unexpected rejection transport {status} {json.dumps(response)[:1200]}")
  if not expect_success and status==200 and (response.get("replay") not in (False,True) or response.get("notification_delta")!=0):raise RuntimeError(f"{label} rejection wrapper failure {status} {json.dumps(response)[:1200]}")
  was_replay=status==200 and response.get("replay") is True
  ...
  if expect_success:
   ...
   if (not http_rejection and response.get("ok") is not False) or "conflict" not in text or (conflict_kind and conflict_kind not in text):raise RuntimeError(f"{label} conflict not enforced {json.dumps(response)[:1200]}")
   ...
   booking=None
  expected_receipt_count=len(before_receipts)+(0 if was_replay or http_rejection else 1)
 ...
 schedule("scenario-007-vehicle-conflict-exact59",7,"ELECTRICAL",10,"2026-08-26T02:30:00+00:00",59,False,"vehicle")
