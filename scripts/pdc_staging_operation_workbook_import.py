...
def classify(description:str)->str:
 ...
 for pat,value in [
  ...
  (r"(^| )(fit|install|long range( fuel)? tank|tow ?bar)( |$)","fitting")]:
  if re.search(pat,d):key=value;break
 dn=" ".join(description.lower().replace("-"," ").split())
 if "reflective stripe" in dn:key="sublet"
 if any(x in dn for x in ("bullbar","bull bar","bulbar")):key="fitting"
 if re.search(r"\bfire\b.*\bextin",dn):key="fabrication"
...
def payload()->tuple[list[dict],dict]:
 ...
 if len(p)!=166 or rows!=1488 or counts["UNMAPPED"]!=994 or rows-counts["UNMAPPED"]!=494:raise RuntimeError("PDC_WORKBOOK_CLASSIFICATION_COUNT_DRIFT")
 ...
 return p,{"workbook_sha256":sha,"pair_count":len(p),"operation_count":rows,"mapped_operation_count":494,"quarantined_operation_count":994,"work_key_counts":dict(sorted(counts.items())),"unknown_hours":sum(o["estimated_hours"] is None for x in p for o in x["operations"]),"explicit_zero_hours":sum(o["estimated_hours"]==0 for x in p for o in x["operations"])}
