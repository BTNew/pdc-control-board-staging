...
PREDECESSOR = ("20260901050000", "pdc_email_ai_typed_action_v2_contract_binding_20260901")
TARGET = ("20260901060000", "pdc_email_ai_typed_action_boundary_hardening_20260901")
APPROVAL_ENV = "PDC_APPROVE_STAGING_MIGRATION_20260901060000"
...
def bundle():
    if not BOOTSTRAP.is_file() or not SECRETS.is_file():
        raise RuntimeError("PDC_TYPED_ACTION_HARDENING_PROTECTED_STAGING_CONNECTOR_UNAVAILABLE")
    spec = importlib.util.spec_from_file_location("pdc_staging_bootstrap", BOOTSTRAP)
    if spec is None or spec.loader is None:
        raise RuntimeError("PDC_TYPED_ACTION_HARDENING_STAGING_CONNECTOR_INVALID")
    ...
    if STAGING_REF not in url or PRODUCTION_REF in url:
        raise RuntimeError("PDC_TYPED_ACTION_HARDENING_NON_STAGING_TARGET")
    return data


def main():
    digest = hashlib.sha256(MIGRATION.read_bytes()).hexdigest()
    expected = f"apply migration 20260901060000 pdc email ai typed action boundary hardening source {digest}"
    if os.environ.get(APPROVAL_ENV) != expected:
        raise RuntimeError("PDC_TYPED_ACTION_HARDENING_APPROVAL_MISSING_OR_HASH_MISMATCH")
    data = bundle()
    import psycopg2

    conn = psycopg2.connect(data["PDC_STAGING_DATABASE_URL"], sslmode="verify-full", sslrootcert=data["PDC_STAGING_SSLROOTCERT"], application_name="pdc-email-ai-typed-action-hardening-staging-controller")
    ...
    try:
        ...
        if head != PREDECESSOR:
            raise RuntimeError(f"PDC_TYPED_ACTION_HARDENING_UNEXPECTED_LIVE_HEAD:{head}")
        if one(cur, "select to_regclass('public.pdc_production_environment_sentinel') is not null")[0]:
            raise RuntimeError("PDC_TYPED_ACTION_HARDENING_PRODUCTION_SENTINEL_PRESENT")
        cur.execute(MIGRATION.read_text(encoding="utf-8"))
        conn.commit()
        cur = conn.cursor()
        ledger = tuple(one(cur, "select version,name from supabase_migrations.schema_migrations where version='20260901060000'") or ())
        funcs = {
        ...
        if not proof["ok"]:
            raise RuntimeError("PDC_TYPED_ACTION_HARDENING_POST_APPLY_READBACK_FAILED")
    except Exception:
        conn.rollback()
        raise
    finally:
