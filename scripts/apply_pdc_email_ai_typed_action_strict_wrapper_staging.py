#!/usr/bin/env python3
"""Apply and prove the append-only v2 strict-wrapper correction in STAGING only."""
...
PREDECESSOR = ("20260901090000", "pdc_email_ai_typed_action_timestamp_acl_20260901")
TARGET = ("20260901100000", "pdc_email_ai_typed_action_strict_wrapper_20260901")
APPROVAL_ENV = "PDC_APPROVE_STAGING_MIGRATION_20260901100000"
...
def bundle():
    if not BOOTSTRAP.is_file() or not SECRETS.is_file():
        raise RuntimeError("PDC_TYPED_ACTION_STRICT_WRAPPER_PROTECTED_STAGING_CONNECTOR_UNAVAILABLE")
    spec = importlib.util.spec_from_file_location("pdc_staging_bootstrap", BOOTSTRAP)
    if spec is None or spec.loader is None:
        raise RuntimeError("PDC_TYPED_ACTION_STRICT_WRAPPER_STAGING_CONNECTOR_INVALID")
    ...
    if STAGING_REF not in url or PRODUCTION_REF in url:
        raise RuntimeError("PDC_TYPED_ACTION_STRICT_WRAPPER_NON_STAGING_TARGET")
    return data
...
def probe_plan(vehicle):
    ...
    item = {
        ...
        "payload": {"text": "read-only strict-wrapper contract probe", "event_at": "2026-09-01T00:00:00+00:00"},
        ...
        "provenance": versions.copy(), "audit_event_ref": "audit-plan-0001", "reason": "read-only strict-wrapper contract probe",
    }
    return {
...
def validation_probes(cur, plan):
    ...
    for field, value in (
        ...
        ("aggregate_disposition", "forged"),
        ("planner_status", "forged"),
        ("created_at", "not-a-date"),
    ):
        candidate = copy.deepcopy(plan)
        candidate[field] = value
        malformed[field] = not check(candidate)
...
def main():
    ...
    expected = f"apply migration 20260901100000 pdc email ai typed action strict wrapper source {digest}"
    if os.environ.get(APPROVAL_ENV) != expected:
        raise RuntimeError("PDC_TYPED_ACTION_STRICT_WRAPPER_APPROVAL_MISSING_OR_HASH_MISMATCH")
    data = bundle()
    import psycopg2

    conn = psycopg2.connect(data["PDC_STAGING_DATABASE_URL"], sslmode="verify-full", sslrootcert=data["PDC_STAGING_SSLROOTCERT"], application_name="pdc-email-ai-typed-action-strict-wrapper-staging-controller")
    ...
    try:
        ...
        if head not in {PREDECESSOR, TARGET}:
            raise RuntimeError(f"PDC_TYPED_ACTION_STRICT_WRAPPER_UNEXPECTED_LIVE_HEAD:{head}")
        if one(cur, "select to_regclass('public.pdc_production_environment_sentinel') is not null")[0]:
            raise RuntimeError("PDC_TYPED_ACTION_STRICT_WRAPPER_PRODUCTION_SENTINEL_PRESENT")
        ...
        if not already_applied:
            ...
            conn.commit()
        cur = conn.cursor()
        ledger = tuple(one(cur, "select version,name from supabase_migrations.schema_migrations where version='20260901100000'") or ())
        ...
        if not vehicle:
            raise RuntimeError("PDC_TYPED_ACTION_STRICT_WRAPPER_NO_PROBE_VEHICLE")
        ...
        full_validator_bound = "pdc_email_ai_successor_validate_v2_plan_20260901(p_plan)" in strict_definition
        full_validator_precedes_dispatch = full_validator_bound and strict_definition.index("pdc_email_ai_successor_validate_v2_plan_20260901(p_plan)") < strict_definition.index("apply_pdc_email_ai_operation_update_transaction_20260901")
        ...
        proof = {
            "ok": ledger == TARGET and strict_acl == (True, False, False, False) and low_level_acl == (False, False, False, False) and alias_acl == (False, False, False, False) and valid and all(malformed.values()) and full_validator_bound and full_validator_precedes_dispatch and before_counts == after_counts and not production,
            ...
            "strict_wrapper_full_validator_bound": full_validator_bound, "full_validator_precedes_dispatch": full_validator_precedes_dispatch,
            "probes": {"valid_plan": valid, "malformed_top_level_rejected": malformed},
        ...
        if not proof["ok"]:
            raise RuntimeError("PDC_TYPED_ACTION_STRICT_WRAPPER_POST_APPLY_READBACK_FAILED")
    except Exception:
        conn.rollback()
        raise
    finally:
