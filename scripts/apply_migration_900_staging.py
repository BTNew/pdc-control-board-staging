...
MIGRATION = ROOT / "supabase/staging_only/20260830090000_sublet_auditor_read_ledger.sql"
...
EXPECTED_PREDECESSOR = ("20260830081000", "stock_13017855_restore_navision_parity_successor")
EXPECTED_VERSION = "20260830090000"
EXPECTED_NAME = "sublet_auditor_read_ledger"


def load_bootstrap():
    spec = importlib.util.spec_from_file_location("pdc_staging_bootstrap", BOOTSTRAP)
    if spec is None or spec.loader is None:
        raise RuntimeError("staging bootstrap could not be loaded")
...
def main() -> None:
    ...
    expected_approval = f"apply migration 900 source {digest}"
    if os.environ.get("PDC_APPROVE_STAGING_MIGRATION_900") != expected_approval:
    ...
    connection = psycopg2.connect(
        ...
        application_name="pdc-staging-sublet-read-900",
    )
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("select to_regprocedure('public.get_pdc_sublet_audit_ledgers(uuid,text,text)') is not null")
            rpc_exists = bool(cursor.fetchone()[0])
            cursor.execute("select has_table_privilege('authenticated','public.pdc_sublet_booking_instances','select'), has_table_privilege('authenticated','public.pdc_sublet_booking_instance_history','select'), has_table_privilege('authenticated','public.pdc_sublet_email_update_receipts','select')")
            direct_select = tuple(cursor.fetchone())
            cursor.execute("select has_function_privilege('authenticated','public.get_pdc_sublet_audit_ledgers(uuid,text,text)','execute')")
            rpc_execute = bool(cursor.fetchone()[0])
            if applied != (EXPECTED_VERSION, EXPECTED_NAME) or not rpc_exists or direct_select != (False, False, False) or not rpc_execute:
                raise RuntimeError(f"post-apply ACL/catalog proof failed: applied={applied!r} rpc_exists={rpc_exists!r} direct_select={direct_select!r} rpc_execute={rpc_execute!r}")
            print(json.dumps({
                "ok": True,
                "applied": applied,
                "migration_sha256": digest,
                "rpc_exists": rpc_exists,
                "direct_table_select_authenticated": direct_select,
                "rpc_execute_authenticated": rpc_execute,
