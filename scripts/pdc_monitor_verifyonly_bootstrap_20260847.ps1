...
$ErrorActionPreference='Stop';$ExpectedVersion='2026.08.47';$control=Join-Path $InstallRoot ("control\"+$ExpectedVersion);$trust=Join-Path $InstallRoot ("trust\"+$ExpectedVersion);$runner=Join-Path $control 'run-current-verifyonly.ps1';$anchor=Join-Path $trust 'VERIFYONLY_RUNNER_SHA256'
