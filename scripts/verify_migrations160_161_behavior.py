...
def main():
 ...
 try:
  with con.cursor() as c:
   ...
   claims(c,'authenticated',actor,email);c.execute("select public.process_pdc_email_communication(%s,%s,%s,%s,'pdc-monitor')",(mi,mp,sha(json.dumps(mx,sort_keys=True)),Json(mx)));assert c.fetchone()[0].get('code')=='communication_vehicle_identity_disagreement'
   ...
   fail_text=f'Stock {fail_stock}. Parts Complete. Add long range tank. Sublet booked 22/08/2026.'
