#!/usr/bin/env python3
"""Apply reviewed migration 051 to the approved staging project only."""
...
MIGRATION = ROOT / 'supabase/migrations/051_bus4x4_eight_bays_and_completion_hardening.sql'
NAME = 'bus4x4_eight_bays_and_completion_hardening'
...
def main():
    ...
    try:
        backup_evidence = validate_release_backup(
            ...
            expected_migration='050',
        )
        conn.rollback()
        cursor = conn.cursor()
        cursor.execute('begin')
        cursor.execute("select version from supabase_migrations.schema_migrations where version in('050','051') order by version")
        ...
        cursor.execute("select pg_get_functiondef('public.complete_workshop_work(uuid,integer,text,timestamptz,jsonb)'::regprocedure)")
        function_def = cursor.fetchone()[0]
        for required in ("coalesce(p_actual_end_at, statement_timestamp())", "v_booking.status not in ('started','stoppage')", "'not_completable'"):
            if required not in function_def:
                raise RuntimeError(f'completion hardening missing: {required}')
        cursor.execute("insert into supabase_migrations.schema_migrations(version,name,statements) values('051',%s,%s)", (NAME, [sql]))
        conn.commit()
        print(json.dumps({
            'status': 'applied',
