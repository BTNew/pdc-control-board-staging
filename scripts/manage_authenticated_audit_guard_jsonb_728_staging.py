...
import psycopg2
R=Path(__file__).resolve().parents[1];M=R/'supabase/staging_only/20260828500000_728_agentic_audit_guard_jsonb_precedence.sql';SHA='5e10722b207fd9a8c22eb08e61528578262bc3c9ef5fcee7924750a35de7b2fb';REF='cdsmnqxtyyoeoznmbidd';PROD='vjdtsswhroyguxyfjdkt';APP='PDC_APPROVE_STAGING_MIGRATION_728'
def db():
...
def main():
 ...
 try:
  ...
  if hashlib.sha256(src.encode()).hexdigest()!=SHA:raise RuntimeError('728 source hash mismatch')
  pre=b['head']=='20260828490000' and not b['production'] and b['prior'] and not b['repair'] and b['audit_hash']=='5efe7c886ac48762866e87b56cdce095aa4d201f870d3c902b001596f99e2266' and b['uid514']==1 and b['receipt']==1 and b['vehicle']==1 and b['ops']==5
  if a.mode!='readback' and not pre:raise RuntimeError(f'728 prestate mismatch: {b}')
  if a.mode=='preflight':r.update(ok=True,before=b,committed=False)
  elif a.mode=='apply':
   if os.environ.get(APP)!=f'apply migration 728 audit guard source {SHA}':raise RuntimeError('approval phrase missing')
   with c:
    with c.cursor() as x:x.execute(src.removeprefix('BEGIN;\n').removesuffix('COMMIT;\n'))
   after=state(c)
   if after['head']!='20260828500000' or not after['repair'] or after['production'] or after['uid514']!=1 or after['receipt']!=1 or after['vehicle']!=1 or after['ops']!=5:raise RuntimeError(f'728 poststate mismatch: {after}')
   r.update(ok=True,before=b,after=after,committed=True)
  else:
   if b['head']!='20260828500000' or not b['repair'] or b['production']:raise RuntimeError(f'728 readback mismatch: {b}')
