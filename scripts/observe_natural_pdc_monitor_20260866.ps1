...
$ErrorActionPreference='Stop';$TaskName='PDC-PMB-Email-Monitor-Staging';$Expected='2026.08.66';$before=(Get-ScheduledTaskInfo -TaskName $TaskName).LastRunTime;$success=0;$seen=@();$started=Get-Date;$r=[IO.Path]::GetFullPath($InstallRoot)
try{
 ...
 $flags=@{};Get-Content (Join-Path $r 'config\2026.08.66\runtime.env')|ForEach-Object{if($_ -match '^(PDC_OUTBOUND_EMAIL_ENABLED|IMAP_BRIDGE_MARK_READ)=(.*)$'){$flags[$Matches[1]]=$Matches[2]}}
 ...
 while($success -lt 2 -and ((Get-Date)-$started).TotalMinutes -lt 18){
  ...
  if($info.LastRunTime -and $info.LastRunTime -gt $before -and $seen -notcontains $info.LastRunTime -and $state -eq 'Ready'){
   ...
   if($status.release_version -ne $Expected -or $status.code -ne 'PDC_MONITOR_SUCCESSOR_ACTIVE_CYCLE_COMPLETE' -or -not $status.mailbox_contacted -or $status.production_contacted -or $status.uid514_processed -or -not $proc.ok -or $proc.release -ne $Expected -or $proc.email_processing.failed -ne 0){throw 'natural receipt gate failed'}
   $success++;$before=$info.LastRunTime
  }
 }
 ...
 [ordered]@{ok=$true;natural_runs=$success;task_state=$task.State;last_task_result=$info.LastTaskResult;last_run=$info.LastRunTime;next_run=$info.NextRunTime;release=$Expected;production_contacted=$false;mailbox_flags_changed=$false;outbound_email_enabled=$false}|ConvertTo-Json -Compress;exit 0
