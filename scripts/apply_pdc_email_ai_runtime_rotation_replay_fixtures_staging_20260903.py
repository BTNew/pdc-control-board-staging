...
EVIDENCE = ROOT / "review-evidence/t_f708d28a/runtime-rotation-replay-fixtures-proof.json"
...
# Auditable command surface: supabase db query --linked
SUPABASE_QUERY = ("supabase", "db", "query", "--linked", "--file")
SUPABASE_SCRIPT = Path.home() / ".local/bin/supabase"
...
def run_query(path: Path) -> dict:
    ...
    if not bash or not SUPABASE_SCRIPT.is_file():
        raise RuntimeError("SUPABASE_CLI_NOT_FOUND")
    completed = subprocess.run(
        [bash, str(SUPABASE_SCRIPT), *SUPABASE_QUERY[1:], str(path)],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=False,
    ...
    if completed.returncode:
        raise RuntimeError(
            "SUPABASE_STAGING_QUERY_FAILED:"
            f"stdout={completed.stdout.strip()} stderr={completed.stderr.strip()}"
        )
    start = completed.stdout.find("{")
    if start < 0:
        raise RuntimeError("SUPABASE_STAGING_QUERY_JSON_MISSING")
    return json.loads(completed.stdout[start:])


def linked_staging() -> None:
    if not LINK.is_file() or not PROJECT_REF_FILE.is_file() or not POOLER_URL_FILE.is_file():
        raise RuntimeError("PDC_STAGING_LINK_REQUIRED")
    linked = json.loads(LINK.read_text(encoding="utf-8"))
    if linked.get("ref") != STAGING_REF or linked.get("ref") == PRODUCTION_REF:
        raise RuntimeError("PDC_NON_STAGING_LINK_REFUSED")
    if PROJECT_REF_FILE.read_text(encoding="utf-8").strip() != STAGING_REF:
        raise RuntimeError("PDC_STAGING_PROJECT_REF_ARTIFACT_MISMATCH")
    pooler_text = POOLER_URL_FILE.read_text(encoding="utf-8").strip()
    pooler = urllib.parse.urlparse(pooler_text)
    if (
        pooler.scheme != "postgresql"
        or pooler.username != f"postgres.{STAGING_REF}"
        or not (pooler.hostname or "").endswith(".pooler.supabase.com")
    ):
        raise RuntimeError("PDC_STAGING_POOLER_ARTIFACT_MISMATCH")
    if PRODUCTION_REF in pooler_text:
        raise RuntimeError("PDC_PRODUCTION_POOLER_REFUSED")
