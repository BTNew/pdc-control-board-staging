"""Controlled Stage 2B C5 real-data pilot for the exact guarded staging project.
...
class C5PilotRefusal(RuntimeError):
    pass
...
def assert_exact_project_guard(project_ref_path=PROJECT_REF_PATH, database_url=None):
    ...
    if not path.is_file() or path.read_text(encoding="utf-8").strip() != STAGING_REF:
        raise C5PilotRefusal("refusing C5 pilot: linked project is not exact staging")
    ...
    if (parsed.scheme not in {"postgres", "postgresql"} or parsed.path != "/postgres"
        ...
        raise C5PilotRefusal("refusing C5 pilot: DSN does not identify exact staging")
    return True
...
def load_approved_c4(zip_path):
    ...
    if sha256_bytes(raw) != APPROVED_C4_SHA256:
        raise C5PilotRefusal("C4 package checksum is not the approved checksum")
    ...
    if summary["classification_counts"]["clean"] != 192:
        raise C5PilotRefusal("approved C4 clean-record count changed")
...
def select_records(payload):
    ...
    if len(selected) != SELECTED_COUNT or summary["classification_counts"]["clean"] != 192:
        raise C5PilotRefusal("C5 deterministic clean selection is unavailable")
    return selected
...
def source_payload(row):
    ...
    if not all(isinstance(value, str) and value.strip() for value in payload.values()):
        raise C5PilotRefusal(f"selected source identity is incomplete: {row['record_ref']}")
    return payload
...
def selected_manifest(selected, source_assessment_sha256):
    ...
    logical = {
        "schema": "pdc.stage2b.c5-selected-records/v1",
        "approved_c4_sha256": APPROVED_C4_SHA256,
        "approved_c4_head": APPROVED_C4_HEAD,
        "source_assessment_sha256": source_assessment_sha256,
        "selection_rule": "UTF-8 record_ref ascending; first 5 eligible clean attachment-free records",
...
def validate_backup_evidence(manifest_path, restore_report_path):
    ...
    if not manifest_path.name.endswith(".manifest.json"):
        raise C5PilotRefusal("backup manifest filename is invalid")
    ...
    if not backup_file.is_file() or sha256_bytes(backup_file.read_bytes()) != manifest.get("file_sha256"):
        raise C5PilotRefusal("fresh encrypted backup checksum verification failed")
    if manifest.get("environment") != "staging" or manifest.get("migration_version") != "031" or manifest.get("encrypted") is not True:
        raise C5PilotRefusal("backup environment, ledger, or encryption evidence is invalid")
    if (restore.get("backup_run_id") != manifest.get("backup_run_id")
        ...
        raise C5PilotRefusal("isolated backup restore test did not pass exactly")
    ...
    if age_seconds < 0 or age_seconds > 6 * 60 * 60:
        raise C5PilotRefusal("backup is not fresh enough for this C5 execution")
    return {
        "schema": "pdc.stage2b.c5-backup-restore/v1",
...
def _admin(conn):
    ...
    row = cur.fetchone()
    if not row:
        raise C5PilotRefusal("staging administrator fixture is unavailable")
    return row
...
def _backup_vehicle_hash(manifest_path):
    ...
    if not key:
        raise C5PilotRefusal("backup encryption key is required for baseline comparison")
...
def validate_preview_response(record_ref, first, second):
    if first != second:
        raise C5PilotRefusal(f"preview is not deterministic: {record_ref}")
    data = first.get("data", {}) if isinstance(first, dict) else {}
    if first.get("ok") is not True or first.get("code") != "ok":
        raise C5PilotRefusal(f"preview refused selected record: {record_ref}: {first.get('code')}")
    if data.get("action") not in {"insert", "update", "no_change"}:
        raise C5PilotRefusal(f"preview action is invalid: {record_ref}")
    ...
    if len(candidates) > 1 or any(len(value or []) > 1 for value in candidate_sets.values()):
        raise C5PilotRefusal(f"preview contains ambiguity/conflict: {record_ref}")
    if data.get("action") != "insert" and data.get("expected_version") is None:
        raise C5PilotRefusal(f"update/no-op preview lacks version lock: {record_ref}")
...
def _typed_item(reference, vehicle_id):
    ...
    if len(matches) != 1:
        raise C5PilotRefusal(f"migration 031 artifact did not contain one selected UUID: {vehicle_id}")
    return matches[0]


def _validator_cli(extract, reference, revision):
    with tempfile.TemporaryDirectory(prefix="pdc-c5-validator-") as temp:
        temp = Path(temp)
    ...
    if completed.returncode:
        raise C5PilotRefusal("C2b Node validator refused the selected batch")
...
def _build_rollback_export(conn, vehicle_ids, batch_id, resolver_revision):
    ...
    logical = {
        "schema": "pdc.stage2b.c5-rollback-export/v1",
...
def validate_rollback_export(export, expected_revision):
    ...
    if (export.get("schema") != "pdc.stage2b.c5-rollback-export/v1"
        ...
        raise C5PilotRefusal("rollback export revision, schema, or checksum is invalid")
    return True
...
def _lock_restored_vehicle_master_revision(cur, schema, expected_revision):
    ...
    if not row or row[0] != expected_revision:
        raise C5PilotRefusal("rollback export is stale against restored database revision")
    return row[0]
...
def rehearse_rollback(conn, export, schema, restore_report):
    ...
    if (not re.fullmatch(r"c5_full_rollback_[0-9a-f]{12}", schema or "")
        ...
        raise C5PilotRefusal("full rollback restore schema/report is invalid")
    ...
    try:
        stale_refused = False
        try:
            _lock_restored_vehicle_master_revision(cur, schema, revision + 1)
        except C5PilotRefusal:
            stale_refused = True
        if not stale_refused:
            raise C5PilotRefusal("stale rollback revision was not refused")
        ...
        if validated_foreign_keys != restore_report.get("foreign_keys_added") or restored_indexes < 1:
            raise C5PilotRefusal("restored schema constraints/indexes are incomplete")
        for table in ROLLBACK_TABLES:
            ...
            if selected_rows != export["tables"][table]:
                raise C5PilotRefusal(f"restored rollback rows differ from export: {table}")
        ...
        for table in (
            ...
            if cur.rowcount != before[table]:
                raise C5PilotRefusal(f"full rollback deleted an unexpected row count: {table}")
        ...
        if any(after.values()) or unrelated_before != unrelated_after:
            raise C5PilotRefusal("full isolated rollback left selected residue or changed unrelated rows")
        conn.commit()
    finally:
        cur = conn.cursor()
        ...
        conn.commit()
    return {
        "schema": "pdc.stage2b.c5-rollback-report/v2",
...
def write_evidence(evidence_dir, files):
    ...
    for name, value in files.items():
        ...
        if any(token in lowered for token in ("postgresql://", "postgres://", "password", "service_role")):
            raise C5PilotRefusal(f"prohibited credential token in evidence: {name}")
        if "customer_name" in lowered:
            parsed = json.loads(encoded)
            if name != "operational-proof.json" or _contains_non_null_key(parsed, "customer_name"):
                raise C5PilotRefusal(f"prohibited broad-data value in evidence: {name}")
...
def run_pilot(c4_zip, backup_manifest, restore_report, rollback_restore_report,
    ...
    batch_id = "C5-REAL-PILOT-" + c4_summary["source_assessment_sha256"][:12].upper()
    ...
    if ledger[-1:] != ["031"]:
        conn.close()
        raise C5PilotRefusal("staging migration ledger is not exactly through 031")
    unrelated_before = _backup_vehicle_hash(backup_manifest)
    admin = _admin(conn)

    idempotency_keys = {
        row["record_ref"]: f"C5-029-{index:02d}-{row['record_ref'].replace(':', '-').upper()}"
    ...
    if existing_receipts and set(existing_receipts) != set(idempotency_keys.values()):
        conn.close()
        raise C5PilotRefusal("partial prior C5 receipt set requires manual review")
    ...
    for row in selected:
        ...
        if recovering_completed_apply:
            ...
            if current_safe["action"] != "no_change" or safe["action"] != "insert":
                conn.close()
                raise C5PilotRefusal(f"completed C5 recovery state is not exact: {row['record_ref']}")
            first = original
        else:
            safe = current_safe
            if safe["action"] != "insert":
                conn.close()
                raise C5PilotRefusal(f"fresh C5 pilot expected insert, got {safe['action']}: {row['record_ref']}")
    ...
    preview_logical = {
        "schema": "pdc.stage2b.c5-preview/v1",
        "source_system": SOURCE_SYSTEM,
        "source_batch_id": batch_id,
    ...
    for index, row in enumerate(selected, 1):
        ...
        if (applied.get("ok") is not True or applied.get("code") != "applied"
            ...
            raise C5PilotRefusal(f"preview/apply parity failed: {row['record_ref']}")
        conn.commit()
        ...
        if replay != applied:
            conn.close()
            raise C5PilotRefusal(f"durable receipt replay differed: {row['record_ref']}")
    ...
    if (pilot_window_counts.get("schema") != "pdc.stage2b.c5-row-counts/v1"
            or pilot_window_counts.get("before") != initial_counts
            or pilot_window_counts.get("deltas", {}).get("vehicles") != SELECTED_COUNT):
        conn.close()
        raise C5PilotRefusal("original C5 pilot-window counts are invalid")
    ...
    if unrelated_before != unrelated_after:
        conn.close()
        raise C5PilotRefusal("an unrelated vehicle changed during C5")
    ...
    if (namespace_counts["vehicles"] != SELECTED_COUNT
        ...
        conn.close()
        raise C5PilotRefusal("selected namespace counts or conflict count are invalid")
    ...
    if reference != repeated_reference:
        conn.close()
        raise C5PilotRefusal("migration 031 artifact was not deterministic")
    ...
    for record_ref in sorted(source_by_ref):
        ...
        if not exact:
            conn.close()
            raise C5PilotRefusal(f"source/UUID/version/evidence reconciliation failed: {record_ref}")
    ...
    extract = {
        ...
        "bookings": [{
            "legacy_plan_id": f"C5-C2B-{index:02d}",
            ...
            "status": "planned", "raw_legacy_record": {"c5_reference_only": True},
        } for index, row in enumerate(selected, 1)],
    }
    classified = classify(extract, reference, expected_revision=revision)
    if len(classified["safely_matched"]) != SELECTED_COUNT or any(
            classified[key] for key in classified if key != "safely_matched"):
        conn.close()
        raise C5PilotRefusal("C2b classifier did not safely match exactly the imported batch")
    validator_hash = _validator_cli(extract, reference, revision)

    reconciliation_logical = {
        "schema": "pdc.stage2b.c5-reconciliation/v1",
    ...
    if retained_counts != namespace_counts:
        conn.close()
        raise C5PilotRefusal("isolated rollback rehearsal changed retained public pilot rows")
    cur = conn.cursor()
    cur.execute("select count(*) from pg_namespace where nspname like 'c5_full_rollback_%'")
    if cur.fetchone()[0] != 0:
        conn.close()
        raise C5PilotRefusal("temporary C5 schema remained after rehearsal")
    conn.close()

    apply_result = {
        "schema": "pdc.stage2b.c5-apply/v1", "source_system": SOURCE_SYSTEM,
        "source_batch_id": batch_id, "applied_count": len(applied_rows),
        "preview_apply_parity": True, "actions": applied_rows,
    }
    replay_evidence = {
        "schema": "pdc.stage2b.c5-replay/v1", "replay_count": len(replay_rows),
    ...
    row_counts = {
        "schema": "pdc.stage2b.c5-row-counts/v1", "before": initial_counts,
        "after": after_counts,
    ...
    safety = {
        "schema": "pdc.stage2b.c5-safety/v1", "exact_staging_project_ref": STAGING_REF,
        "migration_ledger": ledger, "migration_ledger_tip": "031",
        "operational_connection_uses_guarded_dsn": True,
        "production_contacted": False, "production_linked": False,
    ...
    summary = {
        "schema": "pdc.stage2b.c5-pilot-summary/v1", "selected_count": SELECTED_COUNT,
...
def main(argv=None):
    ...
    if not args.apply:
        raise C5PilotRefusal("live staging C5 execution requires explicit --apply")
    result = run_pilot(
