...
import psycopg2
R=Path(__file__).resolve().parents[1];M=R/'supabase/staging_only/20260828490000_727_agentic_action_guard_jsonb_precedence.sql';SHA='fb844418205019c087b7926c56c85f1f476042f9243266ad2d168d7cbe8019ff';REF='cdsmnqxtyyoeoznmbidd';PROD='vjdtsswhroyguxyfjdkt';APP='PDC_APPROVE_STAGING_MIGRATION_727'
def db():
...
def main():
 ...
 try:
  ...
  if hashlib.sha256(src.encode()).hexdigest()!=SHA:raise RuntimeError('727 source hash mismatch')
  pre=b['head']=='20260828480000' and not b['production'] and b['prior'] and not b['repair'] and b['trigger_hash']=='80e396fc3ab201b0b5621aca7c8eb34f10cabb0dd93af1df4ab6ed787519eaad' and b['apply_hash']=='973425b72a0a31240813779e282dcf17f0616f6e8bea4b47d41eefc600551887' and b['uid514']==1 and b['receipt']==1 and b['vehicle']==1 and b['ops']==5
  if a.mode!='readback' and not pre:raise RuntimeError(f'727 prestate mismatch: {b}')
  if a.mode=='preflight':r.update(ok=True,before=b,committed=False)
  elif a.mode=='apply':
   if os.environ.get(APP)!=f'apply migration 727 action guard source {SHA}':raise RuntimeError('approval phrase missing')
   with c:
    with c.cursor() as x:x.execute(src.removeprefix('BEGIN;\n').removesuffix('COMMIT;\n'))
   after=state(c)
   if after['head']!='20260828490000' or not after['repair'] or after['production'] or after['uid514']!=1 or after['receipt']!=1 or after['vehicle']!=1 or after['ops']!=5:raise RuntimeError(f'727 poststate mismatch: {after}')
   r.update(ok=True,before=b,after=after,committed=True)
  else:
   if b['head']!='20260828490000' or not b['repair'] or b['production']:raise RuntimeError(f'727 readback mismatch: {b}')
