...
$Version='2026.08.71';$Parent='2026.08.69';$Head='20260831380000';$Task='PDC-PMB-Email-AI-Successor-Staging';$RollbackTask='PDC-PMB-Email-Monitor-Staging'
...
$installerSid=[Security.Principal.WindowsIdentity]::GetCurrent().User.Value;$stalePartialRemoved=$false
...
function RemoveStalePartial([string]$Path){if(-not(Test-Path -LiteralPath $Path)){return};$step='takeown';try{& takeown.exe '/F' $Path '/A' '/R' '/D' 'Y'|Out-Null;if($LASTEXITCODE -ne 0){Fail 'PDC_MONITOR_071_STALE_PARTIAL_TAKEOWN_FAILED'};$step='acl-reset';& icacls.exe $Path '/reset' '/T' '/C' '/Q'|Out-Null;if($LASTEXITCODE -ne 0){Fail 'PDC_MONITOR_071_STALE_PARTIAL_ACL_RESET_FAILED'};$step='acl-grant';& icacls.exe $Path '/grant:r' '*S-1-5-32-544:(OI)(CI)(F)' "*$($script:installerSid):(OI)(CI)(F)" '/T' '/C' '/Q'|Out-Null;if($LASTEXITCODE -ne 0){Fail 'PDC_MONITOR_071_STALE_PARTIAL_ACL_RECOVERY_FAILED'};$step='attributes';& attrib.exe '-R' (Join-Path $Path '*') '/S' '/D' 2>$null;$step='remove';[IO.Directory]::Delete($Path,$true);if(Test-Path -LiteralPath $Path){Fail 'PDC_MONITOR_071_STALE_PARTIAL_REMOVE_FAILED'};$script:stalePartialRemoved=$true}catch{throw ("PDC_MONITOR_071_STALE_PARTIAL_RECOVERY_FAILED:$step`:$Path`:"+$_.Exception.Message)}}
try{
  ...
  $rollbackObj=Get-ScheduledTask -TaskName $RollbackTask -ErrorAction Stop;AssertTask $rollbackObj $true
  $taskObj=Get-ScheduledTask -TaskName $Task -ErrorAction Stop;if($taskObj.State -ne 'Disabled'){Disable-ScheduledTask -TaskName $Task -ErrorAction Stop|Out-Null;$taskObj=Get-ScheduledTask -TaskName $Task -ErrorAction Stop};AssertTask $taskObj $true
  ...
  New-Item -ItemType Directory -Path $stageRootControl -Force|Out-Null;Copy-Item -LiteralPath (Join-Path $bundle 'control-root\bootstrap.ps1') -Destination (Join-Path $stageRootControl 'bootstrap.ps1') -Force
  ...
  $venvLines=TreeLines $stageVenv;$controlHash=TreeHash $stageControl;foreach($name in @('active-bootstrap.ps1','active-dispatch.ps1','current-head-preflight.py')){$rel="control/2026.08.71/$name";$expectedControl=[string]$manifest.files.PSObject.Properties[$rel].Value.sha256;if(-not $expectedControl -or (Hash (Join-Path $stageControl $name)) -ne $expectedControl){Fail 'PDC_MONITOR_071_CONTROL_FILE_HASH_MISMATCH'}};$trustHash=Hash (Join-Path $stageTrust 'TRUST-VALUES.json');$venvHash=TreeHash $stageVenv
  Set-Content -LiteralPath (Join-Path $stageTrust 'VENV_SHA256.tsv') -Value $venvLines -Encoding ascii;Set-Content -LiteralPath (Join-Path $stageTrust 'CONTROL_SHA256') -Value $controlHash -Encoding ascii;Set-Content -LiteralPath (Join-Path $stageTrust 'TRUST_SHA256') -Value $trustHash -Encoding ascii
  ...
  if($cur -eq $Parent){foreach($partial in @($targetRelease,$targetControl,$targetTrust,$targetVenv,$targetConfig)){RemoveStalePartial $partial}}
  ...
  ProtectTree $targetRelease;ProtectTree $targetControl;ProtectTree $targetTrust;ProtectTree $targetVenv;ProtectTree $targetConfig
  AssertAcl $targetRelease;AssertAcl $targetControl;AssertAcl $targetTrust;AssertAcl $targetVenv;AssertAcl (Join-Path $root 'control')
  ...
  $receipt=[ordered]@{ok=$true;release_version=$Version;parent_release_version=$Parent;manifest_sha256=$ExpectedManifestSha256.ToLowerInvariant();parent_manifest_sha256=$ExpectedParentManifestSha256.ToLowerInvariant();storage_bridge_sha256=[string]$manifest.successor_patch.storage_bridge_sha256;processor_sha256=[string]$manifest.successor_patch.processor_sha256;venv_sha256=$venvHash;control_sha256=$controlHash;trust_sha256=$trustHash;current_staging_migration_head=$Head;stale_partial_removed=$stalePartialRemoved;task_enabled=($enabled.State -ne 'Disabled');task_started=$false;mailbox_contacted=$false;uid514_processed=$false;production_contacted=$false;outbound_email_sent=$false;rollback_versions=@('2026.08.68','2026.08.69');secrets_printed=$false}|ConvertTo-Json -Depth 8
