...
ROOT=Path(__file__).resolve().parents[1]; MIGRATION=ROOT/'supabase/staging_only/20260901233000_pdc_email_ai_v2_canonical_note_capability_refresh_20260901.sql'; BOOT=Path(r'C:/Users/nwmgr/AppData/Local/hermes/staging-bootstrap/pdc_staging_bootstrap.py'); SEC=Path(r'C:/Users/nwmgr/AppData/Local/hermes/staging-secrets/pdc-staging.dpapi'); REF='cdsmnqxtyyoeoznmbidd'; PROD='vjdtsswhroyguxyfjdkt'; PRE=('20260901232000','pdc_email_ai_v2_canonical_action_compatibility_20260901'); TARGET=('20260901233000','pdc_email_ai_v2_canonical_note_capability_refresh_20260901'); ENV='PDC_APPROVE_STAGING_MIGRATION_20260901233000'
...
def bundle():
 ...
 if REF not in u or PROD in u: raise RuntimeError('PDC_CANONICAL_NOTE_NON_STAGING_TARGET')
 return d
...
def main():
 digest=hashlib.sha256(MIGRATION.read_bytes()).hexdigest(); expected=f'apply migration 20260901233000 pdc email ai v2 canonical note capability refresh source {digest}'
 if os.environ.get(ENV)!=expected: raise RuntimeError('PDC_CANONICAL_NOTE_APPROVAL_MISSING_OR_HASH_MISMATCH')
 import psycopg2
 d=bundle(); c=psycopg2.connect(d['PDC_STAGING_DATABASE_URL'],sslmode='verify-full',sslrootcert=d['PDC_STAGING_SSLROOTCERT'],application_name='pdc-email-ai-v2-canonical-note-capability-staging-controller'); c.autocommit=False
 try:
  cur=c.cursor(); before=state(cur)
  if before['head'] not in {PRE,TARGET}: raise RuntimeError(f'PDC_CANONICAL_NOTE_UNEXPECTED_HEAD:{before["head"]}')
  if before['production']: raise RuntimeError('PDC_CANONICAL_NOTE_PRODUCTION_SENTINEL_PRESENT')
  already=before['head']==TARGET
  if not already: cur.execute(MIGRATION.read_text(encoding='utf-8'))
  after=state(cur); exe=source(cur,'public.pdc_email_ai_successor_execute_v2_20260901(jsonb)')
  proof={'ok':after['head']==TARGET and before['receipts']==after['receipts'] and not after['production'] and "ELSIF action_type='note_append' THEN PERFORM set_config" in exe,'environment':'staging','project_ref':REF,'migration_identity':TARGET,'migration_sha256':digest,'already_applied':already,'before':before,'after':after,'receipts_preserved':before['receipts']==after['receipts'],'production_writes':False,'mailbox_contacted':False,'outbound_email':False,'action_rpc_invoked':False}
  if not proof['ok']: raise RuntimeError('PDC_CANONICAL_NOTE_POSTCHECK_FAILED')
  c.commit(); out=ROOT/'review-evidence/v2-controlled/canonical-note-capability-apply-proof.json'; out.write_text(json.dumps(proof,sort_keys=True,indent=2)+'\n',encoding='utf-8'); print(json.dumps({'ok':True,'proof':str(out),'migration':TARGET,'migration_sha256':digest,'ledger_head':after['head'],'receipts_preserved':proof['receipts_preserved'],'production_writes':False,'mailbox_contacted':False,'outbound_email':False,'action_rpc_invoked':False},sort_keys=True))
