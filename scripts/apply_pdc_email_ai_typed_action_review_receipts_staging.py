#!/usr/bin/env python3
"""Apply and prove the append-only v2 review/non-dispatch correction in STAGING only."""
...
MIGRATION = ROOT / "supabase/staging_only/20260901110000_pdc_email_ai_typed_action_review_receipts_20260901.sql"
...
PREDECESSOR = ("20260901100000", "pdc_email_ai_typed_action_strict_wrapper_20260901")
TARGET = ("20260901110000", "pdc_email_ai_typed_action_review_receipts_20260901")
APPROVAL_ENV = "PDC_APPROVE_STAGING_MIGRATION_20260901110000"
...
def bundle():
    if not BOOTSTRAP.is_file() or not SECRETS.is_file():
        raise RuntimeError("PDC_TYPED_ACTION_REVIEW_RECEIPTS_PROTECTED_STAGING_CONNECTOR_UNAVAILABLE")
    spec = importlib.util.spec_from_file_location("pdc_staging_bootstrap", BOOTSTRAP)
    if spec is None or spec.loader is None:
        raise RuntimeError("PDC_TYPED_ACTION_REVIEW_RECEIPTS_STAGING_CONNECTOR_INVALID")
    ...
    if STAGING_REF not in url or PRODUCTION_REF in url:
        raise RuntimeError("PDC_TYPED_ACTION_REVIEW_RECEIPTS_NON_STAGING_TARGET")
    return data


def plan(vehicle, *, unresolved=False):
    vehicle_id = "33333333-3333-4333-8333-333333333333" if unresolved else str(vehicle[3])
    identity = {
        "vehicle_id": vehicle_id,
        "stock_number": None if unresolved else vehicle[0],
        "vin": None if unresolved else vehicle[1],
        "backend_record_id": None if unresolved else (str(vehicle[2]).lower() if vehicle[2] else None),
    }
    versions = {
    ...
    item = {
        "instruction_id": "probe-unresolved-0001" if unresolved else "probe-known-review-0001",
        "vehicle_id": vehicle_id, "identity": identity, "expected_state": {"vehicle_version": 1, "backend_revision": 0},
        "action_type": "note_append", "payload": {"text": "read-only review receipt contract probe", "event_at": "2026-09-01T00:00:00+00:00"},
        ...
        "provenance": versions.copy(), "audit_event_ref": "probe-audit-0001", "reason": "read-only review receipt contract probe",
    }
    return {
        "schema_version": "pdc-email-ai-plan-v1", "plan_id": "44444444-4444-4444-8444-444444444444" if unresolved else "11111111-1111-4111-8111-111111111111",
...
def validation_probes(cur, known, unresolved):
    def check(candidate):
        cur.execute("select public.pdc_email_ai_successor_validate_v2_plan_20260901(%s::jsonb)", (json.dumps(candidate),))
        return bool(cur.fetchone()[0])

    return {"known_review_plan": check(known), "unresolved_review_plan": check(unresolved)}
...
def main():
    ...
    expected = f"apply migration 20260901110000 pdc email ai typed action review receipts source {digest}"
    if os.environ.get(APPROVAL_ENV) != expected:
        raise RuntimeError("PDC_TYPED_ACTION_REVIEW_RECEIPTS_APPROVAL_MISSING_OR_HASH_MISMATCH")
    data = bundle()
    import psycopg2

    conn = psycopg2.connect(data["PDC_STAGING_DATABASE_URL"], sslmode="verify-full", sslrootcert=data["PDC_STAGING_SSLROOTCERT"], application_name="pdc-email-ai-typed-action-review-receipts-staging-controller")
    ...
    try:
        ...
        if head not in {PREDECESSOR, TARGET}:
            raise RuntimeError(f"PDC_TYPED_ACTION_REVIEW_RECEIPTS_UNEXPECTED_LIVE_HEAD:{head}")
        if one(cur, "select to_regclass('public.pdc_production_environment_sentinel') is not null")[0]:
            raise RuntimeError("PDC_TYPED_ACTION_REVIEW_RECEIPTS_PRODUCTION_SENTINEL_PRESENT")
        ...
        if not already_applied:
            ...
            conn.commit()
        cur = conn.cursor()
        ledger = tuple(one(cur, "select version,name from supabase_migrations.schema_migrations where version='20260901110000'") or ())
        strict_signature = "public.apply_pdc_email_ai_typed_action_surface_20260901_strict(jsonb)"
        helper_signature = "public.pdc_email_ai_successor_record_non_dispatch_v2_20260901(jsonb)"
        strict_acl = acl(cur, strict_signature)
        helper_acl = acl(cur, helper_signature)
        ...
        if not vehicle:
            raise RuntimeError("PDC_TYPED_ACTION_REVIEW_RECEIPTS_NO_PROBE_VEHICLE")
        probes = validation_probes(cur, plan(vehicle), plan(vehicle, unresolved=True))
        strict_definition = one(cur, "select pg_get_functiondef(%s::regprocedure)", (strict_signature,))[0] or ""
        helper_definition = one(cur, "select pg_get_functiondef(%s::regprocedure)", (helper_signature,))[0] or ""
        guard = "x->>'decision_disposition'<>\'planned\'"
        strict_has_non_dispatch_guard = guard in strict_definition and "pdc_email_ai_successor_record_non_dispatch_v2_20260901(p_plan)" in strict_definition
        ...
        helper_has_nullable_review_receipt = "unresolved_review_evidence" in helper_definition and "GENUINELY_AMBIGUOUS" in helper_definition and "canonical_rpc',NULL" in helper_definition and "action_rpc_invoked',false" in helper_definition
        ...
        proof = {
            "ok": ledger == TARGET and strict_acl == (True, False, False, False) and helper_acl == (False, False, False, False) and low_level_acl == (False, False, False, False) and alias_acl == (False, False, False, False) and all(probes.values()) and strict_has_non_dispatch_guard and non_dispatch_precedes_projection and helper_has_nullable_review_receipt and before_counts == after_counts and not production,
            ...
            "non_dispatch_helper_acl": helper_acl, "legacy_low_level_acl": low_level_acl, "compatibility_alias_acl": alias_acl,
            "strict_has_non_dispatch_guard": strict_has_non_dispatch_guard, "non_dispatch_precedes_projection": non_dispatch_precedes_projection,
            "helper_has_nullable_review_receipt": helper_has_nullable_review_receipt, "probes": probes,
        ...
        if not proof["ok"]:
            raise RuntimeError("PDC_TYPED_ACTION_REVIEW_RECEIPTS_POST_APPLY_READBACK_FAILED")
    except Exception:
        conn.rollback()
        raise
    finally:
