...
(async () => {
  ...
  try {
    await page.route('**/data-test-75.js*', async route => {
      const response = await route.fetch();
      const source = await response.text();
      const match = source.match(/window\.VEHICLE_TRACKING_DATA\s*=\s*(\{[\s\S]*\});?\s*$/);
      if (!match) throw new Error('Unable to expand the 75-vehicle fixture');
      const fixture = JSON.parse(match[1]);
      const baseRows = fixture.vehicles;
      fixture.vehicles = Array.from({ length: 4 }, (_, copyIndex) => baseRows.map((row, rowIndex) => {
        const clone = JSON.parse(JSON.stringify(row));
        const suffix = `-${copyIndex + 1}-${rowIndex + 1}`;
        ['id', 'stock', 'order', 'vin', 'registration', 'rego', 'keyNumber'].forEach(key => {
          if (clone[key]) clone[key] = `${clone[key]}${suffix}`;
        });
        return clone;
      })).flat();
      await route.fulfill({ response, body: `window.VEHICLE_TRACKING_DATA = ${JSON.stringify(fixture)};` });
    });
    ...
    if (initial.vehicles !== 300) throw new Error(`expected 300-vehicle fixture, got ${initial.vehicles}`);
    ...
    const report = {
      schema: 'pdc.station-first-300-fixture-performance/v1',
      targetMs: TARGET_MS,
