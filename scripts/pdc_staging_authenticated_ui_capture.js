...
const tag=String(process.argv[2]||'empty').replace(/[^a-z0-9_-]/gi,'_');
const out=path.resolve(`_staging_deployment_receipts/20260824_staging_ui_authenticated_${tag}.png`);
const evidence=path.resolve(`_staging_deployment_receipts/20260824_staging_ui_authenticated_${tag}.json`);
...
async function main(){
 ...
 loaded=once('Page.loadEventFired');await send('Page.reload',{ignoreCache:true});await loaded;
 let state;for(let i=0;i<160;i++){await sleep(750);state=await send('Runtime.evaluate',{expression:`(()=>({title:document.title,text:document.body.innerText,vehicleCards:document.querySelectorAll('[data-vehicle-id],.vehicle-card,.board-card').length,url:location.href}))()`,returnByValue:true});const t=state.result.value.text||'';if(/PDC staff sign-in/i.test(t)||(/Vehicle Locations/i.test(t)&&!/Checking PDC access/i.test(t)))break;}
 const textReady=state.result.value.text||'';if(/PDC staff sign-in/i.test(textReady))throw Error('authenticated UI remained on sign-in');if(/Checking PDC access/i.test(textReady)||!/Vehicle Locations/i.test(textReady))throw Error('authenticated UI did not reach Board');
 await sleep(15000);state=await send('Runtime.evaluate',{expression:`(()=>({title:document.title,text:document.body.innerText,vehicleCards:document.querySelectorAll('[data-vehicle-id],.vehicle-card,.board-card').length,url:location.href}))()`,returnByValue:true});const text=state.result.value.text||'';
 ...
 const emptyMarkers=(text.match(/[^\n]*(?:no vehicles|0 vehicles|no matching|empty)[^\n]*/ig)||[]).slice(0,20);const countMatch=text.match(/(\d+)\s+of\s+(\d+)\s+vehicles shown/i);const doc={schema:'pdc-staging-authenticated-ui-proof-v1',project_ref:ref,captured_at_utc:new Date().toISOString(),title:state.result.value.title,vehicle_card_count:state.result.value.vehicleCards,shown_vehicle_count:countMatch?Number(countMatch[1]):null,total_vehicle_count:countMatch?Number(countMatch[2]):null,empty_markers:emptyMarkers,screenshot:path.basename(out)};fs.writeFileSync(evidence,JSON.stringify(doc,null,2));console.log(JSON.stringify({status:'AUTHENTICATED_UI_CAPTURED',project_ref:ref,title:doc.title,vehicle_card_count:doc.vehicle_card_count,shown_vehicle_count:doc.shown_vehicle_count,total_vehicle_count:doc.total_vehicle_count,empty_markers:doc.empty_markers,screenshot:out,evidence},null,2));ws.close();
}
