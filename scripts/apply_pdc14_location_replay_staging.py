#!/usr/bin/env python3
"""Apply/read back the PDC-14 location replay successor on STAGING only."""
...
MIGRATION = ROOT / "supabase/staging_only/20260904011000_pdc14_location_replay_idempotency.sql"
PRODUCTION_REF = "vjdtsswhroyguxyfjdkt"
APPROVAL = "PDC_APPROVE_STAGING_MIGRATION_20260904011000"
EXPECTED_PREDECESSOR = ["20260904010900", "provenance_rpc_auth_hardening"]
EXPECTED_HEAD = ["20260904011000", "pdc14_location_replay_idempotency"]
...
def main() -> int:
    ...
    if args.mode == "apply":
        checks = [
            ...
            not after["old_signature_exists"],
            after["new_signature_exists"],
            after["fixed_search_path"],
            after["authenticated_execute"],
            after["anon_denied"],
            after["receipt_rls"] == [True, True],
            after["receipt_public_denied"],
        ]
        if not all(checks):
