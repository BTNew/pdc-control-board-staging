...
import psycopg2
R=Path(__file__).resolve().parents[1];M=R/'supabase/staging_only/20260828460000_724_agentic_apply_acceptance_readback.sql';SHA='66c4c2f5fda9e39d5bb41654913b40c9fe0fcb02355c90be1a1e87771bff934f';REF='cdsmnqxtyyoeoznmbidd';PROD='vjdtsswhroyguxyfjdkt';APP='PDC_APPROVE_STAGING_MIGRATION_724'
def db():
...
def main():
 ...
 try:
  ...
  if hashlib.sha256(src.encode()).hexdigest()!=SHA:raise RuntimeError('724 source hash mismatch')
  pre=b['head']=='20260828450000' and not b['production'] and b['prior'] and not b['repair'] and b['uid514']==1 and b['receipt']==1 and b['vehicle']==1 and b['ops']==5
  if a.mode!='readback' and not pre:raise RuntimeError(f'724 prestate mismatch: {b}')
  if a.mode=='preflight':r.update(ok=True,before=b,committed=False)
  elif a.mode=='apply':
   if os.environ.get(APP)!=f'apply migration 724 readback source {SHA}':raise RuntimeError('approval phrase missing')
   with c:
    with c.cursor() as x:x.execute(src.removeprefix('BEGIN;\n').removesuffix('COMMIT;\n'))
   after=state(c)
   if after['head']!='20260828460000' or not after['repair'] or after['production'] or after['uid514']!=1 or after['receipt']!=1 or after['vehicle']!=1 or after['ops']!=5:raise RuntimeError(f'724 poststate mismatch: {after}')
   r.update(ok=True,before=b,after=after,committed=True)
  else:
   if b['head']!='20260828460000' or not b['repair'] or b['production']:raise RuntimeError(f'724 readback mismatch: {b}')
