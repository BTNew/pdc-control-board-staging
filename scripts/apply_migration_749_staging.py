...
MIGRATION=ROOT/"supabase/staging_only/20260829142000_749_append_qc_retest_photo_evidence.sql"
...
def main():
    spec=importlib.util.spec_from_file_location("pdc_bootstrap_749",BOOTSTRAP); mod=importlib.util.module_from_spec(spec); assert spec and spec.loader; spec.loader.exec_module(mod)
    values=json.loads(mod.unprotect(SECRETS.read_bytes()).decode()); mod.validate(values)
    import psycopg2
    conn=psycopg2.connect(values["PDC_STAGING_DATABASE_URL"],sslmode="verify-full",sslrootcert=values["PDC_STAGING_SSLROOTCERT"],application_name="pdc_13000769_recovery_749_controller")
    try:
        ...
        if head==("20260829141000","748_repair_recovery_identity_guard"):
            cur.execute(MIGRATION.read_text(encoding="utf-8")); head=("20260829142000","749_append_qc_retest_photo_evidence")
        if head!=("20260829142000","749_append_qc_retest_photo_evidence"): raise RuntimeError(f"unexpected staging head: {head}")
