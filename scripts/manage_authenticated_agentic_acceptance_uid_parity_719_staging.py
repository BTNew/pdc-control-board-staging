...
import psycopg2
ROOT=Path(__file__).resolve().parents[1]; MIGRATION=ROOT/'supabase/staging_only/20260828410000_719_agentic_acceptance_uid_parity.sql'; SHA='b11cc424cbc27eadd169efcf8407bfc31b7d3a36408fde32349d513cd7d44996'; REF='cdsmnqxtyyoeoznmbidd'; PROD='vjdtsswhroyguxyfjdkt'; APPROVAL='PDC_APPROVE_STAGING_MIGRATION_719'
def connect():
 spec=importlib.util.spec_from_file_location('b719',Path(r'C:/Users/nwmgr/AppData/Local/hermes/staging-bootstrap/pdc_staging_bootstrap.py')); m=importlib.util.module_from_spec(spec); assert spec and spec.loader; spec.loader.exec_module(m); vals=json.loads(m.unprotect(Path(r'C:/Users/nwmgr/AppData/Local/hermes/staging-secrets/pdc-staging.dpapi').read_bytes()).decode()); m.validate(vals); url=vals['PDC_STAGING_DATABASE_URL']; ep=urlsplit(url)
...
def main():
 ...
 try:
  ...
  if hashlib.sha256(source.encode()).hexdigest()!=SHA: raise RuntimeError('719 source hash mismatch')
  pre=before['head']=='20260828400000' and not before['production'] and before['prior_history'] and not before['repair_history'] and before['uid514']==1 and before['uid514_receipt']==1 and before['uid514_vehicle']==1 and before['uid514_ops']==5 and before['writes']==0 and before['mailboxes']==1
  if a.mode!='readback' and not pre: raise RuntimeError(f'719 exact staging prestate mismatch: {before}')
  if a.mode=='preflight': result.update(ok=True,before=before,committed=False)
  elif a.mode=='apply':
   if os.environ.get(APPROVAL)!=f'apply migration 719 UID parity source {SHA}': raise RuntimeError('PDC_APPROVE_STAGING_MIGRATION_719 approval phrase missing')
   with c:
    with c.cursor() as x:x.execute(source.removeprefix('BEGIN;\n').removesuffix('COMMIT;\n'))
   after=state(c)
   if after['head']!='20260828410000' or not after['repair_history'] or after['production'] or after['uid514']!=1 or after['uid514_receipt']!=1 or after['uid514_vehicle']!=1 or after['uid514_ops']!=5: raise RuntimeError(f'719 poststate mismatch: {after}')
   result.update(ok=True,before=before,after=after,committed=True)
  else:
   if before['head']!='20260828410000' or not before['repair_history'] or before['production']: raise RuntimeError(f'719 readback mismatch: {before}')
