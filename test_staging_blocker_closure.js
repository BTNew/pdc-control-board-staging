...
assert(app.includes("[{ value: 'Yes', label: '✓ JITA Number' }, { value: 'Unknown', label: 'Blank' }]"), 'Control Board JITA filter must expose only imported-number tick or blank');
assert(!app.includes("{ value: 'No', label: '× Cross' }"), 'Control Board must not expose a manual/negative JITA cross');
assert(app.includes("'JITA Number','Contact'"), 'CSV export must expose the authoritative JITA Number');
assert(!app.includes("'JITA Parts Ordered','JITA Qty'"), 'CSV export must not expose legacy boolean JITA authority');
assert(!app.includes('jitaPartsOrdered: data.jitaPartsOrdered'), 'manual vehicle creation must not accept a JITA boolean');
