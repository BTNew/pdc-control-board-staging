...
(async()=>{
 let handlers=null,fetches=0,renders=0,subscriptions=0,timerCallback=null;
 const app={workshopEligibilityRealtime:null,workshopEligibilityReconnectTimer:null,workshopEligibilitySnapshot:{stale:true},workshopEligibilityState:'connected',workshopEligibilityError:'',workshopEligibilityRequestGeneration:0,currentView:'workflow'};
 const context={app,console,window:null,
  ...
  fetch:async()=>{fetches++;return {ok:true,json:async()=>({stages:[],candidates:[],revision:fetches})};},
 ...
 timerCallback(); assert.strictEqual(subscriptions,2,'closed channel must be replaced'); await handlers.onSubscribed(); assert.strictEqual(fetches,4,'replacement subscription must resync');
