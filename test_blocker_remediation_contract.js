...
assert(sql.includes("'ai_email_analysis_results','ai_email_attachments','ai_email_intake','ai_extracted_fields'") && sql.includes("'ai_trusted_senders','ai_undo_actions','ai_workshop_commands'") && sql.includes("'monitored_mailboxes','email_response_drafts','audit_events','import_runs','label_print_events'") && sql.includes("'salespeople','sublet_providers','navision_backend_revision','navision_import_batches'"), 'AI/source/reference workflow tables must deny direct importer authority');
assert(sql.includes('drop policy if exists pdc_email_attachments_read_importer on storage.objects'), 'importer must not read source email attachments directly');
...
for (const fn of ['move_vehicle','mark_vehicle_deleted','qc_complete_vehicle','rft_transfer_vehicle','rft_collect_vehicle','restore_vehicle','edit_vehicle_master','get_vehicle_core_snapshot','resolve_vehicle_lifecycle_identity','get_vehicle_intelligence_snapshot','append_vehicle_timeline_event','rebuild_vehicle_intelligence_summary','create_ai_review_item','list_ai_review_queue','list_salespeople','list_sublet_providers','get_restricted_pilot_vehicle_snapshot','approve_ai_review_item','reject_ai_review_item','cascade_workshop_schedule']) {
