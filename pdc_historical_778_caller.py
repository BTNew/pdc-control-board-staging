...
def build_historical_request(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for attachment in list(_required(row, "attachments")):
        ...
        kind = attachment.get("attachment_kind")
        if kind is None and isinstance(evidence, Mapping) and evidence.get("job_cards"):
            kind = "job_card"
        if kind == "job_card" and isinstance(evidence, Mapping) and (
            evidence.get("conflicts") or len(evidence.get("stocks") or []) > 1
            or len(evidence.get("vins") or []) > 1 or len(evidence.get("job_cards") or []) != 1
        ):
