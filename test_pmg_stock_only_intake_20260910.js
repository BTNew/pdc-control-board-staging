...
test('Department 138 follows explicit station choices',()=>{
  ...
  assert.deepEqual(ui.assignmentsFor(row,{a:'TINT',b:'FITTING'}),[{line_identity:'a',stage_code:'TINT'},{line_identity:'b',stage_code:'FITTING'}]);
  const group=ui.stationGroups(row,{a:'TINT'}).find(x=>x.code==='TINT');
  assert.equal(group.lines.length,1);assert.equal(group.hours,null,'zero-hour work still needs hours when moved to another workshop station');
});
