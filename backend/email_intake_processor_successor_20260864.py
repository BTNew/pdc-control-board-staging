...
STORAGE_HASH_RE = re.compile(r"^[a-f0-9]{64}$")
STORAGE_FILENAME_RE = re.compile(r"^[A-Za-z0-9._-]{1,120}$")
...
JOB_CARD_WORK_KEYS = {
    ...
    "TYRE": "tyre", "PIT_INSPECTION": "pitInspection",
}
...
def _canonical_storage_filename(filename: str) -> str:
    if not isinstance(filename, str) or not filename or filename != filename.strip():
        raise RuntimeError("attachment filename is invalid")
    safe_name = re.sub(r"[^A-Za-z0-9._-]+", "_", filename)[:120]
    if not safe_name or not STORAGE_FILENAME_RE.fullmatch(safe_name):
        raise RuntimeError("attachment filename is invalid")
    return safe_name


def validate_attachment_storage_path(storage_path: str, filename: str, expected_hash: str) -> str:
    """Validate the exact private object contract before any Storage request."""
    if not isinstance(storage_path, str) or not storage_path or storage_path != storage_path.strip():
        raise RuntimeError("attachment storage path is invalid")
    if not isinstance(expected_hash, str) or not STORAGE_HASH_RE.fullmatch(expected_hash):
    ...
    if len(parts) != 3 or any(not part for part in parts):
        raise RuntimeError("attachment storage path is invalid")
    bucket, object_hash, object_name = parts
    if bucket != STORAGE_BUCKET or object_hash != expected_hash:
        raise RuntimeError("attachment storage path hash or bucket binding is invalid")
    if not STORAGE_HASH_RE.fullmatch(object_hash) or not STORAGE_FILENAME_RE.fullmatch(object_name):
        raise RuntimeError("attachment storage path is invalid")
    if object_name != _canonical_storage_filename(filename):
        raise RuntimeError("attachment storage path filename binding is invalid")
    return f"{object_hash}/{object_name}"
...
def canonical_jobcard_request(record: dict[str, Any], proposal: ExtractionProposal) -> dict[str, Any]:
    ...
    for index, line in enumerate(source_lines, 1):
        work_key = JOB_CARD_WORK_KEYS.get(str(line.get("work_type") or ""))
        minutes = line.get("estimated_duration_minutes")
        if not work_key or isinstance(minutes, bool) or not isinstance(minutes, int) or minutes <= 0:
            raise RuntimeError("job-card lines require deterministic work type and explicit positive duration")
        hours = round(minutes / 60, 2)
        description = operation_display_description(line.get("applied_description") or line.get("original_description"), str(proposal.fields.get("jc_number") or ""))[:180]
        if hours <= 0 or not description:
            raise RuntimeError("job-card operation line is invalid")
        operation_no = str(line.get("operation_code") or f"OP{index}")
        operation_lines.append({
            "source_row_no": index, "operation_no": operation_no, "work_key": work_key,
            "description": description, "estimated_hours": hours,
            "source_description": clean_text(line.get("original_description"), 500),
            "supervised_rule": line.get("supervised_rule"),
        })
        if work_key not in required_work:
...
class SupabaseClient:
    ...
    def _download_attachment(self, storage_path: str, filename: str, expected_hash: str = "") -> str:
        object_path = validate_attachment_storage_path(storage_path, filename, expected_hash)
        request = urllib.request.Request(
            f"{self.url}/storage/v1/object/authenticated/{STORAGE_BUCKET}/{urllib.parse.quote(object_path, safe='/')}",
    ...
    def attachments(self, intake_id: str, claim_token: str) -> list[AttachmentEvidence]:
        result = self.rpc("get_pdc_monitor_intake_attachments_735", {"p_intake_id": intake_id, "p_claim_token": claim_token, "p_gateway_instance_id": self.gateway_instance_id})
