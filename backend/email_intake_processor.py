...
class SupabaseClient:
    def __init__(self, url: str, anon_key: str, access_token: str, timeout: int = 45):
        ...
        self.gateway_instance_id = f"pdc-monitor-{os.getpid()}-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')}"
    ...
    def pending_intakes(self, limit: int) -> list[dict[str, Any]]:
        result = self.rpc("claim_pdc_email_intake_batch", {"p_limit": limit, "p_gateway_instance_id": self.gateway_instance_id})
        rows = result.get("items") if result.get("ok") is True else []
        return rows if isinstance(rows, list) else []

    def _download_attachment(self, storage_path: str, filename: str) -> str:
        ...
        request = urllib.request.Request(
            f"{self.url}/storage/v1/object/{bucket}/{urllib.parse.quote(object_path, safe='/')}",
    ...
    def attachments(self, intake_id: str, claim_token: str) -> list[AttachmentEvidence]:
        ...
        for row in rows if isinstance(rows, list) else []:
            path = self._download_attachment(str(row.get("storage_path") or ""), str(row.get("file_name") or ""))
    ...
    def process(self, record: dict[str, Any], proposal: ExtractionProposal) -> dict[str, Any]:
        request = validate_jobcard_request(canonical_jobcard_request(record, proposal))
