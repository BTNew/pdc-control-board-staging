...
class HistoricalStorageQuarantine(RuntimeError):
    """Exact authenticated missing-object projection; never enters extraction."""

    def __init__(self, details: dict[str, Any]):
        self.details = {
            "code": "attachment_storage_incomplete",
            ...
            "attachment_count": details.get("attachment_count"),
            "readable_attachment_count": details.get("readable_attachment_count"),
            "incomplete_attachment_count": details.get("incomplete_attachment_count"),
        }
        super().__init__("authenticated historical attachment storage is incomplete")
...
class SupabaseClient:
    ...
    def attachments(self, intake_id: str, claim_token: str) -> list[AttachmentEvidence]:
        ...
        if (
            ...
            and result.get("contract") == "pdc_email_monitor_attachments_735"
            and result.get("scope") == "authenticated-active-839"
            and result.get("review_required") is True
            and result.get("attachment_storage_incomplete") is True
            and result.get("storage_reconciliation_required") is True
            and result.get("board_mutated") is False
            and result.get("mailbox_flags_changed") is False
            and result.get("uid514_processed") is False
            and result.get("production_writes") is False
            and isinstance(result.get("incomplete_attachment_count"), int)
            and result["incomplete_attachment_count"] > 0
        ):
            raise HistoricalStorageQuarantine(result)
...
def run_once(client: SupabaseClient, limit: int = 20) -> dict[str, Any]:
    ...
    for record in records:
        ...
        except HistoricalStorageQuarantine as exc:
            result = exc.details
            client.record_result(intake_id, claim_token, True, result, "attachment_storage_incomplete", "", False)
            summary["processed"] += 1
            summary["review"] += 1
            summary["results"].append({"intake_id": intake_id, "code": "attachment_storage_incomplete", "ok": True})
        except Exception as exc:
            summary["failed"] += 1; summary["ok"] = False
