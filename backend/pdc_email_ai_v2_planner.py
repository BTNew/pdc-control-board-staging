...
_JOB_CARD = re.compile(r"(?<![A-Z0-9])((?:JC|J)\s*[0-9]{6,12})(?![A-Z0-9])", re.I)
...
def _attachment_job_cards(attachment: Mapping[str, Any]) -> set[str]:
    """Extract only explicit, attachment-local Job Card identifiers."""
    values = (str(attachment.get("filename") or ""), str(attachment.get("extracted_text") or ""))
    found: set[str] = set()
    for value in values:
        for match in _JOB_CARD.finditer(value):
            found.add(re.sub(r"\s+", "", match.group(1)).upper())
    return found


def _attachment_source_uid(source: Mapping[str, Any], digest: str) -> str:
    return str(source.get("source_uid") or (str(source["message_id"]) + ":" + digest))
...
class V2Planner:
    ...
    def plan(self, receipt: Mapping[str, Any], attachments: Sequence[Mapping[str, Any]], contexts: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
        ...
        if receipt.get("source_uid"):
            source["source_uid"] = str(receipt["source_uid"])
        ...
        job_card_candidates: list[tuple[Mapping[str, Any], str, str, str | None]] = []
        ...
        for attachment in attachments:
            ...
            if len(attachment_job_cards) == 1:
                job_card_candidates.append((row, digest, next(iter(attachment_job_cards)), vin or (str(row.get("vin")).upper() if row.get("vin") else None)))
            ...
            for index, raw_line in enumerate(lines, 1):
                ...
                # document does not state hours. Keep them planned so the
                # typed PD importer can preserve the row without inventing 0.
                is_pd_row = bool(re.fullmatch(r"PD[0-9]{3}-[A-F0-9]{8}", str(line.get("operation_no") or "").upper()))
                if estimated_hours is None and decision == "planned" and not is_pd_row:
                    decision = "review"
                    reason = "estimated_hours_unknown"
                operation_no = str(line.get("operation_no") or f"OP{index}").upper()
                work_key = classification.work_key or "PARTS"
                # Deterministically extracted PD accessory rows retain the
                # source parser's controlled work key. Their evidence is
                # classified even when the generic product-word taxonomy would
                # otherwise flag a review conflict (for example a fire
                # extinguisher mapped to the approved Fitting key).
        ...
        if len(distinct_job_cards) == 1 and job_card_candidates:
            row, digest, job_card_number, vin = job_card_candidates[0]
            add(row, "job_card_set", {
                "attachment_digest": digest,
                "job_card_number": job_card_number,
                "source_uid": _attachment_source_uid(source, digest),
                "stock_number": str(row["stock_number"]).upper(),
                "vin": vin,
            }, self._refs(source, digest), "planned", "explicit_attachment_job_card")
        elif len(distinct_job_cards) > 1:
            row, digest, _, _ = job_card_candidates[0]
            add(row, "note_append", {
                "text": "conflicting attachment-scoped Job Card evidence requires review",
                "event_at": "2026-09-01T00:00:00+00:00",
            }, self._refs(source, digest), "review", "conflicting_attachment_job_card")
