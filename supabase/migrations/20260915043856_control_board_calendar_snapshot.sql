-- Add the existing validated calendar values to the viewer-authorized board snapshot.
-- Values are copied without defaults or normalization; the planner configuration is authoritative.
DO $guard$
BEGIN
  IF (SELECT count(*) FROM public.pdc_staging_environment_sentinel
    ...
    RAISE EXCEPTION 'Control Board calendar snapshot migration is staging-only';
  END IF;
  IF md5(pg_get_functiondef('public.get_workshop_eligibility_snapshot()'::regprocedure))
      IS DISTINCT FROM '4d53091e7a502105c4945ab6a70a1a4e' THEN
    RAISE EXCEPTION 'Unexpected get_workshop_eligibility_snapshot predecessor; review before applying';
  END IF;
  IF (SELECT proacl::text FROM pg_proc WHERE oid='public.get_workshop_eligibility_snapshot()'::regprocedure)
      IS DISTINCT FROM '{postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres}' THEN
    RAISE EXCEPTION 'Unexpected snapshot privileges; review before applying';
  END IF;
END $guard$;
...
BEGIN
  IF (SELECT proacl::text FROM pg_proc WHERE oid='public.get_workshop_eligibility_snapshot()'::regprocedure)
      IS DISTINCT FROM '{postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres}' THEN
    RAISE EXCEPTION 'Snapshot privileges changed';
  END IF;
  IF has_function_privilege('anon','public.get_workshop_eligibility_snapshot()','EXECUTE') THEN
    RAISE EXCEPTION 'Anonymous snapshot access must remain denied';
  END IF;
END $check$;
