-- STAGING ONLY: coordinate approval with the automatic clock and concurrent planner edits.
-- Keep the scheduling calculation and all existing booking validation unchanged.
DO $preflight$ BEGIN
 ...
 IF md5(pg_get_functiondef('public.approve_pdc_tune_operation_change_with_schedule(uuid,text,text,numeric,uuid)'::regprocedure))<>'0d449ba1aee562708a952ef52994549e'
 OR md5(pg_get_functiondef('public.pdc_tune_reconcile_booking_plan_20260913(uuid,text[],uuid)'::regprocedure))<>'3886fd4c09c91dfbfbe8ae3fed35b578'
 THEN RAISE EXCEPTION 'Operation approval changed since this repair was reviewed'; END IF;
END $preflight$;
...
BEGIN
 ...
 -- The clock takes bay/booking locks before station revision rows. Share its
 -- gate before taking any operational row lock, so it can never wait on a
 -- revision row while this approval waits on one of its bookings.
 PERFORM pg_advisory_xact_lock(hashtextextended('workshop-clock-cascade-20260911',0));
 PERFORM pg_advisory_xact_lock(hashtextextended('pdc:workshop:top-level-mutation',0));
 -- Acquire bookings before vehicle and required-work writes, matching planner
 -- moves. NOWAIT prevents a cycle with imports that already own a vehicle.
 -- Contention rolls back and is safe to retry with the same approval key.
 PERFORM 1 FROM public.workshop_bookings WHERE deleted_at IS NULL
  AND status IN('queued','planned','started','stoppage') ORDER BY id FOR UPDATE NOWAIT;
 PERFORM 1 FROM public.workshop_admin_blocks WHERE deleted_at IS NULL ORDER BY id FOR SHARE NOWAIT;
 PERFORM 1 FROM public.workshop_booking_assignments WHERE released_at IS NULL ORDER BY id FOR UPDATE NOWAIT;
 SELECT vehicle_id INTO source_id FROM public.pdc_tune_operation_change_reviews WHERE change_id=p_change_id;
 SELECT * INTO v FROM public.vehicles WHERE id=source_id FOR UPDATE NOWAIT;
 SELECT * INTO q FROM public.pdc_tune_operation_change_reviews WHERE change_id=p_change_id FOR UPDATE NOWAIT;
...
EXCEPTION WHEN OTHERS THEN
 ...
 PERFORM set_config('pdc.defer_workshop_adjustment_reconcile',coalesce(original_defer,''),true);
 PERFORM set_config('pdc.defer_workshop_required_work_reconcile',coalesce(original_required_defer,''),true);
 IF SQLSTATE IN('40001','40P01','55P03') THEN
  RETURN jsonb_build_object('ok',false,'code','operation_schedule_busy',
   'message','The workshop is being updated by another action. Please try approving again. No changes were saved.',
   'retry',true);
 END IF;
 RETURN jsonb_build_object('ok',false,'code','operation_schedule_conflict',
  'message',CASE WHEN failure_detail LIKE 'operation_schedule_%' THEN failure_message
   ...
   ||' No changes were saved.','retry',false);
END $function$
;
