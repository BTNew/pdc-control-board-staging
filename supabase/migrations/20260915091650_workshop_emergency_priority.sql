-- Staging emergency scheduling: preview and atomic apply, with no guard bypass.
...
END $guard$;
CREATE SCHEMA IF NOT EXISTS pdc_workshop_priority_private;
REVOKE ALL ON SCHEMA pdc_workshop_priority_private FROM PUBLIC,anon;
GRANT USAGE ON SCHEMA pdc_workshop_priority_private TO authenticated;
CREATE TABLE pdc_workshop_priority_private.receipts(
 actor_id uuid NOT NULL, idempotency_key uuid NOT NULL, request jsonb NOT NULL,
 response jsonb NOT NULL, created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
 PRIMARY KEY(actor_id,idempotency_key)
);
ALTER TABLE pdc_workshop_priority_private.receipts ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON pdc_workshop_priority_private.receipts FROM PUBLIC,anon,authenticated;

CREATE FUNCTION pdc_workshop_priority_private.state_hash(p_stock text,p_floor timestamptz)
...
CREATE FUNCTION pdc_workshop_priority_private.slot(p_booking uuid,p_bay uuid,p_minutes integer,p_floor timestamptz,p_mode text)
...
BEGIN
 ...
 LOOP
  ...
  IF p_mode='parking' THEN
   SELECT greatest(blocked,max(greatest(b.scheduled_end_at,public.workshop_booking_effective_end_at(b.id))
    ...
    WHERE b.id<>p_booking AND b.deleted_at IS NULL AND b.status IN('queued','planned','started','stoppage')
     ...
     AND greatest(b.scheduled_end_at,public.workshop_booking_effective_end_at(b.id))
...
CREATE FUNCTION pdc_workshop_priority_private.reschedule(p_vehicle uuid,p_floor timestamptz)
  ...
  SELECT x.booking_id INTO picked FROM pg_temp.emergency_plan x
   WHERE changed AND apply_order IS NULL AND NOT EXISTS(
    SELECT 1 FROM public.workshop_bookings q WHERE q.id<>x.booking_id AND q.deleted_at IS NULL AND q.status IN('queued','planned','started','stoppage')
     ...
     AND q.scheduled_start_at<x.final_end AND greatest(q.scheduled_end_at,public.workshop_booking_effective_end_at(q.id))>x.final_start)
...
CREATE FUNCTION pdc_workshop_priority_private.request(p_stock_number text,p_apply boolean DEFAULT false,p_plan_hash text DEFAULT NULL,p_idempotency_key uuid DEFAULT NULL)
...
END $fn$;
REVOKE ALL ON ALL FUNCTIONS IN SCHEMA pdc_workshop_priority_private FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION pdc_workshop_priority_private.request(text,boolean,text,uuid) TO authenticated;
CREATE FUNCTION public.prioritise_workshop_vehicle(p_stock_number text,p_apply boolean DEFAULT false,p_plan_hash text DEFAULT NULL,p_idempotency_key uuid DEFAULT NULL)
RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path=pg_catalog AS $fn$
 SELECT pdc_workshop_priority_private.request(p_stock_number,p_apply,p_plan_hash,p_idempotency_key)
$fn$;
REVOKE ALL ON FUNCTION public.prioritise_workshop_vehicle(text,boolean,text,uuid) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.prioritise_workshop_vehicle(text,boolean,text,uuid) TO authenticated;
