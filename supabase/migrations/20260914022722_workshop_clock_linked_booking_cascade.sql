...
DO $preflight$ BEGIN
 ...
 IF md5(pg_get_functiondef('public.workshop_clock_tick(boolean,timestamptz)'::regprocedure))<>'ccad25a00bcb80f71bf6a085efc2e3d3' THEN RAISE EXCEPTION 'Clock changed since review'; END IF;
END $preflight$;
  ...
  BEGIN
   ...
   IF EXISTS(SELECT 1 FROM pg_temp.clock_linked_plan a JOIN pg_temp.clock_linked_plan b
    ...
    WHERE a.component_id=component.component_id AND (a.changed OR b.changed)
     ...
     AND a.final_end+CASE WHEN a.vehicle_id=b.vehicle_id THEN interval '5 hours' ELSE interval '0 minutes' END>b.final_start) THEN
