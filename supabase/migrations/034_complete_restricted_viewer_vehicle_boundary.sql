-- Complete restricted-pilot viewer vehicle boundary hardening.
-- Viewers must use only the approved six-field pilot vehicle contract; broad
-- workshop snapshots and vehicle aliases remain available to operators and
-- higher roles only.

begin;

drop policy if exists vehicle_aliases_select_approved on public.vehicle_aliases;
drop policy if exists vehicle_aliases_select_operator on public.vehicle_aliases;
create policy vehicle_aliases_select_operator
...
-- This broad helper is only used inside owner-executed workshop functions.
revoke all on function public.workshop_booking_snapshot(uuid) from public, anon, authenticated;

comment on function public.get_workshop_snapshot(date, date) is
  'Broad workshop snapshot restricted to operator/importer/administrator roles; viewers use the restricted six-field pilot vehicle contract.';
