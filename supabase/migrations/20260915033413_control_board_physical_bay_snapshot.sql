-- Add the physical bay board to the existing viewer-authorized snapshot.
-- This is a read-only projection; no booking, vehicle, bay, access grant or rule changes.
DO $guard$
BEGIN
  IF (SELECT count(*) FROM public.pdc_staging_environment_sentinel
    ...
    RAISE EXCEPTION 'Physical bay snapshot migration is staging-only';
  END IF;
  IF md5(pg_get_functiondef('public.get_workshop_eligibility_snapshot()'::regprocedure))
      IS DISTINCT FROM 'b44fb16452dcfcc56911c87666ca22e4' THEN
