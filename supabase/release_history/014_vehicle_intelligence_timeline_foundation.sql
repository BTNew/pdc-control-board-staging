...
-- Some clean Preview baselines intentionally omit this optional projection.
-- PostgreSQL still resolves DROP POLICY's relation even with IF EXISTS, so
-- guard both policy statements by the relation catalog entry.
DO $vehicle_intelligence_summary_policy$
BEGIN
  IF to_regclass('public.vehicle_intelligence_summaries') IS NOT NULL THEN
    EXECUTE 'drop trigger if exists vehicle_intelligence_summaries_set_updated_at on public.vehicle_intelligence_summaries';
    EXECUTE 'create trigger vehicle_intelligence_summaries_set_updated_at before update on public.vehicle_intelligence_summaries for each row execute function public.set_updated_at()';
    EXECUTE 'alter table public.vehicle_intelligence_summaries enable row level security';
    EXECUTE 'drop policy if exists vehicle_intelligence_summaries_select_viewer on public.vehicle_intelligence_summaries';
    EXECUTE 'create policy vehicle_intelligence_summaries_select_viewer on public.vehicle_intelligence_summaries for select to authenticated using (public.is_pdc_role(''viewer''))';
    EXECUTE 'revoke insert, update, delete on table public.vehicle_intelligence_summaries from anon, authenticated';
    IF NOT EXISTS (
      SELECT 1 FROM pg_catalog.pg_publication_tables
      WHERE pubname='supabase_realtime'
        AND schemaname='public'
        AND tablename='vehicle_intelligence_summaries'
    ) THEN
      EXECUTE 'alter publication supabase_realtime add table public.vehicle_intelligence_summaries';
    END IF;
  END IF;
END
$vehicle_intelligence_summary_policy$;
