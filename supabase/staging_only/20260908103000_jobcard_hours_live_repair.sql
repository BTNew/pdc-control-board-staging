-- STAGING ONLY: repair atomic Job Card hours for Pilbara Service operations.
BEGIN;
...
'STAGING authenticated atomic Job Card hour batch for canonical email and Pilbara Service operations. Immutable raw/source hours remain untouched; numeric or explicit unknown effective overlays are audited. Parts, bookings and completion are never mutated.';

DO $verify$
DECLARE d text;
BEGIN
  d:=pg_get_functiondef('public.save_vehicle_workshop_line_hours_batch_768(uuid,text,text,bigint,jsonb,uuid)'::regprocedure);
  IF position('pdc_pilbara_service_operations' in d)=0
     OR position('manual_operator_unknown' in d)=0
     OR position('pdc_auditor_vehicle_dealer' in d)=0
     OR position('source_estimated_hours' in d)=0
     OR position('effective_estimated_hours' in d)=0
     OR position('UPDATE public.'||'pdc_pilbara_service_operations' in d)>0
     OR position('UPDATE public.'||'workshop_bookings' in d)>0
     OR position('UPDATE public.'||'vehicle_parts_updates' in d)>0
     OR has_function_privilege('authenticated','public.get_vehicle_workshop_detail(uuid)','execute')
     OR NOT has_function_privilege('authenticated','public.get_vehicle_workshop_detail_scoped(uuid,text)','execute')
     OR has_function_privilege('anon','public.get_vehicle_workshop_detail_scoped(uuid,text)','execute')
     OR has_function_privilege('service_role','public.get_vehicle_workshop_detail_scoped(uuid,text)','execute')
     ...
     OR has_function_privilege('anon','public.save_vehicle_workshop_line_hours_batch_768(uuid,text,text,bigint,jsonb,uuid)','execute')
     OR has_function_privilege('service_role','public.save_vehicle_workshop_line_hours_batch_768(uuid,text,text,bigint,jsonb,uuid)','execute')
  THEN RAISE EXCEPTION 'PDC_JOB_CARD_HOURS_LIVE_REPAIR_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $verify$;

INSERT INTO supabase_migrations.schema_migrations(version,name,statements)
VALUES('20260908103000','jobcard_hours_live_repair',ARRAY[
  'Accept strict whole-hour and two-decimal browser forms while preserving blank as an explicit unknown effective overlay',
  'Extend the atomic hours RPC to immutable Pilbara Service operation identities without modifying raw/source evidence',
  'Bind the mutation to the signed-in dealer scope and retain one idempotent all-or-none receipt',
  'Seal the unscoped Workshop detail SECURITY DEFINER function and retain only the authenticated dealer-scoped wrapper',
  'Keep Parts, bookings and completion outside the authoritative hours mutation'
]);
