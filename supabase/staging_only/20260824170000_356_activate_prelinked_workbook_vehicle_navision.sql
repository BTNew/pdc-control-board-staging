-- Staging-only exact Stock bridge successor: activate one prelinked active workbook vehicle for one current Navision backend record.
-- This expands no generic DML authority and preserves Manager + independent Administrator approval.
BEGIN;
SET LOCAL lock_timeout='10s';
SET LOCAL statement_timeout='180s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-356-prelinked-workbook-navision-activation',0));
DO $guard$
BEGIN
 IF NOT public.pdc_monitor_staging_guard()
   OR (SELECT count(*) FROM public.pdc_staging_environment_sentinel WHERE singleton AND project_ref='cdsmnqxtyyoeoznmbidd')<>1
   OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL THEN RAISE EXCEPTION 'PDC_356_WRONG_ENVIRONMENT'; END IF;
 IF NOT EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260824160000' AND name='355_attach_existing_workbook_vehicle_to_navision')
   OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version>'20260824160000' AND version~'^[0-9]{14}$') THEN
  RAISE EXCEPTION 'PDC_356_MIGRATION_HEAD_MISMATCH';
 END IF;
 IF NOT EXISTS(SELECT 1 FROM public.pdc_staging_full_reset_receipts_354 WHERE action_key='craig-full-vehicle-history-reset-20260824') THEN
  RAISE EXCEPTION 'PDC_356_RESET_PREDECESSOR_MISSING';
 END IF;
 IF NOT EXISTS(SELECT 1 FROM public.pdc_staging_verified_backup_manifests WHERE backup_manifest_sha256='0cba8a1feb4a01ef55de4de93b29fd6e950949ccd34bf6ef5ecf82bf3031b2c0') THEN
  RAISE EXCEPTION 'PDC_356_VERIFIED_BACKUP_MISSING';
 END IF;
END $guard$;
...
BEGIN
 ...
 IF FOUND AND cardinality(owner_ids)=1 AND owner_ids[1]=v.id
   AND NOT EXISTS(SELECT 1 FROM unnest(vin_ids) z WHERE z<>v.id)
   AND v.deleted_at IS NULL AND v.lifecycle_state='active' AND v.visible_on_board AND v.board_purged_at IS NULL
   AND v.rft_collected_at IS NULL AND upper(btrim(coalesce(v.current_location,'')))<>'COMPLETED'
   AND v.stock_number_normalized=stock AND v.source_system_normalized='pdc_pmb_workbook'
   AND (target_vin IS NULL OR v.vin_normalized IS NULL OR v.vin_normalized=target_vin)
   AND NOT EXISTS(SELECT 1 FROM public.navision_board_activations x WHERE x.backend_record_id=r.id OR public.normalize_vehicle_stock_number(x.activated_stock_number)=stock) THEN
  RETURN jsonb_build_object('eligible',true,'action','activate_exact_prelinked_workbook_vehicle','backend_record_id',r.id,
   'backend_record_version',r.version,'target_vehicle_id',v.id,'target_vehicle_version',v.version,'stock_number',stock);
 END IF;
...
COMMENT ON FUNCTION public.pdc_pmb_workbook_canonical_candidate(uuid) IS
 'Staging-only fail-closed candidate. Migration356 permits one exact prelinked active pdc_pmb_workbook Stock owner to activate one current non-completed Navision backend record through the existing Manager + independent Administrator canonical activation contract.';
...
BEGIN
 ...
 IF position('attach_exact_existing_workbook_vehicle' IN d)=0
   OR position('activate_exact_prelinked_workbook_vehicle' IN d)=0
  ...
  RAISE EXCEPTION 'PDC_356_CANDIDATE_INSTALL_POSTCONDITION_FAILED';
 END IF;
END $post$;

INSERT INTO supabase_migrations.schema_migrations(version,name,statements)
VALUES('20260824170000','356_activate_prelinked_workbook_vehicle_navision',array[
 'Require exact staging sentinel, verified pre-reset backup, reset receipt and migration 355 head',
 'Permit one exact prelinked active pdc_pmb_workbook Stock owner to activate one current non-completed Navision backend record',
