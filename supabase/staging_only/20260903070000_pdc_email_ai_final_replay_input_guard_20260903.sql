-- STAGING-only correction: fail-closed input guard for final exact replay.
BEGIN;
SET LOCAL lock_timeout='10s';
SET LOCAL statement_timeout='180s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-20260903070000-email-ai-final-replay-input-guard',0));
...
BEGIN
  ...
  IF current_user<>'postgres' OR session_user<>'postgres'
     ...
     OR v_head<>'20260903060000'
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='20260903060000' AND name='pdc_email_ai_final_replay_binding_20260903')<>1
     OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260903070000')
  THEN RAISE EXCEPTION 'PDC_20260903070000_STAGING_PRECONDITION_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
  IF position('attachment_hashes.hashes' IN v_def)=0
     ...
     OR position('coalesce(p_plan->>''source_receipt_id'','''') !~ ''^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$''' IN v_def)=0
     OR position('coalesce(p_plan->>''source_digest'','''') !~ ''^[a-f0-9]{64}$''' IN v_def)=0
     OR position('coalesce(p_plan->>''evidence_digest'','''') !~ ''^[a-f0-9]{64}$''' IN v_def)=0
  ...
  THEN RAISE EXCEPTION 'PDC_20260903070000_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;

INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES(
 '20260903070000','pdc_email_ai_final_replay_input_guard_20260903',ARRAY[
  ...
  'Reject malformed source receipt and digest fields before any UUID cast in the pre-validation replay path',
