-- STAGING ONLY 798: repair the exact 752 rollback history insert.
-- This append-only successor preserves 672 containment and repairs only the
-- already-reviewed exact mailbox rollback control; no runtime is enabled.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='90s';
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-798-repair-752-rollback-history',0));
...
BEGIN
  IF current_user<>'postgres' OR session_user<>'postgres'
     ...
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='20260830210000' AND name='797_complete_domain_readback_successor')<>1
     OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' AND version>'20260830210000')
     OR to_regclass('public.pdc_email_monitor_reactivation_752') IS NULL
     OR to_regprocedure('public.admin_rollback_pdc_email_monitor_reactivation_752(text)') IS NULL
     OR (SELECT count(*) FROM public.pdc_email_monitor_reactivation_752 WHERE event_kind='forward_reactivation' AND controls_enabled AND writer_enabled AND NOT task_enabled AND NOT mailbox_contacted AND NOT mailbox_flags_changed AND NOT uid514_processed AND NOT production_writes)<>1
     OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>1
     OR (SELECT count(*) FROM public.monitored_mailboxes WHERE id='12fe383d-5c1e-5801-96e4-f67cf3e3bb57' AND active AND test_mode AND mailbox_key='pdc_pmb_email' AND lower(mailbox_address)='pmbcontroller@gmail.com' AND lower(provider)='gmail')<>1
     ...
     OR (SELECT count(*) FROM public.pdc_email_monitor_reactivation_752 WHERE event_kind='rollback')<>0
  THEN RAISE EXCEPTION 'PDC_798_EXACT_797_752_REPAIR_PRESTATE_MISMATCH' USING errcode='55000'; END IF;
  ...
  IF v_hash<>'e8933be5268a1b9b5014aa236960831ed74907bee5e132b604d3e9930faacc4a'
  THEN RAISE EXCEPTION 'PDC_798_752_PREDECESSOR_FUNCTION_HASH_MISMATCH' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
  ...
  FROM public.monitored_mailboxes
  WHERE id='12fe383d-5c1e-5801-96e4-f67cf3e3bb57'
    AND active AND test_mode AND mailbox_key='pdc_pmb_email'
  ...
  SELECT * INTO v_writer
  FROM public.pdc_monitor_stage_activation_writers
  WHERE user_id='df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b' AND active AND revoked_at IS NULL
  FOR UPDATE;
  ...
  FOR SHARE;
  IF NOT FOUND THEN RAISE EXCEPTION 'PDC_798_752_FORWARD_HISTORY_MISSING' USING errcode='55000'; END IF;

  v_before_mailbox:=to_jsonb(v_mailbox);
  v_before_writer:=to_jsonb(v_writer);
  UPDATE public.monitored_mailboxes
  SET active=false,updated_at=clock_timestamp()
  WHERE id=v_mailbox.id AND active
  ...
  SET enabled=false,changed_at=clock_timestamp() WHERE singleton;
  ...
  WHERE user_id=v_writer.user_id AND active AND revoked_at IS NULL
  ...
  INSERT INTO public.pdc_email_monitor_reactivation_752(
    ...
    before_mailbox,after_mailbox,before_writer,after_writer,
    controls_enabled,writer_enabled,task_enabled,mailbox_contacted,
    mailbox_flags_changed,uid514_processed,production_writes,
    performed_by,performed_by_email,rollback_contract)
  VALUES(
    v_event_key,'rollback',v_forward.predecessor_head,v_forward.successor_head,
    v_forward.actor_id,v_forward.actor_email,v_forward.gateway_instance_id,
    v_forward.release_name,v_forward.mailbox_id,v_forward.mailbox_address,
  ...
  VALUES('update','monitored_mailboxes',v_mailbox.id,v_admin_id,v_admin_email,
    v_before_mailbox,v_after_mailbox,jsonb_build_object(
      'event_type','pdc_email_monitor_reactivation_752_rollback',
      'exact_mailbox_only',true,'writer_disabled',true,'task_enabled',false,
      'mailbox_contacted',false,'mailbox_flags_changed',false,
      'uid514_processed',false,'production_untouched',true));

  RETURN jsonb_build_object('ok',true,'code','pdc_email_monitor_reactivation_rolled_back_752',
    'idempotent',false,'mailbox_active',false,'controls_enabled',false,
    'writer_enabled',false,'task_enabled',false,'mailbox_contacted',false,
    'mailbox_flags_changed',false,'uid514_processed',false,'production_writes',false,
...
BEGIN
 ...
 IF v_hash='e8933be5268a1b9b5014aa236960831ed74907bee5e132b604d3e9930faacc4a'
    OR (SELECT count(*) FROM public.pdc_email_monitor_reactivation_752 WHERE event_kind='rollback')<>0
 THEN RAISE EXCEPTION 'PDC_798_REPAIR_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;

INSERT INTO supabase_migrations.schema_migrations(version,name,statements)
VALUES('20260830211000','798_repair_752_rollback_history_insert',ARRAY[
  'Bind exact applied 797 head and the exact 752 forward mailbox/writer evidence',
  'Repair only admin_rollback_pdc_email_monitor_reactivation_752 history insertion by using the exact forward receipt row',
