...
BEGIN
 FOR r IN SELECT * FROM (VALUES
   ...
   IF v_owner IS DISTINCT FROM 'postgres' OR v_secdef IS DISTINCT FROM true OR v_config IS DISTINCT FROM r.config OR v_body IS DISTINCT FROM r.body_hash OR v_acl IS DISTINCT FROM r.acl OR position(lower(r.callee) in lower(pg_get_functiondef(r.sig::regprocedure)))=0 THEN RAISE EXCEPTION 'PDC_796_DEPENDENCY_CONTRACT_DRIFT:%',r.sig USING errcode='55000'; END IF;
 END LOOP;
 ...
 IF v_trigger_hash IS DISTINCT FROM 'febe0f5ea5b52920039833b853c56a40184c7b771111a83e8d59b12f34206851' THEN RAISE EXCEPTION 'PDC_796_TRIGGER_CONTRACT_DRIFT' USING errcode='55000'; END IF;
 SELECT encode(extensions.digest(convert_to(coalesce(string_agg(x.event_object_table||'|'||x.trigger_name||'|'||x.event_manipulation||'|'||x.action_timing||'|'||x.action_statement||'|'||t.tgenabled::text||'|'||t.tgtype::text||'|'||t.tgconstraint::text||'|'||t.tgdeferrable::text||'|'||t.tginitdeferred::text||'|'||coalesce(encode(extensions.digest(convert_to(pg_get_functiondef(t.tgfoid),'UTF8'),'sha256'),'hex'),'')||'|'||coalesce(pg_get_userbyid(p.proowner),'')||'|'||p.prosecdef::text||'|'||coalesce(array_to_string(p.proconfig,','),'')||'|'||coalesce((SELECT string_agg(coalesce(pg_get_userbyid(a.grantee),'PUBLIC')||':'||a.privilege_type||':'||a.is_grantable,'|' ORDER BY coalesce(pg_get_userbyid(a.grantee),'PUBLIC'),a.privilege_type,a.is_grantable) FROM aclexplode(coalesce(p.proacl,acldefault('f',p.proowner))) a),'') ,'|' ORDER BY x.event_object_table,x.trigger_name,x.event_manipulation),''),'UTF8'),'sha256'),'hex') INTO v_executor_hash FROM information_schema.triggers x JOIN pg_trigger t ON t.tgname=x.trigger_name AND t.tgrelid=(x.event_object_schema||'.'||x.event_object_table)::regclass JOIN pg_proc p ON p.oid=t.tgfoid WHERE x.event_object_schema='public' AND x.event_object_table IN ('ai_email_intake','ai_email_attachments','vehicles','vehicle_work_items','pdc_authenticated_email_operation_lines','vehicle_workshop_line_adjustments','vehicle_parts_updates','workshop_bookings','workshop_booking_assignments','pdc_sublet_bookings','pdc_sublet_booking_instances','pdc_qc_operation_completions_379','pdc_pmb_stoppage_receipts_422','pdc_email_monitor_current_head_compatibility_controls_766','pdc_email_monitor_pilot','pdc_email_monitor_status','pdc_qc_salesperson_update_outbox_399','pdc_rft_transport_email_outbox_734','pdc_rft_transport_salesperson_outbox_412','pdc_sublet_email_update_receipts','pdc_historical_provider_observations_778','pdc_historical_reconciliation_778_receipts');
 IF v_executor_hash IS DISTINCT FROM 'ef5830d8940d1f0699ed565d2f463cf8f1b32e78207de36a8c0529883c016aa3' THEN RAISE EXCEPTION 'PDC_796_TRIGGER_EXECUTOR_CONTRACT_DRIFT' USING errcode='55000'; END IF;
END $dependency$;
