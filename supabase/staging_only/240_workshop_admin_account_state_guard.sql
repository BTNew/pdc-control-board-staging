-- Staging-only migration 240: align Administrator guard with live account-state schema.
begin;
...
begin
  ...
  where auth_user_id=auth.uid() and active and approved_at is not null
    and disabled_at is null and account_status::text='approved';
...
values('240','workshop_admin_account_state_guard',array['use disabled_at and approved account_status in website Administrator guard'])
