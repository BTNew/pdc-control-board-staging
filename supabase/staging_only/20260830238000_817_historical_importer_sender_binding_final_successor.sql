-- STAGING ONLY 817: final exact historical importer sender fallback successor.
-- 815/816 were not applied because their guard postconditions were brittle.
-- Replace only the remaining stale historical fallback comparison from the
-- unchanged 814 importer. The locked intake sender is already bound before
-- both historical fallback checks; normal active-mailbox behavior is unchanged.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-817-historical-importer-sender',0));
...
BEGIN
 ...
 IF owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'824118bfa90b509cf23eea078cb8f4a78bfe086df0535354e31471564b71040a' OR position('h.sender_email=v_sender' in v)>0 THEN RAISE EXCEPTION 'PDC_817_HISTORICAL_IMPORTER_SENDER_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830238000','817_historical_importer_sender_binding_final_successor',ARRAY['replace only the stale historical sender fallback comparison from the 814 importer','preserve normal importer, exact evidence, identity, atomic rollback, idempotency, RLS/grants, ten conflicts and zero drift','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
