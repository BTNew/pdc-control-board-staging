-- STAGING ONLY 819: final exact historical authorization fallback sender successor.
-- The historical importer authorization fallback is evaluated before the later
-- v_sender assignment. Use the locked intake sender for that one remaining
-- argument while preserving the prior 817 body and all fail-closed controls.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-819-historical-importer-auth-sender',0));
...
BEGIN
 ...
 IF owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'ee743279ed1b3a9d4c46e2a12267ad054dacf59eb9b6eb1b9de216e25922947b' THEN RAISE EXCEPTION 'PDC_819_HISTORICAL_IMPORTER_AUTH_SENDER_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830240000','819_historical_importer_auth_sender_final_successor',ARRAY['replace only the pre-assignment historical authorization sender argument with locked intake sender','preserve exact evidence identity actor gateway atomic rollback idempotency RLS grants and ten conflicts','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
