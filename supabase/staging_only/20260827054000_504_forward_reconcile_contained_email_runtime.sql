...
BEGIN
  IF current_user<>'postgres'
     ...
     OR to_regprocedure('public.verify_pdc_monitor_contained_binding_504(text,text,text,text,text,text)') IS NOT NULL
  ...
  SELECT count(*), max(version) INTO v_ledger_count, v_head
  FROM supabase_migrations.schema_migrations
  WHERE version~'^[0-9]{14}$';
  IF v_ledger_count=0 OR v_head<>'20260827053000'
  ...
  IF v_version<>'20260827053000' OR v_name<>'503_existing_sales_contained_monitor_commissioning'
  ...
  WHERE p.oid='public.verify_pdc_monitor_runtime_binding_503(text,text,text,text,text,text,text)'::regprocedure
    AND n.nspname='public' AND p.prokind='f' AND p.provolatile='s' AND p.prosecdef
    AND pg_get_userbyid(p.proowner)='postgres';
...
CREATE TABLE public.pdc_monitor_contained_binding_reconciliations_504(
  ...
  predecessor_ledger_version text NOT NULL CHECK(predecessor_ledger_version='20260827053000'),
...
BEGIN
  ...
  IF (SELECT count(*) FROM supabase_migrations.schema_migrations
      WHERE version~'^[0-9]{14}$' AND version>'20260827054000')<>0
...
END
$verify$;
REVOKE ALL ON FUNCTION public.verify_pdc_monitor_contained_binding_504(text,text,text,text,text,text) FROM public,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.verify_pdc_monitor_contained_binding_504(text,text,text,text,text,text) TO authenticated;
...
BEGIN
  IF (SELECT relrowsecurity AND relforcerowsecurity FROM pg_class
     ...
     OR NOT has_function_privilege('authenticated','public.verify_pdc_monitor_contained_binding_504(text,text,text,text,text,text)','EXECUTE')
...
VALUES('20260827054000','504_forward_reconcile_contained_email_runtime',ARRAY[
  'Require exact staging project and timestamped migration head 20260827053000 with installed contained migration-503 predecessor RPCs',
