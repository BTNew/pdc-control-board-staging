-- STAGING ONLY 807: final pre796 runtime containment successor after 806.
-- 806 canonicalized the frozen authentication tuple, but the nested pre796
-- function still invoked legacy 766 runtime compatibility requiring one active
-- mailbox. Replace only that nested runtime call with the authenticated 802/672
-- zero-mailbox adapter. Preserve all proposal/evidence conflicts, append-only
-- notice/replay behavior, identity, sibling, RLS, grants, readback, rollback,
-- normal 766 runtime and Production/outbound/task boundaries.
-- Do not resolve the ten material proposal tuple conflicts here.
BEGIN;
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard()
    ...
    OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' AND version>'20260830223000')
...
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830224000','807_pre796_766_final_contained_runtime_successor',ARRAY[
 ...
 'preserve ten material proposal tuple conflicts as review-only evidence outcomes and preserve genuine append-only five-row notice/replay behavior',
 'preserve identity, malformed/wrong-actor denial, sibling isolation, RLS, grants, readback, rollback, normal 766, task, pilot, outbound and Production controls'
]);
