-- STAGING ONLY 789: proposal-binding compatibility successor after 788.
-- Existing pending proposals are never updated, deleted, reclassified, or
-- equated when their stored observations differ. The historical path records
-- an immutable proposal binding beside the 788 receipt.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-789-historical-proposal-binding',0));
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
DO $guard$
BEGIN
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830185000,788_canonical_historical_digest_contract)' OR to_regprocedure('public.submit_pdc_historical_reconciliation_782_base(jsonb)') IS NULL OR to_regprocedure('public.submit_pdc_historical_reconciliation_789_proposal_binding_base(jsonb)') IS NOT NULL OR to_regclass('public.pdc_historical_proposal_bindings_789') IS NOT NULL THEN RAISE EXCEPTION 'PDC_789_CURRENT_HEAD_GUARD_FAILED' USING errcode='55000'; END IF;
END $guard$;
CREATE TABLE public.pdc_historical_proposal_bindings_789(
 binding_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
 receipt_id uuid NOT NULL REFERENCES public.pdc_historical_reconciliation_778_receipts(receipt_id) ON DELETE RESTRICT,
 proposal_id uuid NOT NULL REFERENCES public.pdc_ai_intake_proposals(proposal_id) ON DELETE RESTRICT,
 contract_version text NOT NULL CHECK(contract_version='789.1'),
 historical_contract_version text NOT NULL CHECK(historical_contract_version='788.1'),
 ...
 proposal_observations jsonb NOT NULL,
 requested_observations jsonb NOT NULL,
 observation_match boolean NOT NULL,
 binding_kind text NOT NULL CHECK(binding_kind IN ('pending_proposal_observation_match','pending_proposal_observation_mismatch')),
 created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
 UNIQUE(proposal_id,request_sha256),
 UNIQUE(receipt_id)
);
ALTER TABLE public.pdc_historical_proposal_bindings_789 ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pdc_historical_proposal_bindings_789 FORCE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.pdc_historical_proposal_bindings_789 FROM public,anon,authenticated,service_role,pdc_email_monitor;
CREATE OR REPLACE FUNCTION public.pdc_historical_proposal_binding_789_immutable() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $immutable$
BEGIN RAISE EXCEPTION 'PDC_789_PROPOSAL_BINDING_IMMUTABLE' USING errcode='55000'; END;
$immutable$;
REVOKE ALL ON FUNCTION public.pdc_historical_proposal_binding_789_immutable() FROM public,anon,authenticated,service_role,pdc_email_monitor;
GRANT EXECUTE ON FUNCTION public.pdc_historical_proposal_binding_789_immutable() TO postgres;
CREATE TRIGGER pdc_789_proposal_binding_immutable BEFORE UPDATE OR DELETE ON public.pdc_historical_proposal_bindings_789 FOR EACH ROW EXECUTE FUNCTION public.pdc_historical_proposal_binding_789_immutable();
CREATE OR REPLACE FUNCTION public.submit_pdc_historical_reconciliation_789_proposal_binding_base(p_request jsonb)
...
DECLARE
 ...
 v_proposal public.pdc_ai_intake_proposals%rowtype; v_proposal_id uuid; v_proposal_binding_id uuid:=gen_random_uuid(); v_proposal_observation_match boolean; v_proposal_binding_kind text;
BEGIN
...
REVOKE ALL ON FUNCTION public.submit_pdc_historical_reconciliation_789_proposal_binding_base(jsonb) FROM public,anon,authenticated,service_role,pdc_email_monitor;
GRANT EXECUTE ON FUNCTION public.submit_pdc_historical_reconciliation_789_proposal_binding_base(jsonb) TO postgres;
...
v_request_hash:=encode(extensions.digest(convert_to(v_canonical_request,'UTF8'),'sha256'),'hex');
 ...
 v_result:=public.submit_pdc_historical_reconciliation_789_proposal_binding_base(p_request);
...
DO $verify$
DECLARE d text; w text; b boolean; a boolean;
BEGIN
 SELECT pg_get_functiondef('public.submit_pdc_historical_reconciliation_789_proposal_binding_base(jsonb)'::regprocedure) INTO d;
 ...
 IF position('historical_proposal_tuple_conflict' in d)=0 OR position('historical_proposal_terminal_conflict' in d)=0 OR position('pending_proposal_observation_mismatch' in d)=0 OR position('pdc_historical_proposal_bindings_789' in d)=0 OR position('v_proposal.observations' in d)=0 OR position('v_request->''observations''' in d)=0 OR position('submit_pdc_historical_reconciliation_789_proposal_binding_base' in w)=0 OR NOT coalesce(b,false) OR has_function_privilege('authenticated','public.submit_pdc_historical_reconciliation_789_proposal_binding_base(jsonb)','execute') OR has_table_privilege('authenticated','public.pdc_historical_proposal_bindings_789','select') OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL THEN RAISE EXCEPTION 'PDC_789_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $verify$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830190000','789_historical_proposal_binding_successor',ARRAY['Compare existing pending proposal full source/evidence/UID/sender/authentication/Stock and payload tuple before historical writes','Allow only observation JSON drift through a separate immutable 789 proposal binding and canonical 788 receipt','Return typed conflict/review for material tuple or terminal proposal mismatch; preserve 788 atomicity, replay and sibling isolation']);
