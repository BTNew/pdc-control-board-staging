-- Staging-only: prevent Auditor batches from mutating completed work or overriding manual assignment locks.
begin;
set local lock_timeout='5s';
set local statement_timeout='180s';
select pg_advisory_xact_lock(hashtextextended('pdc-staging-201-auditor-completed-manual-work',0));
do $guard$ begin
 if not exists(select 1 from public.pdc_staging_environment_sentinel where singleton and project_ref='cdsmnqxtyyoeoznmbidd')
 or not exists(select 1 from supabase_migrations.schema_migrations where version='200' and name='claim_bound_canonical_monitor_work')
 or exists(select 1 from supabase_migrations.schema_migrations where version='201') then
  raise exception 'PDC_201_STAGING_OR_LEDGER_MISMATCH' using errcode='55000';
 end if;
end $guard$;
...
declare
 ...
 v_stage text;v_work_key text;v_source_work_key text;v_affected uuid[]:='{}';v_seq integer:=0;v_before_work jsonb;v_stock text;v_jc text;v_override boolean;v_identifier text;v_has_before boolean;
begin
 ...
 -- Preflight every item before creating a run or writing change/audit rows. The vehicle
 -- lock also closes the gap in which a completed work row could be inserted after this check.
 for v_item in select value from jsonb_array_elements(p_changes) loop
  v_action:=null;v_reason:=null;v_operation:=null;v_vehicle:=null;v_source:=null;v_before:=null;v_after:=null;
  v_stage:=null;v_work_key:=null;v_source_work_key:=null;v_stock:=null;v_jc:=null;v_override:=false;v_identifier:=null;v_has_before:=false;
  v_action:=v_item->>'action';v_reason:=btrim(coalesce(v_item->>'reason',''));v_override:=coalesce((v_item->>'override_manual_assignment')::boolean,false);
  ...
  if v_override then raise exception 'pdc_auditor_manual_assignment_override_forbidden' using errcode='42501'; end if;
  if v_action='add' then
   v_vehicle:=nullif(v_item->>'vehicle_id','')::uuid;
   select v.stock_number,v.job_card_number into v_stock,v_jc from public.vehicles v where v.id=v_vehicle and v.deleted_at is null and v.lifecycle_state='active' and public.pdc_auditor_vehicle_dealer(v.id)=v_dealer for update;
   ...
   v_vehicle:=v_source.vehicle_id;
   select v.stock_number,coalesce(v_source.job_card_number,v.job_card_number) into v_stock,v_jc from public.vehicles v where v.id=v_vehicle and v.deleted_at is null and public.pdc_auditor_vehicle_dealer(v.id)=v_dealer for update;
   if not found then raise exception 'pdc_auditor_vehicle_not_found' using errcode='P0002'; end if;
   ...
   v_has_before:=found;
   if v_has_before and v_before.manual_assignment_locked then raise exception 'pdc_auditor_manual_assignment_locked' using errcode='42501'; end if;
   v_source_work_key:=coalesce(public.pdc_auditor_work_key_for_stage(v_before.stage_code),v_source.work_key);
   v_work_key:=coalesce(nullif(v_item->>'work_key',''),v_source_work_key);
  ...
  perform 1 from public.vehicle_work_items wi where wi.vehicle_id=v_vehicle and wi.work_key in(v_source_work_key,v_work_key) for update;
  if exists(select 1 from public.vehicle_work_items wi where wi.vehicle_id=v_vehicle and wi.completed and wi.work_key in(v_source_work_key,v_work_key)) then
   raise exception 'pdc_auditor_completed_work_locked' using errcode='42501';
  end if;
 ...
 into v_before_work from public.vehicle_work_items wi where wi.vehicle_id=any(v_affected);
  ...
  if v_action='add' then
   ...
   if not v_has_before then
    ...
    update public.vehicle_workshop_line_adjustments set stage_code=v_stage,
      ...
      manual_assignment_locked=manual_assignment_locked where adjustment_id=v_before.adjustment_id returning * into v_after;
   end if;
  end if;
  v_affected:=array_append(v_affected,v_vehicle);
...
insert into supabase_migrations.schema_migrations(version,name,statements) values('201','harden_auditor_completed_and_manual_work',array['Reject client requests to override manual assignment locks','Lock affected source and target work and reject completed work before run, change, or audit writes','Reset per-item state while preserving the existing Auditor operation batch contract and ACL']);
commit;
