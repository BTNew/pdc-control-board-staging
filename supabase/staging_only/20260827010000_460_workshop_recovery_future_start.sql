-- STAGING ONLY 460: recovery starts at the next configured operational increment.
BEGIN;SET LOCAL lock_timeout='10s';SET LOCAL statement_timeout='120s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-460-workshop-recovery-future-start',0));
DO $pre$ BEGIN
 ...
 OR NOT EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260827009000' AND name='459_workshop_recovery_overlap_containment')
 OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' AND version>'20260827009000')
 OR encode(extensions.digest(convert_to(pg_get_functiondef('public.recover_overdue_planned_workshop_bookings(text,timestamptz)'::regprocedure),'UTF8'),'sha256'),'hex')<>'03c24794e39c0bc089e57814a7d77934ba56a21ac1e29d2bd88bead48f856804'
 THEN RAISE EXCEPTION 'PDC_460_STAGING_HEAD_OR_FUNCTION_MISMATCH' USING errcode='55000';END IF;
END $pre$;
...
DO $post$ BEGIN
 IF pg_get_functiondef('public.recover_overdue_planned_workshop_bookings(text,timestamptz)'::regprocedure) NOT LIKE '%greatest(v_as_of,clock_timestamp())%'
 ...
 THEN RAISE EXCEPTION 'PDC_460_POSTCONDITION_FAILED' USING errcode='55000';END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260827010000','460_workshop_recovery_future_start',ARRAY['Recovery begins at the next configured increment after both requested and server current time','Overlap containment from 459 remains active; no past or started/completed history is rewritten','Production untouched']);
