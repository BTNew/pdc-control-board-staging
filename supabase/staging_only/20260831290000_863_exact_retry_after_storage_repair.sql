-- STAGING ONLY 863: requeue the exact failed monitor intake after the
...
SELECT pg_advisory_xact_lock(hashtextextended('pdc-monitor-staging-863-requeue-692',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres'
    ...
    OR v_head IS DISTINCT FROM '20260831280000,pdc_checklist_completion_booking_preservation'
    ...
    OR to_regclass('public.pdc_monitor_exact_requeue_history_863') IS NOT NULL
 THEN RAISE EXCEPTION 'PDC_863_EXACT_CURRENT_REQUEUE_PRESTATE_FAILED' USING errcode='55000'; END IF;
END
$guard$;

CREATE TABLE public.pdc_monitor_exact_requeue_history_863(
 history_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
 event_key text NOT NULL UNIQUE,
 event_kind text NOT NULL CHECK(event_kind='exact_retry_after_storage_repair'),
 predecessor_head text NOT NULL CHECK(predecessor_head='20260831280000'),
 successor_head text NOT NULL CHECK(successor_head='20260831290000'),
...
ALTER TABLE public.pdc_monitor_exact_requeue_history_863 ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pdc_monitor_exact_requeue_history_863 FORCE ROW LEVEL SECURITY;
REVOKE ALL ON public.pdc_monitor_exact_requeue_history_863 FROM public,anon,authenticated,service_role,pdc_email_monitor;
CREATE FUNCTION public.pdc_monitor_exact_requeue_history_immutable_863()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public
AS $immutable$ BEGIN RAISE EXCEPTION 'PDC_863_EXACT_REQUEUE_HISTORY_IMMUTABLE' USING errcode='55000'; END $immutable$;
REVOKE ALL ON FUNCTION public.pdc_monitor_exact_requeue_history_immutable_863() FROM public,anon,authenticated,service_role,pdc_email_monitor;
CREATE TRIGGER pdc_monitor_exact_requeue_history_immutable_863 BEFORE UPDATE OR DELETE ON public.pdc_monitor_exact_requeue_history_863 FOR EACH ROW EXECUTE FUNCTION public.pdc_monitor_exact_requeue_history_immutable_863();
...
BEGIN
 ...
 IF NOT FOUND THEN RAISE EXCEPTION 'PDC_863_EXACT_REQUEUE_TARGET_LOST' USING errcode='55000'; END IF;
 ...
 IF NOT FOUND THEN RAISE EXCEPTION 'PDC_863_EXACT_REQUEUE_CONCURRENT_DRIFT' USING errcode='40001'; END IF;
 v_event_key:=encode(extensions.digest(convert_to('pdc-monitor-staging-863-requeue-692|forward|'||v_id::text,'UTF8'),'sha256'),'hex');
 INSERT INTO public.pdc_monitor_exact_requeue_history_863(event_key,event_kind,predecessor_head,successor_head,intake_id,provider_uid,source_hash,before_state,after_state,source_evidence_unchanged,attachments_unchanged,task_enabled,mailbox_flags_changed,uid514_processed,outbound_email_sent,production_writes,rollback_contract)
 VALUES(v_event_key,'exact_retry_after_storage_repair','20260831280000','20260831290000',v_id,'imap_uid:692','cdc66328f62d3eac365127763ac13ed01da83fe16ca951029d17360db6553565',v_before,v_after,true,(v_attachment_count=2),false,false,false,false,false,'Exact one-row retry after reviewed storage-path repairs; preserves source/evidence/attachments and records rollback/readback without task, mailbox-flag, UID514, outbound or Production action');
 INSERT INTO public.audit_events(action,table_name,row_id,actor_id,actor_email,before_data,after_data,metadata)
 VALUES('update','ai_email_intake',v_id,NULL,'staging-management-remediation',v_before,v_after,jsonb_build_object('event_type','pdc_monitor_exact_requeue_863','provider_uid','imap_uid:692','source_evidence_unchanged',true,'attachments_unchanged',v_attachment_count=2,'task_enabled',false,'mailbox_flags_changed',false,'uid514_processed',false,'outbound_email_sent',false,'production_untouched',true));
END
$requeue$;
...
BEGIN
 IF (SELECT count(*) FROM public.ai_email_intake WHERE id='0172352b-6045-4ab4-83ba-c8069c9ab8de'::uuid AND provider_uid='imap_uid:692' AND status='received' AND source_hash='cdc66328f62d3eac365127763ac13ed01da83fe16ca951029d17360db6553565' AND queue_attempts=2 AND NOT permanent_failure AND retry_class IS NULL AND next_attempt_at IS NOT NULL AND locked_at IS NULL AND locked_by IS NULL AND claim_token IS NULL AND gateway_instance_id IS NULL)<>1
    OR (SELECT count(*) FROM public.pdc_monitor_exact_requeue_history_863 WHERE event_kind='exact_retry_after_storage_repair' AND source_evidence_unchanged AND attachments_unchanged AND NOT task_enabled AND NOT mailbox_flags_changed AND NOT uid514_processed AND NOT outbound_email_sent AND NOT production_writes)<>1
    OR (SELECT count(*) FROM public.ai_email_attachments WHERE intake_id='0172352b-6045-4ab4-83ba-c8069c9ab8de'::uuid)<>2
    ...
    OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL
 THEN RAISE EXCEPTION 'PDC_863_EXACT_REQUEUE_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END
$post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260831290000','863_exact_retry_after_storage_repair',ARRAY['Requeue only the exact failed imap_uid:692 intake after 859-861 storage repairs','Preserve source hash evidence attachment count and all current attachment rows','Record forced-RLS immutable before/after retry evidence','Keep task UID514 mailbox flags outbound and Production controls fail-closed']);
