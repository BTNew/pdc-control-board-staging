-- Staging-only migration 126: short-circuit work-authority quarantine before identity scans.
begin;
...
begin
  if to_regclass('public.pdc_staging_environment_sentinel') is null
    ...
    raise exception 'PDC_BULK_126_STAGING_SENTINEL_MISMATCH';
  end if;
  if not exists(select 1 from supabase_migrations.schema_migrations where version='125' and name='fix_bulk_workbook_operation_number_contract') then
    raise exception 'PDC_BULK_126_PREDECESSOR_125_IDENTITY_MISMATCH';
  end if;
  if exists(select 1 from supabase_migrations.schema_migrations where version='126') then
    raise exception 'PDC_BULK_126_VERSION_CONFLICT';
  end if;
end;
$guard$;

create or replace function public.preview_pdc_bulk_jc_stock_workbook(p_workbook_sha256 text,p_payload jsonb)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions set statement_timeout='60s' as $preview$
declare
...
begin
  ...
  for v_row in select value from jsonb_array_elements(v_payload) loop
    ...
    v_reason:=null;
    if exists(select 1 from jsonb_array_elements(v_row->'operations') o where o->'work_key'='null'::jsonb) then
      v_reason:='missing_authoritative_work_key';
    else
    ...
    if v_exact=1 and v_stock_count=1 and v_jc_count=1 and v_bound=1 and v_oper_exact=1 and v_oper_partial=0 then v_accepted:=v_accepted+1;
...
values('126','short_circuit_bulk_workbook_work_authority_quarantine',array['staging-only Preview quarantines null work authority before database identity scans and has a bounded sixty-second function timeout']);
commit;
