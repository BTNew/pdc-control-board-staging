-- Staging-only: retain provider authentication evidence on intake rows and make enqueue binding executable.
begin;
set local lock_timeout='5s';set local statement_timeout='120s';
select pg_advisory_xact_lock(hashtextextended('pdc-staging-192-monitor-provider-columns',0));
do $guard$ begin
 if not exists(select 1 from public.pdc_staging_environment_sentinel where singleton and project_ref='cdsmnqxtyyoeoznmbidd')
    or not exists(select 1 from supabase_migrations.schema_migrations where version='191' and name='monitor_claim_bound_attachment_extraction')
    or exists(select 1 from supabase_migrations.schema_migrations where version='192') then raise exception 'PDC_192_STAGING_OR_LEDGER_MISMATCH' using errcode='55000';end if;
end $guard$;
alter table public.ai_email_intake add column provider_authserv_id text,add column provider_authentication jsonb;
...
declare s jsonb:=public.pdc_monitor_actor_scope();i public.ai_email_intake%rowtype;h text;sender text;begin
 ...
 on conflict(source_hash) do update set updated_at=clock_timestamp() returning * into i;
 ...
 perform public.touch_pdc_email_monitor_status('received',null,null,null);
...
insert into supabase_migrations.schema_migrations(version,name,statements) values('192','monitor_provider_evidence_columns_and_enqueue_binding',array['Retain provider authentication evidence on intake rows','Require exact active monitored mailbox, enrolled sender and aligned Gmail provider evidence at enqueue']);
commit;
