...
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-20260903040000-email-ai-success-evidence-immutability',0));
...
BEGIN
  ...
  IF current_user<>'postgres' OR session_user<>'postgres'
     ...
     OR v_head<>'20260903030000'
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='20260903030000' AND name='pdc_email_ai_runtime_rotation_fixture_hardening_20260903')<>1
     OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260903040000')
  THEN RAISE EXCEPTION 'PDC_20260903040000_STAGING_PRECONDITION_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
DROP TRIGGER pdc_email_ai_v2_acceptance_source_immutable_20260903 ON public.ai_email_intake;
...
DROP TRIGGER pdc_email_ai_v2_fixture_attachment_immutable_20260903 ON public.ai_email_attachments;
...
BEGIN
  ...
  THEN RAISE EXCEPTION 'PDC_20260903040000_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;

INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES(
 '20260903040000','pdc_email_ai_legacy_success_evidence_immutability_20260903',ARRAY[
  'Freeze successful receipt-bound correspondence and attachments against update, delete, insert and reparenting',
  'Validate current attachment hashes against the immutable intake attachment digest list before exact replay',
  'Retain exact legacy evidence digest compatibility while preventing independent source or evidence alteration',
