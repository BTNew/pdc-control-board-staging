...
-- Append-only successor to the observed live head 20260830165000.
...
begin
  if current_user<>'postgres' or session_user<>'postgres'
     ...
     or not exists(select 1 from supabase_migrations.schema_migrations where version='20260830165000' and name='777_historical_reconciliation_canonical_adapter_repair')
     ...
     or exists(select 1 from supabase_migrations.schema_migrations where version='20260830166000') then
    raise exception 'PDC_777_1660_PREDECESSOR_OR_CANONICAL_CONTRACT_MISMATCH' using errcode='55000';
  end if;
end
$guard$;
...
insert into supabase_migrations.schema_migrations(version,name,statements) values('20260830166000','777_historical_reconciliation_canonical_adapter_repair2',array[
...
begin
 if to_regclass('public.pdc_production_environment_sentinel') is not null
   ...
   raise exception 'PDC_777_1660_POSTCONDITION_FAILED' using errcode='55000';
 end if;
end
$verify$;
