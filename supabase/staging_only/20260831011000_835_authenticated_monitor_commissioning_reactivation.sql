-- STAGING ONLY 835: re-enable the exact pre-provisioned monitor test mailbox
...
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-835-authenticated-monitor-commissioning-reactivation',0));
...
BEGIN
  ...
  IF current_user<>'postgres' OR session_user<>'postgres'
     ...
     OR v_head IS DISTINCT FROM '20260831010000,834_monitor_current_head_833_compatibility_successor'
     ...
     OR (SELECT count(*) FROM public.ai_email_intake WHERE provider_uid='imap_uid:514' AND status='processing' AND source_hash='440d174b6e1994fbb1afb9c285f068cb62b130f541ebaa834d590f44bdbec280' AND monitored_mailbox_id='12fe383d-5c1e-5801-96e4-f67cf3e3bb57' AND lower(recipient_mailbox)='pmbcontroller@gmail.com' AND locked_by='df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b'::uuid AND gateway_instance_id='pdc-monitor-staging-sales-uid509-v1')<>1
     OR (SELECT count(*) FROM public.pdc_uid514_staging_commissioning_terminal_receipts_507 WHERE recovery_event_id=25751401 AND actor_id='df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b' AND gateway_instance_id='pdc-monitor-staging-sales-uid509-v1' AND release_name='pdc-monitor-staging-m502-2026.08.44' AND source_sha='e850c319989d98b45b95a28aa815d78e2c2e3a4b' AND manifest_sha256='d48b49f6598a99fbef99fc4f0d0ab36b8b47576b8ff7cd8ecd2cb64d6cfed58d' AND mailbox_uidvalidity=1 AND mailbox_uid=514 AND terminal_status='staging_commissioned' AND synthetic_staging_commissioning AND NOT physical_mailbox_fetch AND NOT mailbox_flags_changed AND vehicle_operations=0 AND operation_lines=0 AND NOT operational AND NOT activation_ready AND NOT writer_active AND NOT planner_commissioned AND NOT production_writes)<>1
     OR (SELECT count(*) FROM public.pdc_uid514_receipt_code_compatibility_controls_508 WHERE recovery_event_id=25751401 AND response_code='uid514_receipt_terminal' AND receipt_kind='staging_commissioning' AND receipt_source='logical_507_exact_terminal_receipt' AND NOT operational AND NOT activation_ready AND NOT writer_active AND NOT planner_commissioned AND NOT production_writes)<>1
     OR to_regclass('public.pdc_email_monitor_authenticated_mailbox_reactivation_history_835') IS NOT NULL
  THEN RAISE EXCEPTION 'PDC_835_EXACT_833_COMMISSIONING_PRESTATE_FAILED' USING errcode='55000'; END IF;
END
$guard$;

CREATE TABLE public.pdc_email_monitor_authenticated_mailbox_reactivation_history_835(
  history_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  event_key text NOT NULL UNIQUE,
  event_kind text NOT NULL CHECK(event_kind='commissioning_reactivation'),
  predecessor_head text NOT NULL CHECK(predecessor_head='20260831010000'),
  successor_head text NOT NULL CHECK(successor_head='20260831011000'),
...
ALTER TABLE public.pdc_email_monitor_authenticated_mailbox_reactivation_history_835 ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pdc_email_monitor_authenticated_mailbox_reactivation_history_835 FORCE ROW LEVEL SECURITY;
REVOKE ALL ON public.pdc_email_monitor_authenticated_mailbox_reactivation_history_835 FROM public,anon,authenticated,service_role,pdc_email_monitor;

CREATE FUNCTION public.pdc_email_monitor_authenticated_mailbox_reactivation_history_immutable_835()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public
AS $immutable$ BEGIN RAISE EXCEPTION 'PDC_835_AUTHENTICATED_MAILBOX_REACTIVATION_HISTORY_IMMUTABLE' USING errcode='55000'; END $immutable$;
REVOKE ALL ON FUNCTION public.pdc_email_monitor_authenticated_mailbox_reactivation_history_immutable_835() FROM public,anon,authenticated,service_role,pdc_email_monitor;
CREATE TRIGGER pdc_email_monitor_authenticated_mailbox_reactivation_history_immutable_835
BEFORE UPDATE OR DELETE ON public.pdc_email_monitor_authenticated_mailbox_reactivation_history_835
FOR EACH ROW EXECUTE FUNCTION public.pdc_email_monitor_authenticated_mailbox_reactivation_history_immutable_835();
...
BEGIN
  SELECT * INTO v_mailbox FROM public.monitored_mailboxes
   ...
   FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'PDC_835_EXACT_MAILBOX_REACTIVATION_SCOPE_FAILED' USING errcode='55000'; END IF;
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'PDC_835_MAILBOX_REACTIVATION_CONCURRENT_DRIFT' USING errcode='40001'; END IF;
  ...
  v_event_key:=encode(extensions.digest(convert_to('pdc-staging-835-authenticated-monitor-commissioning-reactivation|forward|12fe383d-5c1e-5801-96e4-f67cf3e3bb57','UTF8'),'sha256'),'hex');
  INSERT INTO public.pdc_email_monitor_authenticated_mailbox_reactivation_history_835(
  ...
  VALUES(v_event_key,'commissioning_reactivation','20260831010000','20260831011000',
  ...
  VALUES('update','monitored_mailboxes',v_mailbox.id,NULL,'staging-management-remediation',v_before_mailbox,v_after_mailbox,
    jsonb_build_object('event_type','pdc_email_monitor_authenticated_mailbox_reactivated_835','authorized_by','Craig Watson','actor_id','df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b','gateway_instance_id','pdc-monitor-staging-sales-uid509-v1','exact_mailbox_only',true,'task_enabled',false,'mailbox_flags_changed',false,'uid514_processed',false,'outbound_email_sent',false,'production_untouched',true));
END
$activate$;
...
BEGIN
  IF (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>1
     ...
     OR (SELECT count(*) FROM public.pdc_uid514_staging_commissioning_terminal_receipts_507 WHERE recovery_event_id=25751401 AND actor_id='df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b' AND gateway_instance_id='pdc-monitor-staging-sales-uid509-v1' AND release_name='pdc-monitor-staging-m502-2026.08.44' AND source_sha='e850c319989d98b45b95a28aa815d78e2c2e3a4b' AND manifest_sha256='d48b49f6598a99fbef99fc4f0d0ab36b8b47576b8ff7cd8ecd2cb64d6cfed58d' AND mailbox_uidvalidity=1 AND mailbox_uid=514 AND terminal_status='staging_commissioned' AND synthetic_staging_commissioning AND NOT physical_mailbox_fetch AND NOT mailbox_flags_changed AND vehicle_operations=0 AND operation_lines=0 AND NOT operational AND NOT activation_ready AND NOT writer_active AND NOT planner_commissioned AND NOT production_writes)<>1
     OR (SELECT count(*) FROM public.pdc_uid514_receipt_code_compatibility_controls_508 WHERE recovery_event_id=25751401 AND response_code='uid514_receipt_terminal' AND receipt_kind='staging_commissioning' AND receipt_source='logical_507_exact_terminal_receipt' AND NOT operational AND NOT activation_ready AND NOT writer_active AND NOT planner_commissioned AND NOT production_writes)<>1
     OR (SELECT count(*) FROM public.pdc_email_monitor_authenticated_mailbox_reactivation_history_835 WHERE event_kind='commissioning_reactivation' AND NOT task_enabled AND NOT mailbox_flags_changed AND NOT uid514_processed AND NOT outbound_email_sent AND NOT production_writes)<>1
     OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL
  THEN RAISE EXCEPTION 'PDC_835_AUTHENTICATED_MONITOR_COMMISSIONING_REACTIVATION_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END
$post$;

INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260831011000','835_authenticated_monitor_commissioning_reactivation',ARRAY[
