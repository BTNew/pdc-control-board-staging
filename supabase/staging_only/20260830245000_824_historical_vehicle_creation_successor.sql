-- STAGING ONLY 824: invoke canonical Navision vehicle creation before postcondition.
-- The historical child has a current backend Stock with no operational vehicle.
-- pre310 already owns the bounded board activation subtransaction; invoke the
-- existing low-level pre171 reconciler there so it creates the deterministic
-- vehicle/link before the unchanged operational identity postcondition.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-824-historical-vehicle-creation',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830244000,823_historical_parent_fan_in_successor)' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM 'c230762a533f3af4327cc250f5b0729212b3ce79ec52d528491f90adfdd37881' OR position('pdc_ai_intake_135_postcondition_failed' in v)=0 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_824_CURRENT_HEAD_OR_PRE310_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
begin
  ...
  begin
    ...
    perform public.reconcile_navision_operational_record_pre171(v_record.id,p_actor_id,v_actor_email);
    select coalesce(array_agg(distinct vehicle_id order by vehicle_id),'{}'::uuid[])
    into v_operational_ids
    from (
...
BEGIN
 ...
 IF encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'1dfa773a939bd4a580b8ff646c94f661cb147d3c569de0081b4dbafa1f13e7e4' OR owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' THEN RAISE EXCEPTION 'PDC_824_HISTORICAL_VEHICLE_CREATION_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830245000','824_historical_vehicle_creation_successor',ARRAY['invoke existing pre171 canonical Navision vehicle creation inside the bounded pre310 activation subtransaction','preserve the existing PDC_AI_INTAKE_135_POSTCONDITION_FAILED identity check and atomic rollback','preserve exact auth evidence identity RLS grants idempotency ten conflicts and zero-mailbox containment','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
