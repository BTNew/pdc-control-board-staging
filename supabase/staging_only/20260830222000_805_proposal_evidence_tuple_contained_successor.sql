-- STAGING ONLY 805: proposal/evidence tuple compatibility successor after 804.
-- The frozen export is authoritative and immutable. Existing pending proposal
-- rows and source claims are never overwritten, deleted or equated when their
-- tuples differ. Only genuine unclaimed frozen source hashes may reach the
-- existing append-only pending-proposal creation contract.
-- This successor closes the remaining nested 793 proposal-review and 777
-- writer-authorization legacy mailbox gates with the authenticated 802/672
-- zero-mailbox adapter. No historical Apply, outbox, mailbox, task, pilot,
-- outbound or Production path is invoked here.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-805-proposal-evidence-tuple',0));
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
DO $guard$
DECLARE v_head text; v_acl text; v_owner text; v_secdef boolean; v777 text; v793 text;
BEGIN
 ...
 SELECT p.proowner::regrole::text,p.prosecdef,p.proacl::text,p.prosrc INTO v_owner,v_secdef,v_acl,v777 FROM pg_proc p WHERE p.oid='public.pdc_historical_writer_authorized_777(text,text,text,text,jsonb,text,jsonb)'::regprocedure;
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard()
    OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL
    OR v_head IS DISTINCT FROM '(20260830221000,804_nested_pre797_contained_runtime_802_successor)'
    OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' AND version>'20260830221000')
    ...
    OR to_regprocedure('public.submit_pdc_historical_reconciliation_793_proposal_review_successor(jsonb)') IS NULL
    OR to_regprocedure('public.submit_pdc_historical_reconciliation_778(jsonb)') IS NULL
    OR to_regprocedure('public.pdc_historical_writer_authorized_777(text,text,text,text,jsonb,text,jsonb)') IS NULL
    ...
    OR v_owner<>'postgres' OR NOT v_secdef OR v_acl<>'{postgres=X/postgres}'
    OR encode(extensions.digest(convert_to(v777,'UTF8'),'sha256'),'hex')<> '61b20e18a9baffb2bf6c5df3f2c5a7fbb2eff28c201858db9e627793e8adb8c2'
    OR (SELECT count(*) FROM public.pdc_email_source_claims)<>19
 THEN RAISE EXCEPTION 'PDC_805_CURRENT_HEAD_OR_CONTAINMENT_GUARD_FAILED' USING errcode='55000'; END IF;
 SELECT p.prosrc INTO v793 FROM pg_proc p WHERE p.oid='public.submit_pdc_historical_reconciliation_793_proposal_review_successor(jsonb)'::regprocedure;
 IF encode(extensions.digest(convert_to(v793,'UTF8'),'sha256'),'hex')<> '51e45e06a18356865753d90b69fd53a12d6b18ba2782b4166e75a8ac016a69eb'
 THEN RAISE EXCEPTION 'PDC_805_NESTED_SOURCE_PRESTATE_MISMATCH' USING errcode='55000'; END IF;
END $guard$;
...
DO $post$
DECLARE v777 text; v793 text; v_acl text; v_owner text; v_secdef boolean;
BEGIN
 SELECT p.prosrc,p.proowner::regrole::text,p.prosecdef,p.proacl::text INTO v777,v_owner,v_secdef,v_acl FROM pg_proc p WHERE p.oid='public.pdc_historical_writer_authorized_777(text,text,text,text,jsonb,text,jsonb)'::regprocedure;
 IF encode(extensions.digest(convert_to(v777,'UTF8'),'sha256'),'hex')<> 'e395bf4b7c2ca358ecbc930034f69a57ee145709a9772b896c7bee0e5a476215' OR v_owner<>'postgres' OR NOT v_secdef OR v_acl<>'{postgres=X/postgres}' OR position('pdc_monitor_authenticated_active_scope_674' in v777)>0 OR position('verify_pdc_historical_runtime_binding_authenticated_802' in v777)=0 THEN RAISE EXCEPTION 'PDC_805_WRITER_AUTH_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
 SELECT p.prosrc,p.proowner::regrole::text,p.prosecdef,p.proacl::text INTO v793,v_owner,v_secdef,v_acl FROM pg_proc p WHERE p.oid='public.submit_pdc_historical_reconciliation_793_proposal_review_successor(jsonb)'::regprocedure;
 IF encode(extensions.digest(convert_to(v793,'UTF8'),'sha256'),'hex')<> '5227b575c60a66987e59e2ef0d9e4ac8c1128dfb463d8974a083730d16ed74a7' OR v_owner<>'postgres' OR NOT v_secdef OR v_acl<>'{postgres=X/postgres}' OR position('pdc_monitor_authenticated_active_scope_674' in v793)>0 OR position('verify_pdc_historical_runtime_binding_authenticated_802' in v793)=0 THEN RAISE EXCEPTION 'PDC_805_793_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
 IF position('pdc_monitor_authenticated_active_scope_674' in (SELECT p.prosrc FROM pg_proc p WHERE p.oid='public.submit_pdc_historical_reconciliation_778(jsonb)'::regprocedure))>0 THEN RAISE EXCEPTION 'PDC_805_PUBLIC_SCOPE_DRIFT' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830222000','805_proposal_evidence_tuple_contained_successor',ARRAY[
 'replace only the legacy 674 nested proposal-review and writer-authorization mailbox predicates with authenticated 802/672 zero-mailbox containment',
 'preserve immutable proposal/source-claim tuple conflicts and allow only genuine unclaimed frozen evidence to reach the existing append-only pending-proposal creation contract',
 'preserve frozen export identity, sibling isolation, replay/idempotency, evidence, RLS, grants, task, pilot, outbound and Production fail-closed boundaries'
]);
