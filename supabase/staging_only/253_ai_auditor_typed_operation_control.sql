...
begin
 ...
 if found then
  ...
  if (select count(*) from jsonb_object_keys(actor))<>7
   or not (actor ?& array['service_identity_id','service_user_id','service_email','admin_user_id','admin_email','dealer_code','environment'])
   or exists(select 1 from unnest(array['service_identity_id','service_user_id','service_email','admin_user_id','admin_email','dealer_code','environment']) actor_key where jsonb_typeof(actor->actor_key) is distinct from 'string')
   ...
   or btrim(actor->>'service_email')='' or btrim(actor->>'admin_email')='' or btrim(actor->>'dealer_code')='' or actor->>'environment'<>'staging'
