-- Staging-only remediation: queue intake identifiers use the existing UUID contract.
begin;
do $guard$ begin if not exists(select 1 from public.pdc_staging_environment_sentinel where singleton and project_ref='cdsmnqxtyyoeoznmbidd') or not exists(select 1 from supabase_migrations.schema_migrations where version='184') or exists(select 1 from supabase_migrations.schema_migrations where version='185') then raise exception 'PDC_185_GUARD_MISMATCH';end if;end $guard$;
...
declare s jsonb:=public.pdc_monitor_actor_scope();v_id uuid;v_new boolean:=false;a jsonb;
begin
...
revoke all on function public.enqueue_pdc_email_intake(jsonb,jsonb) from public,anon,authenticated,service_role;grant execute on function public.enqueue_pdc_email_intake(jsonb,jsonb) to authenticated;insert into supabase_migrations.schema_migrations(version,name,statements) values('185','align_monitor_enqueue_uuid_intake_id',array['Use the existing UUID intake identifier contract']);commit;
