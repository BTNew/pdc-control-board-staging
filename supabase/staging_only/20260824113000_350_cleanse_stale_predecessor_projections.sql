-- STAGING ONLY 350: repair the one-shot cleanse for operational children
-- recreated against vehicles already purged by the 2026-08-18 run.
begin;
set local lock_timeout='10s';
set local statement_timeout='600s';
select pg_advisory_xact_lock(hashtextextended('pdc-staging-350-stale-predecessor-cleanup',0));
...
begin
  if not public.pdc_monitor_staging_guard()
     ...
     or exists(select 1 from supabase_migrations.schema_migrations
               where version='20260824113000') then
    raise exception 'PDC_350_STAGING_PREDECESSOR_OR_FAILED_ATTEMPT_MISMATCH' using errcode='55000';
  end if;
end
$guard$;
...
begin
  ...
  update public.navision_board_activations x
     set active=false,completed_at=coalesce(completed_at,clock_timestamp()),
         completion_reason=coalesce(completion_reason,'Staging board cleanse predecessor repair'),
         updated_at=clock_timestamp()
    from public.vehicles v where x.canonical_vehicle_id=v.id and x.active
     and v.board_purged_at is not null and v.deleted_at is not null
     and v.lifecycle_state='deleted' and not v.visible_on_board;
...
values('20260824113000','350_cleanse_stale_predecessor_projections',array[
  'Require exact staging target, containment and zero committed cleanse receipts',
  'Delete only backup-covered workshop/work projections attached to already-purged vehicles',
  'Deactivate only backup-covered stale line-adjustment and Navision activation projections',
  'Retain original fixed-catalog purge, replay-evidence equality and zero-state postconditions'
]);
