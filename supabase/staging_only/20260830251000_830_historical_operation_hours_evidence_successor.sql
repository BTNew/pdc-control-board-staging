-- STAGING ONLY 830: preserve evidence-faithful operation-hour aggregates.
-- Unknown per-line hours remain NULL; the legacy aggregate is NULL when no
-- known numeric hours exist, while explicit numeric zero remains 0.
-- Add explicit known/unknown counts and coverage so NULL is never treated as zero.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-830-operation-hours-evidence',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830249000,828_historical_replay_order_successor)' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM 'ccdd559262b3c8c180f1fb374863c487d3c45d99f161ca904bebbe9d740bdd7d' OR position('coalesce(sum((x->>''estimated_hours'')::numeric),0)' in lower(v))=0 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_830_CURRENT_HEAD_OR_OPERATION_HOURS_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
ALTER TABLE public.pdc_jobcard_attachment_import_receipts ALTER COLUMN estimated_hours_sum DROP NOT NULL;
ALTER TABLE public.pdc_jobcard_attachment_import_receipts ADD COLUMN known_hours_sum numeric(10,2), ADD COLUMN known_hours_count integer, ADD COLUMN unknown_hours_count integer, ADD COLUMN hours_coverage numeric(6,5);
ALTER TABLE public.pdc_jobcard_attachment_import_receipts ADD CONSTRAINT pdc_jobcard_attachment_import_receipts_known_hours_sum_check CHECK (known_hours_sum IS NULL OR (known_hours_sum>=0 AND known_hours_sum<=49999.50)), ADD CONSTRAINT pdc_jobcard_attachment_import_receipts_known_hours_count_check CHECK (known_hours_count IS NULL OR (known_hours_count BETWEEN 0 AND 50)), ADD CONSTRAINT pdc_jobcard_attachment_import_receipts_unknown_hours_count_check CHECK (unknown_hours_count IS NULL OR (unknown_hours_count BETWEEN 0 AND 50)), ADD CONSTRAINT pdc_jobcard_attachment_import_receipts_hours_coverage_check CHECK (hours_coverage IS NULL OR (hours_coverage>=0 AND hours_coverage<=1));
...
declare
  ...
  v_known_hours_sum numeric(10,2);
  v_known_hours_count integer;
  v_unknown_hours_count integer;
  v_hours_coverage numeric(6,5);
  v_line_ids uuid[];
  v_submit jsonb;
...
begin
  ...
  select count(*) filter (where jsonb_typeof(x->'estimated_hours')='number'),
    count(*) filter (where jsonb_typeof(x->'estimated_hours')='null'),
    sum((x->>'estimated_hours')::numeric) filter (where jsonb_typeof(x->'estimated_hours')='number')
  into v_known_hours_count,v_unknown_hours_count,v_known_hours_sum
  from jsonb_array_elements(v_canonical_lines) x;
  -- NULL means no known hour value exists; explicit numeric zero remains zero.
  v_hours_sum:=v_known_hours_sum;
  v_hours_coverage:=round(v_known_hours_count::numeric/nullif(v_operation_count,0),5);
  ...
  if v_intake.duplicate_of is not null or v_intake.status in ('duplicate_detected','failed','ignored','vehicle_created')
     ...
     or (v_intake.received_at<clock_timestamp()-interval '30 days' and not public.pdc_historical_writer_authorized_773(v_parent_hash,coalesce(v_intake.provider_uid,''),v_sender,v_auth,v_email_vehicle->'stock_numbers'->>0)) then
  ...
  if v_intake.monitored_mailbox_id is null
     and public.pdc_historical_writer_authorized_773(v_parent_hash,coalesce(v_intake.provider_uid,''),v_sender,v_auth,v_email_vehicle->'stock_numbers'->>0) then
    perform set_config('pdc.historical_context',jsonb_build_object(
      'parent_source_hash',v_parent_hash,'provider_uid',coalesce(v_intake.provider_uid,''),
      'sender_email',v_sender,'authentication',v_auth,
      'stock_number',v_stock,'canonical_source_uid',v_source_uid,
  ...
  begin
    ...
    v_response:=jsonb_build_object(
      ...
      'estimated_hours_sum',v_hours_sum,'known_hours_sum',v_known_hours_sum,
      'known_hours_count',v_known_hours_count,'unknown_hours_count',v_unknown_hours_count,
      'hours_coverage',v_hours_coverage,'hours_knowledge_status',case when v_unknown_hours_count=0 then 'complete' when v_known_hours_count=0 then 'all_unknown' else 'partial' end);
    insert into public.pdc_jobcard_attachment_import_receipts(
      ...
      operation_count,estimated_hours_sum,known_hours_sum,known_hours_count,unknown_hours_count,hours_coverage,canonical_operation_line_ids,response
    ) values(
      ...
      v_operation_count,v_hours_sum,v_known_hours_sum,v_known_hours_count,v_unknown_hours_count,v_hours_coverage,v_line_ids,v_response);
    ...
    values('insert','pdc_jobcard_attachment_import_receipts',v_receipt_id,v_vehicle.id,v_actor,v_actor_email,null,
      jsonb_build_object('receipt_id',v_receipt_id,'intake_id',p_intake_id,'attachment_id',p_attachment_id,
        ...
        'known_hours_sum',v_known_hours_sum,'known_hours_count',v_known_hours_count,
        'unknown_hours_count',v_unknown_hours_count,'hours_coverage',v_hours_coverage),
...
BEGIN
 ...
 IF encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'171613a477d28621459ec224de11ed3858cdfb92addbd7f68bfbcad5a7dff484' OR owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' OR position('v_known_hours_count' in v)=0 OR position('v_unknown_hours_count' in v)=0 OR position('v_hours_coverage' in v)=0 THEN RAISE EXCEPTION 'PDC_830_OPERATION_HOURS_EVIDENCE_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830251000','830_historical_operation_hours_evidence_successor',ARRAY['preserve NULL per-line estimated_hours','preserve NULL aggregate estimated_hours_sum when all lines are unknown','preserve explicit numeric zero and add known_hours_sum known_hours_count unknown_hours_count hours_coverage','preserve atomic rollback idempotency identity evidence RLS grants ten conflicts and zero mailbox containment','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
