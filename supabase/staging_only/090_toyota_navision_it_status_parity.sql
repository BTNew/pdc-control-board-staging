-- Staging-only migration 090: align all Toyota Navision pre-arrival statuses
-- with the Kewdale ETA gate. This closes the UI/server gap for Planned for
-- Production, Line Off Complete, Final Inspection and Ready for Shipment.
begin;
...
begin
  ...
  if to_regprocedure('public.navision_kewdale_eta_from_payload(jsonb)') is null
    ...
    raise exception 'PDC_MIGRATION_090_DEPENDENCY_MISSING';
  end if;
end;
$guard$;
...
begin
  ...
  loop
    ...
    v_target:=case when v_derived='IT' then 'IT' when v_derived='Other' then 'Other' else v_row.current_location end;
    if v_row.eta_to_kewdale is distinct from v_eta or v_row.current_location is distinct from v_target then
      update public.vehicles set eta_to_kewdale=v_eta,current_location=v_target,version=version+1
      where id=v_row.id returning to_jsonb(public.vehicles.*) into v_after;
      ...
      values('update','vehicles',v_row.id,v_row.id,v_row.before_data,v_after,
        jsonb_build_object('source','toyota_navision_it_status_parity_090','reason','All pre-arrival Navision statuses require Kewdale ETA for IT'));
      v_changed:=v_changed+1;
    end if;
  end loop;
  if v_changed>0 then update public.pdc_email_vehicle_revision set revision=revision+1,updated_at=clock_timestamp() where singleton; end if;
end;
$backfill$;
