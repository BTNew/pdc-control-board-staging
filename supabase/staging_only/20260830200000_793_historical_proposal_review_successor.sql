-- STAGING ONLY 793: pending-proposal compatibility review successor after 792.
-- When the full proposal tuple matches but no authorized intake exists, record
-- an immutable canonical review and stop before enqueue; never mutate the old
-- proposal or bypass the protected pilot/intake gate.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-793-historical-proposal-review',0));
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
DO $guard$
BEGIN
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830193000,792_historical_vehicle_identity_successor)' OR to_regprocedure('public.submit_pdc_historical_reconciliation_792_vehicle_identity_successor(jsonb)') IS NULL OR to_regprocedure('public.submit_pdc_historical_reconciliation_793_proposal_review_successor(jsonb)') IS NOT NULL OR to_regclass('public.pdc_historical_proposal_compatibility_reviews_793') IS NOT NULL THEN RAISE EXCEPTION 'PDC_793_CURRENT_HEAD_GUARD_FAILED' USING errcode='55000'; END IF;
END $guard$;
CREATE TABLE public.pdc_historical_proposal_compatibility_reviews_793(
 review_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
 proposal_id uuid NOT NULL REFERENCES public.pdc_ai_intake_proposals(proposal_id) ON DELETE RESTRICT,
 contract_version text NOT NULL CHECK(contract_version='793.1'),
 ...
 review_code text NOT NULL CHECK(review_code='historical_proposal_observation_review_required'),
 created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
 UNIQUE(proposal_id,request_sha256)
);
ALTER TABLE public.pdc_historical_proposal_compatibility_reviews_793 ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pdc_historical_proposal_compatibility_reviews_793 FORCE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.pdc_historical_proposal_compatibility_reviews_793 FROM public,anon,authenticated,service_role,pdc_email_monitor;
CREATE OR REPLACE FUNCTION public.pdc_historical_proposal_compatibility_review_793_immutable() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $immutable$
BEGIN RAISE EXCEPTION 'PDC_793_PROPOSAL_REVIEW_IMMUTABLE' USING errcode='55000'; END;
$immutable$;
REVOKE ALL ON FUNCTION public.pdc_historical_proposal_compatibility_review_793_immutable() FROM public,anon,authenticated,service_role,pdc_email_monitor;
GRANT EXECUTE ON FUNCTION public.pdc_historical_proposal_compatibility_review_793_immutable() TO postgres;
CREATE TRIGGER pdc_793_proposal_review_immutable BEFORE UPDATE OR DELETE ON public.pdc_historical_proposal_compatibility_reviews_793 FOR EACH ROW EXECUTE FUNCTION public.pdc_historical_proposal_compatibility_review_793_immutable();
CREATE OR REPLACE FUNCTION public.submit_pdc_historical_reconciliation_793_proposal_review_successor(p_request jsonb)
...
v_request_hash:=encode(extensions.digest(convert_to(v_canonical_request,'UTF8'),'sha256'),'hex');
 ...
 v_result:=public.submit_pdc_historical_reconciliation_793_proposal_review_successor(p_request);
...
BEGIN
 SELECT pg_get_functiondef('public.submit_pdc_historical_reconciliation_793_proposal_review_successor(jsonb)'::regprocedure) INTO d;
 SELECT pg_get_functiondef('public.submit_pdc_historical_reconciliation_778(jsonb)'::regprocedure) INTO w;
 SELECT relrowsecurity AND relforcerowsecurity INTO b FROM pg_class WHERE oid='public.pdc_historical_proposal_compatibility_reviews_793'::regclass;
 IF position('historical_proposal_observation_review_required' in lower(d))=0 OR position('v_parent_result->>''code''=''already_noticed''' in lower(d))=0 OR position('on conflict(proposal_id,request_sha256) do nothing' in lower(d))=0 OR position('submit_pdc_historical_reconciliation_793_proposal_review_successor' in lower(w))=0 OR position('historical_proposal_tuple_conflict' in lower(w))=0 OR position('limit 1 for update' in lower(w))=0 OR NOT coalesce(b,false) OR has_function_privilege('authenticated','public.submit_pdc_historical_reconciliation_793_proposal_review_successor(jsonb)','execute') OR has_table_privilege('authenticated','public.pdc_historical_proposal_compatibility_reviews_793','select') OR has_function_privilege('anon','public.submit_pdc_historical_reconciliation_778(jsonb)','execute') OR has_function_privilege('service_role','public.submit_pdc_historical_reconciliation_778(jsonb)','execute') OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL THEN RAISE EXCEPTION 'PDC_793_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $verify$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830200000','793_historical_proposal_review_successor',ARRAY['Record immutable canonical review when an existing pending proposal has the same source tuple but no authorized intake','Stop before enqueue and preserve protected pilot/intake gate; never update/delete/reclassify/equate the pending proposal','Retain typed conflicts, replay, forced RLS, private base and authenticated-only wrapper controls']);
