-- Staging-only 771 successor: narrow authenticated Workshop Admin-block audit projection.
-- This append-only successor preserves the original source contract after the
-- 20260830073000 logical/timestamp slot was occupied by another migration.
...
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-771-successor-workshop-admin-block-audit',0));
...
BEGIN
  IF current_user <> 'postgres'
     ...
     OR (SELECT max(version) FILTER (WHERE version~'^[0-9]{14}$')
         FROM supabase_migrations.schema_migrations) <> '20260830095000'
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations
         WHERE version='20260830095000' AND name='pdc_lifecycle_history_yard_hold_transition') <> 1
     OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260830100000')
...
CREATE OR REPLACE FUNCTION public.get_workshop_admin_block_audit_771_successor(
  p_stage_code text,
...
DECLARE
  ...
  v_dealer_code text;
BEGIN
  v_actor_scope:=public.pdc_auditor_actor_scope();
  v_dealer_code:=v_actor_scope->>'dealer_code';
  ...
  FOR v_block IN
    ...
    WHERE a.stage_id=v_stage.id AND a.bay_id=v_bay.id
      ...
      AND EXISTS (
        ...
        WHERE h.metadata ? 'admin_block_id'
          AND h.metadata->>'admin_block_id'=a.id::text
          AND public.pdc_auditor_vehicle_dealer(b.vehicle_id)=v_dealer_code
      )
  ...
  v_response:=jsonb_build_object(
    'ok',true,'contract','get_workshop_admin_block_audit_771_successor',
    ...
    'cascade_history',(
      ...
      JOIN public.workshop_bookings b ON b.id=h.booking_id
      WHERE h.metadata ? 'admin_block_id'
        AND h.metadata->>'admin_block_id'=ANY(v_block_ids::text[])
        AND public.pdc_auditor_vehicle_dealer(b.vehicle_id)=v_dealer_code
    ),
    'history',(
      SELECT coalesce(jsonb_agg(jsonb_build_object(
...
REVOKE ALL ON FUNCTION public.get_workshop_admin_block_audit_771_successor(text,integer,date,date,uuid) FROM public,anon,service_role;
GRANT EXECUTE ON FUNCTION public.get_workshop_admin_block_audit_771_successor(text,integer,date,date,uuid) TO authenticated;
COMMENT ON FUNCTION public.get_workshop_admin_block_audit_771_successor(text,integer,date,date,uuid) IS
...
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES(
 '20260830100000','771_workshop_admin_block_audit_projection_successor',ARRAY[
  'Require exact staging predecessor 20260830095000 and reject production sentinel after the 20260830073000 collision',
