...
select pg_advisory_xact_lock(hashtextextended('pdc-staging-193-monitor-enqueue-partial-index',0));
do $guard$ begin if not exists(select 1 from public.pdc_staging_environment_sentinel where singleton and project_ref='cdsmnqxtyyoeoznmbidd') or not exists(select 1 from supabase_migrations.schema_migrations where version='192' and name='monitor_provider_evidence_columns_and_enqueue_binding') or exists(select 1 from supabase_migrations.schema_migrations where version='193') then raise exception 'PDC_193_STAGING_OR_LEDGER_MISMATCH' using errcode='55000';end if;end $guard$;
...
insert into supabase_migrations.schema_migrations(version,name,statements) values('193','align_monitor_enqueue_partial_source_hash_index',array['Use the existing partial source-hash unique index for provider-bound enqueue idempotency']);
commit;
