-- STAGING ONLY 788: shared canonical UTF-8 digest contract.
-- The caller supplies exact canonical bytes only as an echo. The server
-- reconstructs those bytes from protected evidence before hashing.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-788-canonical-historical-digest',0));
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
DO $guard$
BEGIN
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830184000,787_cycle7_contract_version_repair)' OR to_regprocedure('public.submit_pdc_historical_reconciliation_782_base(jsonb)') IS NULL OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260830185000') THEN RAISE EXCEPTION 'PDC_788_CURRENT_HEAD_GUARD_FAILED' USING errcode='55000'; END IF;
END $guard$;
DO $boundary_guard$
DECLARE d text;
BEGIN
 SELECT pg_get_functiondef('public.pdc_historical_782_boundary_snapshot()'::regprocedure) INTO d;
 IF position('vehicle_parts_updates' in d)=0 OR position('workshop_bookings' in d)=0 OR position('workshop_booking_assignments' in d)=0 OR position('pdc_sublet_bookings' in d)=0 OR position('pdc_sublet_booking_instances' in d)=0 OR position('pdc_qc_operation_completions_379' in d)=0 OR position('pdc_pmb_stoppage_receipts_422' in d)=0 OR position('monitored_mailboxes' in d)=0 OR position('pdc_email_monitor_current_head_compatibility_controls_766' in d)=0 OR position('pdc_email_monitor_pilot' in d)=0 OR position('pdc_email_monitor_status' in d)=0 OR position('pdc_qc_salesperson_update_outbox_399' in d)=0 OR position('pdc_rft_transport_email_outbox_734' in d)=0 OR position('pdc_sublet_email_update_receipts' in d)=0 THEN RAISE EXCEPTION 'PDC_788_PROTECTED_SNAPSHOT_INCOMPLETE' USING errcode='55000'; END IF;
 IF NOT EXISTS(SELECT 1 FROM pg_class WHERE oid='public.pdc_historical_provider_observations_778'::regclass AND relrowsecurity AND relforcerowsecurity) OR NOT EXISTS(SELECT 1 FROM pg_class WHERE oid='public.pdc_historical_reconciliation_778_receipts'::regclass AND relrowsecurity AND relforcerowsecurity) OR NOT EXISTS(SELECT 1 FROM pg_class WHERE oid='public.pdc_historical_job_card_attachments_782'::regclass AND relrowsecurity AND relforcerowsecurity) THEN RAISE EXCEPTION 'PDC_788_HISTORICAL_RLS_NOT_FORCED' USING errcode='55000'; END IF;
 IF NOT EXISTS(SELECT 1 FROM pg_index i WHERE i.indrelid='public.pdc_historical_provider_observations_778'::regclass AND i.indisunique AND pg_get_indexdef(i.indexrelid) LIKE '%(intake_id, attachment_id)%') OR NOT EXISTS(SELECT 1 FROM pg_index i WHERE i.indrelid='public.pdc_historical_provider_observations_778'::regclass AND i.indisunique AND pg_get_indexdef(i.indexrelid) LIKE '%(observation_sha256)%') THEN RAISE EXCEPTION 'PDC_788_OBSERVATION_UNIQUENESS_MISSING' USING errcode='55000'; END IF;
END $boundary_guard$;
CREATE OR REPLACE FUNCTION public.pdc_historical_canonical_field_788(p_name text,p_value text)
RETURNS text LANGUAGE plpgsql IMMUTABLE SECURITY DEFINER
SET search_path=pg_catalog
AS $field$
BEGIN
 IF p_name IS NULL THEN RAISE EXCEPTION 'PDC_788_NULL_FIELD_NAME' USING errcode='22023'; END IF;
 RETURN octet_length(convert_to(p_name,'UTF8'))::text||':'||p_name||'='||CASE WHEN p_value IS NULL THEN '-1:' ELSE octet_length(convert_to(p_value,'UTF8'))::text||':'||p_value END;
END $field$;
REVOKE ALL ON FUNCTION public.pdc_historical_canonical_field_788(text,text) FROM public,anon,authenticated,service_role,pdc_email_monitor;
GRANT EXECUTE ON FUNCTION public.pdc_historical_canonical_field_788(text,text) TO postgres;

CREATE OR REPLACE FUNCTION public.pdc_historical_canonical_request_788(p_request jsonb,p_actor uuid,p_actor_email text,p_runtime jsonb)
RETURNS text LANGUAGE plpgsql SECURITY DEFINER
SET search_path=pg_catalog,public,auth,extensions
AS $request$
DECLARE o text:=''; s jsonb:=p_request->'source_metadata'; a jsonb:=p_request->'authentication'; r record; i integer:=0; key text;
BEGIN
 o:=o||public.pdc_historical_canonical_field_788('contract_version','788.1')||public.pdc_historical_canonical_field_788('manifest_sha256',p_request->>'manifest_sha256')||public.pdc_historical_canonical_field_788('manifest_uidvalidity',p_request->>'manifest_uidvalidity')||public.pdc_historical_canonical_field_788('manifest_high_water_uid',p_request->>'manifest_high_water_uid')||public.pdc_historical_canonical_field_788('manifest_uid_count',p_request->>'manifest_uid_count')||public.pdc_historical_canonical_field_788('actor_id',p_actor::text)||public.pdc_historical_canonical_field_788('actor_email',lower(btrim(p_actor_email)))||public.pdc_historical_canonical_field_788('gateway_instance_id',p_request->>'gateway_instance_id')||public.pdc_historical_canonical_field_788('release_name',p_request->>'release_name')||public.pdc_historical_canonical_field_788('release_source_sha',p_request->>'release_source_sha')||public.pdc_historical_canonical_field_788('release_manifest_sha256',p_request->>'release_manifest_sha256')||public.pdc_historical_canonical_field_788('runtime_activation_ready',p_runtime->>'activation_ready')||public.pdc_historical_canonical_field_788('runtime_writer_active',p_runtime->>'writer_active')||public.pdc_historical_canonical_field_788('runtime_task_enabled',p_runtime->>'task_enabled')||public.pdc_historical_canonical_field_788('runtime_mailbox_contacted',p_runtime->>'mailbox_contacted')||public.pdc_historical_canonical_field_788('runtime_production_writes',p_runtime->>'production_writes')||public.pdc_historical_canonical_field_788('provider_uid',p_request->>'provider_uid')||public.pdc_historical_canonical_field_788('parent_source_hash',p_request->>'parent_source_hash')||public.pdc_historical_canonical_field_788('sender_email',p_request->>'sender_email')||public.pdc_historical_canonical_field_788('stock_number',p_request->>'stock_number')||public.pdc_historical_canonical_field_788('action_type',p_request->>'action_type')||public.pdc_historical_canonical_field_788('evidence_hash',p_request->>'evidence_hash')||public.pdc_historical_canonical_field_788('subject',p_request->>'subject')||public.pdc_historical_canonical_field_788('summary',p_request->>'summary')||public.pdc_historical_canonical_field_788('observations_jsonb',(p_request->'observations')::text);
 FOREACH key IN ARRAY ARRAY['dkim_aligned','dmarc_aligned','gmail_authentication_results','sender_domain','spf_aligned'] LOOP o:=o||public.pdc_historical_canonical_field_788('authentication.'||key,a->>key); END LOOP;
 FOR r IN SELECT value::text AS value,ordinality FROM jsonb_array_elements_text(s->'attachment_names') WITH ORDINALITY AS x(value,ordinality) ORDER BY ordinality LOOP i:=i+1; o:=o||public.pdc_historical_canonical_field_788('source.attachment_names['||i||']',r.value); END LOOP;
 FOREACH key IN ARRAY ARRAY['graph_message_id','internet_message_id','parsed_text','provider_authserv_id','raw_body','received_at','recipient_mailbox','sender_name','uid','uidvalidity'] LOOP o:=o||public.pdc_historical_canonical_field_788('source.'||key,s->>key); END LOOP;
 i:=0; FOR r IN SELECT value AS item,ordinality FROM jsonb_array_elements(p_request->'attachment_manifest') WITH ORDINALITY AS x(value,ordinality) ORDER BY ordinality LOOP i:=i+1; FOREACH key IN ARRAY ARRAY['attachment_kind','content_type','filename','ordinal','sha256','size'] LOOP o:=o||public.pdc_historical_canonical_field_788('attachment['||i||'].'||key,r.item->>key); END LOOP; END LOOP;
 i:=0; FOR r IN SELECT value AS item,ordinality FROM jsonb_array_elements(p_request->'job_card_children') WITH ORDINALITY AS x(value,ordinality) ORDER BY ordinality LOOP i:=i+1; FOREACH key IN ARRAY ARRAY['attachment_hash','attachment_kind','attachment_ordinal','extraction_hash'] LOOP o:=o||public.pdc_historical_canonical_field_788('child['||i||'].'||key,r.item->>key); END LOOP; o:=o||public.pdc_historical_canonical_field_788('child['||i||'].extraction_jsonb',(r.item->'extraction')::text); END LOOP;
 RETURN o;
END $request$;
REVOKE ALL ON FUNCTION public.pdc_historical_canonical_request_788(jsonb,uuid,text,jsonb) FROM public,anon,authenticated,service_role,pdc_email_monitor;
GRANT EXECUTE ON FUNCTION public.pdc_historical_canonical_request_788(jsonb,uuid,text,jsonb) TO postgres;

CREATE OR REPLACE FUNCTION public.pdc_historical_canonical_observation_788(p_request jsonb,p_runtime jsonb,p_authorization_id uuid,p_intake_id uuid,p_attachment_id uuid,p_ordinal integer,p_kind text,p_hash text,p_request_sha text)
RETURNS text LANGUAGE plpgsql SECURITY DEFINER
SET search_path=pg_catalog,public,auth,extensions
AS $observation$
DECLARE o text:=''; a jsonb:=p_request->'authentication'; key text;
BEGIN
 o:=o||public.pdc_historical_canonical_field_788('contract_version','788.1')||public.pdc_historical_canonical_field_788('request_sha256',p_request_sha)||public.pdc_historical_canonical_field_788('authorization_id',p_authorization_id::text)||public.pdc_historical_canonical_field_788('intake_id',p_intake_id::text)||public.pdc_historical_canonical_field_788('attachment_id',p_attachment_id::text)||public.pdc_historical_canonical_field_788('provider_uid',p_request->>'provider_uid')||public.pdc_historical_canonical_field_788('parent_source_hash',p_request->>'parent_source_hash')||public.pdc_historical_canonical_field_788('manifest_sha256',p_request->>'manifest_sha256')||public.pdc_historical_canonical_field_788('attachment_ordinal',p_ordinal::text)||public.pdc_historical_canonical_field_788('attachment_kind',p_kind)||public.pdc_historical_canonical_field_788('attachment_source_hash',p_hash)||public.pdc_historical_canonical_field_788('provider_message_id',p_request->'source_metadata'->>'internet_message_id')||public.pdc_historical_canonical_field_788('provider_authserv_id',p_request->'source_metadata'->>'provider_authserv_id')||public.pdc_historical_canonical_field_788('actor_id',auth.uid()::text)||public.pdc_historical_canonical_field_788('actor_email',lower(btrim(auth.jwt()->>'email')))||public.pdc_historical_canonical_field_788('gateway_instance_id',p_request->>'gateway_instance_id')||public.pdc_historical_canonical_field_788('release_name',p_request->>'release_name')||public.pdc_historical_canonical_field_788('release_source_sha',p_request->>'release_source_sha')||public.pdc_historical_canonical_field_788('release_manifest_sha256',p_request->>'release_manifest_sha256')||public.pdc_historical_canonical_field_788('runtime_task_enabled',p_runtime->>'task_enabled')||public.pdc_historical_canonical_field_788('runtime_mailbox_contacted',p_runtime->>'mailbox_contacted')||public.pdc_historical_canonical_field_788('runtime_production_writes',p_runtime->>'production_writes');
 FOREACH key IN ARRAY ARRAY['dkim_aligned','dmarc_aligned','gmail_authentication_results','sender_domain','spf_aligned'] LOOP o:=o||public.pdc_historical_canonical_field_788('authentication.'||key,a->>key); END LOOP;
 RETURN o;
END $observation$;
REVOKE ALL ON FUNCTION public.pdc_historical_canonical_observation_788(jsonb,jsonb,uuid,uuid,uuid,integer,text,text,text) FROM public,anon,authenticated,service_role,pdc_email_monitor;
GRANT EXECUTE ON FUNCTION public.pdc_historical_canonical_observation_788(jsonb,jsonb,uuid,uuid,uuid,integer,text,text,text) TO postgres;
...
SELECT count(*) INTO v_authz_count FROM public.pdc_historical_reconciliation_writer_authorizations_773 e
 ...
 IF NOT EXISTS(SELECT 1 FROM public.pdc_ai_intake_proposals p WHERE p.proposal_id=(v_parent_result->'data'->>'proposal_id')::uuid AND p.observations IS NOT DISTINCT FROM v_request->'observations') THEN RAISE EXCEPTION 'PDC_788_OBSERVATIONS_READBACK_FAILED' USING errcode='55000'; END IF;
...
BEGIN
 ...
 IF position('canonical_request_utf8' in d)=0 OR position('pdc_historical_canonical_request_788' in d)=0 OR position('pdc_historical_canonical_observation_788' in d)=0 OR position('request_sha256=v_request_hash' in d)=0 OR position('observation_sha256=v_observation_sha' in d)=0 OR position('PDC_788_PROTECTED_BOUNDARY_DRIFT' in d)=0 OR position('782.1' in d)>0 OR position('canonical_request_utf8' in w)=0 OR position('pdc_historical_canonical_request_788' in w)=0 OR position('PDC_788_PROTECTED_BOUNDARY_DRIFT' in w)>0 OR has_function_privilege('authenticated','public.submit_pdc_historical_reconciliation_782_base(jsonb)','execute') OR NOT has_function_privilege('authenticated','public.submit_pdc_historical_reconciliation_778(jsonb)','execute') THEN RAISE EXCEPTION 'PDC_788_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
 IF to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL THEN RAISE EXCEPTION 'PDC_788_PRODUCTION_SENTINEL_PRESENT' USING errcode='55000'; END IF;
END $verify$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830185000','788_canonical_historical_digest_contract',ARRAY['Recompute one fixed length-prefixed UTF-8 request representation from protected evidence and reject caller echo drift','Recompute per-occurrence observation digest including request digest, attachment kind/ordinal/hash, manifest, actor and runtime bindings','Verify all captured protected boundary state is unchanged before receipt and preserve private base/authenticated wrapper/forced-RLS controls']);
