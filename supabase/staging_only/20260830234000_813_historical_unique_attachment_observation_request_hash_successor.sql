-- STAGING ONLY 813: unique per-attachment historical observation request hashes.
-- 793 reused the parent request hash for all attachments, violating the existing
-- immutable observation UNIQUE(request_sha256) contract on attachment 2.
-- Derive request_sha256 from the parent request hash and exact attachment ID;
-- preserve parent proposal request hash, evidence, actor, gateway, resolver,
-- atomic rollback, RLS/grants, idempotency, ten conflicts and zero-mailbox state.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-813-historical-observation-request-hash',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830233000,812_historical_full_transaction_zero_mailbox_successor)' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '9965d76b424b91404769dfdd33900c53da077c2c14fcbe4b0fcf3ce44f06bc04' OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_813_CURRENT_HEAD_OR_793_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
DECLARE
 ...
 v_runtime jsonb; v_manifest_canonical jsonb; v_manifest_hash text; v_request_hash text; v_observation_request_hash text; v_observation_sha text; v_extraction_hash text;
...
BEGIN
 ...
 IF owner_name IS DISTINCT FROM 'postgres' OR NOT secdef OR acl IS DISTINCT FROM '{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM 'e8fbaed69090f74a277ea091122ebe6a3ab945f40834a571fef5ca686dd43599' OR position('v_observation_request_hash' in v)=0 OR position('h.request_sha256=v_observation_request_hash' in v)=0 OR position('verify_pdc_monitor_runtime_binding_authenticated_766' in v)>0 OR position('pdc_historical_writer_authorization_809_resolve' in v)=0 THEN RAISE EXCEPTION 'PDC_813_OBSERVATION_REQUEST_HASH_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830234000','813_historical_unique_attachment_observation_request_hash_successor',ARRAY['derive deterministic per-attachment observation request_sha256 from parent request hash and attachment id','preserve parent proposal request hash and exact immutable evidence bindings','preserve atomic rollback, idempotency, RLS/grants, ten conflicts and zero-mailbox containment','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
