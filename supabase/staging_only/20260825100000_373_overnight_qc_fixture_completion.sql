...
BEGIN
 IF current_user<>'postgres' OR session_user<>'postgres'
   ...
   OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='20260825090000' AND name='372_overnight_exact_synthetic_booking_constraint')<>1
   OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' AND version>'20260825090000')
  ...
  RAISE EXCEPTION 'PDC_373_STAGING_HEAD_OR_CONTAINMENT_MISMATCH' USING errcode='55000';
 END IF;
END $pre$;

CREATE FUNCTION public.pdc_hermes_test_complete_qc_fixture_373(
...
BEGIN
 IF has_function_privilege('public','public.pdc_hermes_test_complete_qc_fixture_373(text,uuid,integer,uuid,text)','EXECUTE')
  ...
  RAISE EXCEPTION 'PDC_373_ACL_POSTCONDITION' USING errcode='55000'; END IF;
END $post$;

INSERT INTO supabase_migrations.schema_migrations(version,name,statements)
VALUES('20260825100000','373_overnight_qc_fixture_completion',array[
 'Exact staging head and disabled Monitor/mailbox/writer/notification containment',
 'Exact registry scenarios 012-014 and expected single work-key completion only',
 'Actor/idempotency/version receipts, protected/sibling digests and bounded revisions',
 'Explicit synthetic evidence with no physical-work claim and no notification creation'
]);
COMMIT;
