-- STAGING ONLY 802: repair the exact 800 post-reconciliation
-- idempotency cardinality predicate. The writer is expected to be exactly
-- one active after reconciliation, while the first-call prestate remains zero.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='90s';
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-802-repair-800-idempotency-cardinality',0));
DO $guard$
...
BEGIN
 ...
 OR (SELECT count(*) FROM public.pdc_monitor_672_writer_reconciliation_800 WHERE event_key=encode(extensions.digest(convert_to('pdc-staging-800-672-writer-reconciliation|df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b','UTF8'),'sha256'),'hex') AND event_kind='writer_reconciliation' AND predecessor_head='20260830212000' AND successor_head='20260830213000' AND actor_id='df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b' AND actor_email='sales@broometoyota.com.au' AND role='importer' AND reader_id='df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b' AND writer_id='df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b' AND mailbox_active=false AND controls_enabled=false AND pilot_enabled=false AND automatic_enabled=false AND outbound_email_enabled=false AND task_enabled=false AND mailbox_contacted=false AND uid514_processed=false AND production_writes=false AND performed_by='8a83b715-8d79-4b0e-95b2-02b55da6e8d7' AND performed_by_email='craig.watson@broometoyota.com.au' AND before_reader='{"active": true, "reason": "Craig-authorised replacement PMB Email Monitor identity for staging commissioning", "user_id": "df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b", "granted_at": "2026-08-18T09:10:25.590943+00:00", "granted_by": "8a83b715-8d79-4b0e-95b2-02b55da6e8d7", "revoked_at": null}'::jsonb AND after_reader=before_reader AND before_writer='{"active": false, "reason": "Craig-authorised exact staging Email Monitor activation 752", "user_id": "df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b", "granted_at": "2026-08-28T12:07:55.853171+00:00", "granted_by": "8a83b715-8d79-4b0e-95b2-02b55da6e8d7", "revoked_at": "2026-08-30T13:35:23.042585+00:00"}'::jsonb AND after_writer='{"active": true, "reason": "672 containment writer reconciliation 800: Reconcile exact sole Monitor writer required by 672 after contained mailbox rollback", "user_id": "df7c55d9-6ba0-47f6-ba16-44d6ae2c2a4b", "granted_at": "2026-08-30T14:27:44.175662+00:00", "granted_by": "8a83b715-8d79-4b0e-95b2-02b55da6e8d7", "revoked_at": null}'::jsonb)<>1
 ...
 IF position('WHERE active AND revoked_at IS NULL)<>0' in v_prosrc)=0 THEN RAISE EXCEPTION 'PDC_802_EXPECTED_800_FIRST_CALL_GUARD_MISSING' USING errcode='55000'; END IF;
END $guard$;
