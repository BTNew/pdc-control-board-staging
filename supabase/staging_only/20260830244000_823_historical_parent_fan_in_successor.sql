-- STAGING ONLY 823: exclude only the exact historical parent from fan-in conflict.
-- The verified 822 context identifies the derived child and immutable parent.
-- Exclude that parent only when canonical UID/actor match and 773 revalidates;
-- preserve every unrelated pending conflict and ordinary proposal behavior.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-823-historical-parent-fan-in',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830243000,822_historical_derived_proposal_context_successor)' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '9c2cfe3d1effc0083ecd0b0ed0917e5019d823918470c1e6e44ea613bcc7c2ee' OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_823_CURRENT_HEAD_OR_PRE310_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
 ...
 IF encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'c230762a533f3af4327cc250f5b0729212b3ce79ec52d528491f90adfdd37881' OR owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' THEN RAISE EXCEPTION 'PDC_823_HISTORICAL_PARENT_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830244000','823_historical_parent_fan_in_successor',ARRAY['exclude only context-matched exact historical parent from generic same-Stock conflict scan','revalidate actor canonical UID parent hash provider UID sender authentication and Stock via 773','preserve all unrelated conflicts normal fan-in identity evidence atomic rollback idempotency and RLS/grants','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
