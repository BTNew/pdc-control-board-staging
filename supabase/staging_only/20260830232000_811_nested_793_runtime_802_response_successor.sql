-- STAGING ONLY 811: correct the nested 793 runtime response contract.
-- The first 802 containment guard remains unchanged. Replace only the later
-- v_runtime legacy 766 call with the same authenticated 802/672 contained
-- adapter so zero-mailbox STAGING remains valid. Preserve all response checks.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-811-nested-793-runtime-802',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR v_head IS DISTINCT FROM '(20260830231000,810_pre796_historical_authorization_renewal_resolver_successor)' OR owner_name IS DISTINCT FROM 'postgres' OR NOT secdef OR acl IS DISTINCT FROM '{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '666a8d0dc172e0ca5de52c25867dbcc7121dba1b668063a2b6de95291301fd0e' OR 5<>5 OR 0<>0 OR 0<>0 THEN RAISE EXCEPTION 'PDC_811_CURRENT_HEAD_OR_793_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
 SELECT p.proowner::regrole::text,p.prosecdef,p.proacl::text,p.prosrc INTO owner_name,secdef,acl,v FROM pg_proc p WHERE p.oid='public.submit_pdc_historical_reconciliation_793_proposal_review_successor(jsonb)'::regprocedure;
 IF owner_name IS DISTINCT FROM 'postgres' OR NOT secdef OR acl IS DISTINCT FROM '{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '20ee8098061146d720cc63ad9e93fae2bcb6ba11f6d4f13b9268dd69bb42edab' OR (length(v)-length(replace(v,'verify_pdc_historical_runtime_binding_authenticated_802','')))/length('verify_pdc_historical_runtime_binding_authenticated_802')<>2 OR position('verify_pdc_monitor_runtime_binding_authenticated_766' in v)>0 OR position('v_runtime->>''actor_id''' in v)=0 OR position('v_runtime->>''actor_email''' in v)=0 OR position('v_runtime->>''task_enabled''' in v)=0 OR position('v_runtime->>''mailbox_contacted''' in v)=0 OR position('v_runtime->>''production_writes''' in v)=0 THEN RAISE EXCEPTION 'PDC_811_793_RUNTIME_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830232000','811_nested_793_runtime_802_response_successor',ARRAY['replace only the later nested 793 legacy 766 runtime call with authenticated 802/672 contained authorization','preserve correct actor/task/mailbox/production response checks and fail-closed behavior','preserve five renewed rows and ten material proposal tuple conflicts','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
