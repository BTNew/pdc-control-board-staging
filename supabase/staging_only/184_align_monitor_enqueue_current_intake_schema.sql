-- Staging-only remediation: align durable queue enqueue with current intake schema.
begin;
do $guard$ begin if not exists(select 1 from public.pdc_staging_environment_sentinel where singleton and project_ref='cdsmnqxtyyoeoznmbidd') or not exists(select 1 from supabase_migrations.schema_migrations where version='183') or exists(select 1 from supabase_migrations.schema_migrations where version='184') then raise exception 'PDC_184_GUARD_MISMATCH';end if;end $guard$;
...
revoke all on function public.enqueue_pdc_email_intake(jsonb,jsonb) from public,anon,authenticated,service_role;grant execute on function public.enqueue_pdc_email_intake(jsonb,jsonb) to authenticated;insert into supabase_migrations.schema_migrations(version,name,statements) values('184','align_monitor_enqueue_current_intake_schema',array['Use received enum state and queue_attempts for durable intake rows']);commit;
