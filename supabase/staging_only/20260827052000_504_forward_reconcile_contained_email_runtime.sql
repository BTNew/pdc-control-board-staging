...
-- Bind the successor to the authoritative live 503 ledger row and function
-- bodies.  The predecessor name is read from the live row, not guessed; its
-- canonical row hash and both function source hashes are retained in the
-- append-only successor event below.
DO $guard$
DECLARE
...
BEGIN
  IF current_user<>'postgres'
     ...
     OR to_regprocedure('public.reconcile_pdc_monitor_contained_binding_504(uuid,text,text,text,text,text)') IS NOT NULL
     OR to_regprocedure('public.verify_pdc_monitor_contained_binding_504(text,text,text,text,text)') IS NOT NULL
  ...
  SELECT count(*), max(version::integer) INTO v_ledger_count, v_head
  FROM supabase_migrations.schema_migrations
  WHERE version~'^[0-9]+$';
  IF v_ledger_count=0 OR v_head<>503
     OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations
               WHERE version~'^[0-9]+$' AND version::integer>503)
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='503')<>1
  THEN
    RAISE EXCEPTION 'PDC_504_EXACT_LEDGER_HEAD_REQUIRED' USING errcode='55000';
  END IF;

  SELECT version,name,statements INTO v_version,v_name,v_statements
  FROM supabase_migrations.schema_migrations WHERE version='503';
  IF v_version<>'503' OR v_name IS NULL OR btrim(v_name)=''
     OR v_statements IS NULL
  THEN
    RAISE EXCEPTION 'PDC_504_PREDECESSOR_LEDGER_ROW_INVALID' USING errcode='55000';
  END IF;

  SELECT count(*) INTO v_provision_count
  FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
  WHERE p.oid='public.provision_pdc_monitor_contained_binding_503(uuid,text,text,text,text)'::regprocedure
    AND n.nspname='public' AND p.prokind='f' AND p.provolatile='v' AND p.prosecdef
  ...
  IF v_provision_count<>1 OR v_verify_count<>1
     ...
     OR position('pdc-monitor-staging-sales-uid509-v1' in coalesce(v_provision_source,''))=0
     OR position('pdc-monitor-staging-m502-2026.08.44' in coalesce(v_provision_source,''))=0
     OR position('37a1fc0d83e0aa311cfb40b8c1804b9840922ea9' in coalesce(v_provision_source,''))=0
     OR position('5b84745badb9f7bf90690ae82196960ad51a19489c3c5c841b1a2019f42f67ca' in coalesce(v_provision_source,''))=0
  THEN
    RAISE EXCEPTION 'PDC_504_PREDECESSOR_FUNCTION_MARKER_MISMATCH' USING errcode='55000';
  END IF;
END
$guard$;
...
CREATE TABLE public.pdc_monitor_contained_binding_reconciliations_504(
  ...
  predecessor_ledger_version text NOT NULL CHECK(predecessor_ledger_version='503'),
  predecessor_ledger_name text NOT NULL,
  predecessor_ledger_sha256 text NOT NULL CHECK(predecessor_ledger_sha256~'^[a-f0-9]{64}$'),
  predecessor_provision_function_sha256 text NOT NULL CHECK(predecessor_provision_function_sha256~'^[a-f0-9]{64}$'),
  predecessor_verify_function_sha256 text NOT NULL CHECK(predecessor_verify_function_sha256~'^[a-f0-9]{64}$'),
  predecessor_markers jsonb NOT NULL,
  ...
  predecessor_binding_sha256 text NOT NULL CHECK(predecessor_binding_sha256~'^[a-f0-9]{64}$'),
  predecessor_operational boolean NOT NULL CHECK(NOT predecessor_operational),
  predecessor_activation_ready boolean NOT NULL CHECK(NOT predecessor_activation_ready),
  predecessor_writer_active boolean NOT NULL CHECK(NOT predecessor_writer_active),
  predecessor_planner_commissioned boolean NOT NULL CHECK(NOT predecessor_planner_commissioned),
  predecessor_production_writes boolean NOT NULL CHECK(NOT predecessor_production_writes),
  ...
  rollback_contract text NOT NULL CHECK(rollback_contract='transaction rollback only; predecessor 503 remains unchanged'),
...
BEGIN
  ...
  IF (SELECT count(*) FROM supabase_migrations.schema_migrations
      WHERE version~'^[0-9]+$' AND version::integer>503)<>0
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='503')<>1
  THEN
    RAISE EXCEPTION 'PDC_504_EXACT_LEDGER_HEAD_REQUIRED' USING errcode='55000';
  END IF;
  ...
  FROM supabase_migrations.schema_migrations WHERE version='503';
  v_ledger_hash:=encode(extensions.digest(convert_to(concat_ws('|','503',v_ledger_name,
  ...
  INSERT INTO public.pdc_monitor_contained_binding_reconciliations_504(
    event_kind,actor_id,gateway_instance_id,release_name,source_sha,manifest_sha256,
  ...
  VALUES(
    ...
    p_manifest_sha256,p_archive_sha256,503,'contained','503',v_ledger_name,
...
END
$reconcile$;
REVOKE ALL ON FUNCTION public.reconcile_pdc_monitor_contained_binding_504(uuid,text,text,text,text,text) FROM public,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.reconcile_pdc_monitor_contained_binding_504(uuid,text,text,text,text,text) TO authenticated;
...
BEGIN
  ...
  THEN
    RETURN jsonb_build_object('ok',false,'code','contained_reviewed_pair_mismatch',
      'mode','contained','operational',false,'activation_ready',false,
      'writer_active',false,'planner_commissioned',false,'production_writes',false);
  END IF;
  ...
  RETURN jsonb_build_object('ok',true,'code','pdc_monitor_contained_binding_reconciled_504',
    ...
    'planner_commissioned',v_event.planner_commissioned,'production_writes',v_event.production_writes,
    'rollback_available',true,'rollback_contract',v_event.rollback_contract);
END
$verify$;
REVOKE ALL ON FUNCTION public.verify_pdc_monitor_contained_binding_504(text,text,text,text,text) FROM public,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.verify_pdc_monitor_contained_binding_504(text,text,text,text,text) TO authenticated;
...
BEGIN
  IF (SELECT relrowsecurity AND relforcerowsecurity FROM pg_class
     ...
     OR NOT has_function_privilege('authenticated','public.reconcile_pdc_monitor_contained_binding_504(uuid,text,text,text,text,text)','EXECUTE')
     OR NOT has_function_privilege('authenticated','public.verify_pdc_monitor_contained_binding_504(text,text,text,text,text)','EXECUTE')
     OR NOT has_function_privilege('authenticated','public.get_pdc_monitor_contained_binding_504()','EXECUTE')
     OR has_function_privilege('anon','public.reconcile_pdc_monitor_contained_binding_504(uuid,text,text,text,text,text)','EXECUTE')
     OR has_function_privilege('service_role','public.reconcile_pdc_monitor_contained_binding_504(uuid,text,text,text,text,text)','EXECUTE')
  THEN
...
VALUES('504','504_forward_reconcile_contained_email_runtime',ARRAY[
  'Require the exact staging project, numeric migration head 503 and installed contained migration-503 predecessor RPCs',
  'Capture the live predecessor ledger name/row hash, exact function source hashes and marker evidence before any successor write',
  'Append the reviewed source/manifest/archive pair without rewriting the applied 503 singleton or its history',
  'Preserve the exact non-human sales actor, gateway, release, contained mode and fail-closed non-operational readiness',
  'Require administrator identity, actor role identity, no active writer/mailbox/automatic actions and exact predecessor proof',
  'Expose authenticated verification and one-time idempotent replay while denying direct table DML and keeping reconciliation history immutable',
  'Use transaction rollback only; the predecessor 503 binding remains unchanged and no monitor, planner, scheduler, email or vehicle write is enabled',
  'Retain the staging sentinel and leave Production untouched'
]);
