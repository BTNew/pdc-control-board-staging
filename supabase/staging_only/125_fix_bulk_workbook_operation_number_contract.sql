-- Staging-only migration 125: align deterministic OP001..OP100 payload numbers with Preview validation.
begin;
set local lock_timeout='5s';
set local statement_timeout='60s';
set local idle_in_transaction_session_timeout='60s';
select pg_advisory_xact_lock(hashtextextended('pdc-staging-migration-125-bulk-workbook-op-number',0));
do $guard$
begin
  if to_regclass('public.pdc_staging_environment_sentinel') is null
     or not exists(select 1 from public.pdc_staging_environment_sentinel where singleton and project_ref='cdsmnqxtyyoeoznmbidd')
     or to_regclass('public.pdc_production_environment_sentinel') is not null then
    raise exception 'PDC_BULK_125_STAGING_SENTINEL_MISMATCH';
  end if;
  if not exists(select 1 from supabase_migrations.schema_migrations where version='124' and name='bulk_jc_stock_workbook_contract') then
    raise exception 'PDC_BULK_125_PREDECESSOR_124_IDENTITY_MISMATCH';
  end if;
  if exists(select 1 from supabase_migrations.schema_migrations where version='125') then
    raise exception 'PDC_BULK_125_VERSION_CONFLICT';
  end if;
end;
$guard$;
...
values('125','fix_bulk_workbook_operation_number_contract',array['staging-only Preview accepts the adapter deterministic OP001 through OP100 operation-number contract']);
commit;
