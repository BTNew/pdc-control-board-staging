-- Staging-only migration 096: exact Stock number makes Navision authoritative.
--
-- For authenticated email/PDF intake, one unique current Navision Stock/Batch match
-- takes precedence over conflicting extracted VIN/customer/vehicle details. All
-- Navision-owned identity and display fields are refreshed from that backend row;
-- email/PDF evidence contributes required work only. Multiple current Stock matches
-- remain ambiguous and fail closed. Operational workflow state and bookings remain
...
begin
  ...
  if not exists (
       select 1 from supabase_migrations.schema_migrations where version='095'
     )
    ...
    raise exception 'PDC_MIGRATION_096_DEPENDENCY_MISSING';
  end if;
end;
$guard$;
...
begin
  ...
  -- A supplied Stock number is the primary vehicle key. One unique current
  -- Navision Stock match wins even when PDF-extracted VIN/details disagree.
  -- The conflicting extracted values remain in immutable source evidence/audit,
  -- but never become canonical vehicle identity. Duplicate Stock remains unsafe.
  if v_stock is not null then
    if cardinality(v_nav_stock_ids)>1 then
      return public.navision_backend_response(false,'multiple_current_navision_stock_matches',jsonb_build_object(
    ...
    elsif cardinality(v_nav_vin_ids)>0 then
      return public.navision_backend_response(false,'navision_stock_not_found_vin_points_elsewhere',jsonb_build_object(
        'stock_matches',0,'vin_matches',cardinality(v_nav_vin_ids)));
    end if;
  else
    if cardinality(v_nav_vin_ids)>1 then
      return public.navision_backend_response(false,'navision_identity_conflict',jsonb_build_object(
        'stock_matches',0,'vin_matches',cardinality(v_nav_vin_ids)));
    elsif cardinality(v_nav_vin_ids)=1 then
  ...
  -- Apply the same Stock authority to an existing canonical vehicle. If one
  -- operational row owns the Stock, refresh that row from Navision rather than
  -- following an extracted VIN to a different vehicle.
  if v_stock is not null then
    if cardinality(v_operational_stock_ids)>1 then
      return public.navision_backend_response(false,'multiple_operational_stock_matches',jsonb_build_object(
        'stock_matches',cardinality(v_operational_stock_ids),'vin_matches',cardinality(v_operational_vin_ids)));
    elsif cardinality(v_operational_stock_ids)=1 then
      v_vehicle_id:=v_operational_stock_ids[1];
    end if;
  else
    if cardinality(v_operational_vin_ids)>1 then
      return public.navision_backend_response(false,'operational_identity_conflict',jsonb_build_object(
        'stock_matches',0,'vin_matches',cardinality(v_operational_vin_ids)));
    elsif cardinality(v_operational_vin_ids)=1 then
  ...
  if v_record.id is not null then
    ...
    -- Never retain a conflicting PDF VIN after a Stock match. Some Navision rows
    -- carry a frame-derived identifier rather than a valid 17-character VIN; in
    -- that case canonical VIN is null while the raw value remains in Navision.
    v_vin:=case
      when public.is_valid_vehicle_vin(v_record.normalized_data->>'vin')
      then nullif(public.normalize_vehicle_vin(v_record.normalized_data->>'vin'),'')
      else null
    end;
    ...
    v_nav_salesperson:=coalesce(public.navision_original_column_value(v_record.normalized_data,'Salesperson'),nullif(btrim(v_record.normalized_data->>'salesperson'),''),nullif(btrim(v_record.normalized_data->>'consultant'),''),nullif(btrim(v_record.normalized_data->>'owner'),''));
  ...
  if v_vehicle_id is null then
    ...
    insert into public.vehicles(
      ...
      customer_name,vehicle_description,model,salesperson_reference,registration,lifecycle_state,visible_on_board,
      ...
      v_vin,
      v_order_number,v_job_card,v_customer,v_vehicle_description,v_vehicle_description,v_nav_salesperson,v_registration,
    ...
    update public.vehicles set
      ...
      vin=v_vin,
      toyota_order_number=case when v_record.id is not null then v_order_number else coalesce(v_order_number,toyota_order_number) end,
      job_card_number=case when v_record.id is not null then v_nav_job else coalesce(v_job_card,job_card_number) end,
      customer_name=case when v_record.id is not null then v_nav_customer else coalesce(v_customer,customer_name) end,
      vehicle_description=case when v_record.id is not null then v_nav_vehicle else coalesce(v_vehicle_description,vehicle_description) end,
      model=case when v_record.id is not null then v_nav_vehicle else coalesce(v_vehicle_description,model) end,
      salesperson_reference=case when v_record.id is not null then v_nav_salesperson else salesperson_reference end,
      registration=case when v_record.id is not null then v_nav_registration else coalesce(v_registration,registration) end,
      eta_to_kewdale=case when v_record.id is not null then v_eta else coalesce(v_eta,eta_to_kewdale) end,
  ...
  values(case when v_before_vehicle is null then 'insert'::public.audit_action else 'update'::public.audit_action end,
    ...
    jsonb_build_object('source','pdc_navision_stock_authority_096','source_hash',v_source_hash,
      'evidence_hash',v_evidence_hash,'identity_source',v_identity_source,
      'extracted_vin',v_extracted_vin,'navision_vin',v_record.normalized_data->>'vin',
      'extracted_vin_overridden',v_record.id is not null and v_extracted_vin is distinct from v_vin,
      'no_booking',true));
  ...
  if v_record.id is not null then
    ...
    if v_inserted=1 then
      ...
      values('board_activate',v_record.id,v_navision_revision,jsonb_build_object(
        'activation_source','approved_email_build','contract','pdc_navision_stock_authority_096',
  ...
  insert into public.pdc_authenticated_email_import_receipts(
    ...
    v_sender,p_source_received_at,v_stock,v_extracted_vin,v_record.id,v_vehicle_id,
  ...
  if found and v_proposal.status='pending' then
    ...
    insert into public.pdc_ai_intake_history(
      ...
      jsonb_build_object('automatic_contract','pdc_navision_stock_authority_096',
        'vehicle_id',v_vehicle_id,'identity_source',v_identity_source,
        'extracted_vin',v_extracted_vin,'canonical_vin',v_vin,
        'extracted_vin_overridden',v_record.id is not null and v_extracted_vin is distinct from v_vin,
...
comment on function public.import_pdc_authenticated_vehicle_email(text,text,text,text,text,jsonb,timestamptz,text,jsonb,jsonb) is
  'Staging-only authenticated email import (096): one unique current Stock match makes Navision authoritative for canonical identity/details despite PDF identity disagreements; ambiguous Stock fails closed; PDF work/evidence is retained; manual workflow state is preserved; creates no bookings.';
