-- STAGING ONLY 828: resolve aggregate replay before terminal proposal gate.
-- A successful child transaction terminalizes the parent proposal. On exact
-- replay, the existing aggregate receipt is authoritative and must be returned
-- before the unchanged terminal-proposal conflict check.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-828-historical-replay-order',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830248000,827_historical_operation_hours_successor)' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '0ab2a18e16ec30608908763ee7bce54222a20f292e852bf78a1d8f343d9d9cd0' OR position('historical_proposal_terminal_conflict' in lower(v))=0 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_828_CURRENT_HEAD_OR_REPLAY_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
v_request_hash:=encode(extensions.digest(convert_to(v_canonical_request,'UTF8'),'sha256'),'hex');
 ...
 SELECT count(*) INTO v_authz_count
  FROM public.pdc_historical_writer_authorization_809_resolve(p_request->>'manifest_sha256',v_uid,v_parent,v_sender,v_auth,v_stock,p_request->>'evidence_hash') r;
 IF v_authz_count<>1 THEN RETURN jsonb_build_object('ok',false,'code','pdc_778_exact_authorization_failed'); END IF;
 SELECT * INTO v_authz
  FROM public.pdc_historical_writer_authorization_809_resolve(p_request->>'manifest_sha256',v_uid,v_parent,v_sender,v_auth,v_stock,p_request->>'evidence_hash');
...
BEGIN
 SELECT p.prosrc,p.proowner::regrole::text,p.prosecdef,p.proacl::text INTO v,owner_name,secdef,acl FROM pg_proc p WHERE p.oid='public.submit_pdc_historical_reconciliation_778_pre796(jsonb)'::regprocedure;
 IF encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'5934c4334abc2db5a272d8bb7972e7078c3f273c56227235e59fdfc98f589544' OR owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' THEN RAISE EXCEPTION 'PDC_828_HISTORICAL_REPLAY_ORDER_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830249000','828_historical_replay_order_successor',ARRAY['resolve exact aggregate receipt replay before terminal proposal conflict','preserve hash mismatch terminal tuple and payload conflicts fail-closed','preserve atomic rollback idempotency identity evidence RLS grants ten conflicts and zero mailbox containment','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
