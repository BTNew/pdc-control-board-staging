-- STAGING ONLY 765: exact authenticated claim floor successor.
-- Changes only the live 732 floor predicate from 639 to the authoritative 640
-- recovery floor. Production, mailbox flags, UID514 and task state are guarded.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='180s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-765-authenticated-claim-floor-640',0));
...
BEGIN
  IF current_user<>'postgres' OR session_user<>'postgres'
     ...
     OR (SELECT version FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1)<>'20260830030000'
     OR (SELECT name FROM supabase_migrations.schema_migrations WHERE version='20260830030000')<>'exact_manual_reimport_manifest_order_alias_fix'
     OR to_regprocedure('public.claim_pdc_email_intake_authenticated_exact_732(integer,text)') IS NULL
     OR (SELECT encode(extensions.digest(convert_to(p.prosrc,'UTF8'),'sha256'),'hex') FROM pg_proc p WHERE p.oid='public.claim_pdc_email_intake_authenticated_exact_732(integer,text)'::regprocedure)<>'4912b9bb8dcb60af9c33cba9d7b46d154734e83288104a6fce28b972cef6f4c9'
     OR (SELECT minimum_uid FROM public.pdc_email_monitor_pilot WHERE singleton)<>640
     ...
     OR (SELECT enabled OR automatic_rule_application OR automatic_authenticated_jobcards OR outbound_email_enabled FROM public.pdc_email_monitor_pilot WHERE singleton)
  THEN RAISE EXCEPTION 'PDC_765_EXACT_732_FLOOR_PRESTATE_MISMATCH' USING errcode='55000';
  END IF;
END
$guard$;
...
BEGIN
  ...
  THEN RAISE EXCEPTION 'PDC_732_EXACT_ACTOR_RUNTIME_UNAUTHORIZED' USING errcode='42501';
  END IF;
  ...
  RETURN jsonb_build_object('ok',true,'items',v_rows,'count',jsonb_array_length(v_rows),'claim_scope','authenticated-exact-732-floor-640','source_scope','server-side-provider-uid-and-source-hash');
END
$claim$;
...
VALUES('20260830040000','765_authenticated_exact_claim_floor_640_successor',ARRAY[
  'Guard the exact live 20260830030000 predecessor and authoritative monitor floor 640',
  'Replace only the authenticated exact claim floor predicate 639 with 640',
  'Preserve exact actor, mailbox, UID514 exclusion, source-hash, lock, retry and idempotency guards',
  'Preserve authenticated-only execute and deny anon, service_role and pdc_email_monitor'
]);
COMMIT;
