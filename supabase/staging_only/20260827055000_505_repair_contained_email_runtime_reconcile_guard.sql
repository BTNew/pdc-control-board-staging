-- STAGING ONLY 505: repair the applied 504 reconcile poststate guard.
-- Forward-only repair; no applied migration row is rewritten and no
-- operational runtime is enabled.
BEGIN;
SET LOCAL lock_timeout='10s';
SET LOCAL statement_timeout='120s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-505-contained-email-runtime-reconcile-guard',0));
...
BEGIN
  IF current_user<>'postgres' OR session_user<>'postgres'
     ...
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' AND version>'20260827054000')<>0
     ...
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='20260827055000')<>0
  THEN RAISE EXCEPTION 'PDC_505_STAGING_PREDECESSOR_OR_COLLISION_MISMATCH' USING errcode='55000'; END IF;
END
$guard$;
...
BEGIN
  ...
  THEN RAISE EXCEPTION 'PDC_505_RECONCILE_FUNCTION_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END
$post$;

INSERT INTO supabase_migrations.schema_migrations(version,name,statements)
VALUES('20260827055000','505_repair_contained_email_runtime_reconcile_guard',ARRAY[
  'Require the exact applied timestamped 504 ledger row and reject later timestamped drift',
  'Allow the authenticated 504 reconcile RPC to observe its own applied 504 poststate while retaining exact 503 predecessor binding proof',
