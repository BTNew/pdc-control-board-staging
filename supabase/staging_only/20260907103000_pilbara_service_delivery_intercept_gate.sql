-- STAGING ONLY: allow the scoped Pilbara activation to link an unlinked record before delivery reconciliation.
BEGIN;
SET LOCAL lock_timeout='30s';
SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-pilbara-service-delivery-intercept-gate',0));
...
BEGIN
  IF current_user<>'postgres' OR session_user<>'postgres'
     ...
     OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1)
        IS DISTINCT FROM '(20260907102000,pilbara_service_scoped_activation_gate)'
     OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260907103000')
  THEN RAISE EXCEPTION 'PDC_20260907103000_STAGING_PREDECESSOR_OR_SCOPE_GUARD_FAILED' USING ERRCODE='55000';
  END IF;
END
$guard$;
...
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES(
 '20260907103000','pilbara_service_delivery_intercept_gate',ARRAY[
  'preserve exact delivered-at-dealer reconciliation for every existing path',
  'allow only an unlinked approved-email activation by the scoped runtime viewer to establish its canonical vehicle before delivery interception'
 ]);
