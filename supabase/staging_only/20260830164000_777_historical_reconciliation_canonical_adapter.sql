-- STAGING ONLY 777 canonical historical reconciliation adapter.
-- Existing pdc_ai_intake_history remains the parent audit authority.
-- Append-only successor to the observed live head 20260830162000.
-- This is a bounded frozen-manifest adapter, never a mailbox worker.
begin;
...
begin
  if current_user<>'postgres' or session_user<>'postgres'
     ...
     or not exists(select 1 from supabase_migrations.schema_migrations where version='20260830162000' and name='777_historical_observation_binding_adapter_repair2')
     ...
     or to_regprocedure('public.submit_pdc_historical_observation_777(jsonb)') is null
     ...
     or to_regprocedure('public.read_pdc_jobcard_attachment_import_receipt(uuid)') is null
     or to_regprocedure('public.verify_pdc_monitor_runtime_binding_authenticated_766(text,text,text,text,text,text,text)') is null
     or exists(select 1 from supabase_migrations.schema_migrations where version='20260830164000') then
    raise exception 'PDC_777_1640_PREDECESSOR_OR_CANONICAL_CONTRACT_MISMATCH' using errcode='55000';
  end if;
end
$guard$;

-- A successful or partial message result consumes one exact authorization. The
-- row is insert-only and is created in the same transaction as all canonical
-- work, so failed child subtransactions remain retryable and cannot leave a
-- half-consumed authorization behind.
create table public.pdc_historical_observation_777_claims(
  ...
  release_source_sha text not null check(release_source_sha='e850c319989d98b45b95a28aa815d78e2c2e3a4b'),
  release_manifest_sha256 text not null check(release_manifest_sha256='d48b49f6598a99fbef99fc4f0d0ab36b8b47576b8ff7cd8ecd2cb64d6cfed58d'),
  manifest_sha256 text not null check(manifest_sha256='aa9e2451645b3fc51eba68c422b5eaf6f146ed18596a94ce8560c55b80729018'),
  manifest_uidvalidity integer not null check(manifest_uidvalidity=1),
  manifest_high_water_uid integer not null check(manifest_high_water_uid=685),
  manifest_uid_count integer not null check(manifest_uid_count=669),
  ...
  receipt_id uuid not null references public.pdc_historical_observation_777_receipts(receipt_id) on delete restrict,
  unique(authorization_id),
  unique(actor_id,provider_uid,parent_source_hash),
  unique(request_sha256)
);
alter table public.pdc_historical_observation_777_claims enable row level security;
alter table public.pdc_historical_observation_777_claims force row level security;
revoke all on table public.pdc_historical_observation_777_claims from public,anon,authenticated,service_role,pdc_email_monitor;
create function public.pdc_historical_observation_777_claim_immutable()
returns trigger language plpgsql security definer set search_path=pg_catalog,public as $immutable$
begin
  raise exception 'PDC_777_CLAIM_IMMUTABLE' using errcode='55000';
end
$immutable$;
revoke all on function public.pdc_historical_observation_777_claim_immutable() from public,anon,authenticated,service_role,pdc_email_monitor;
create trigger pdc_historical_observation_777_claims_immutable
before update or delete on public.pdc_historical_observation_777_claims
for each row execute function public.pdc_historical_observation_777_claim_immutable();

-- The seven-argument gate is also used by the canonical 159 child importer.
-- It accepts only the exact frozen parent tuple or a server-derived child tuple
-- tied to that parent and its exact attachment hash.
...
insert into supabase_migrations.schema_migrations(version,name,statements) values('20260830164000','777_historical_reconciliation_canonical_adapter',array[
 'Guard the observed 20260830162000 predecessor, staging sentinel, exact current Monitor actor/gateway/release binding and frozen UIDVALIDITY 1/high-water 685/669-message manifest',
 'Rehydrate only the exact frozen message and attachment siblings into staging canonical intake/observation rows without mailbox contact; reject old completed mail, sender ambiguity, source collisions, excluded UID 1:197 and Stock 13056899',
 'Route the parent through submit_pdc_ai_intake_observation_pre135 and each genuine Job Card through import_pdc_jobcard_attachment_canonical with ordinal/hash sibling isolation; isolate ambiguous siblings without cross-vehicle leakage',
 'Consume each authorization once through immutable forced-RLS claims and receipts with exact replay conflict, expiry, audit, no-booking, no-completion and no-location boundaries',
 'Preserve automatic Monitor containment, outbound email disabled, task boundary, prior migrations and Production exclusion'
]);

do $verify$
begin
 if to_regclass('public.pdc_production_environment_sentinel') is not null
    or (select count(*) from public.pdc_historical_reconciliation_writer_authorizations_773 where provider_uid='1:197' or stock_number='13056899')<>0
   ...
   raise exception 'PDC_777_1640_POSTCONDITION_FAILED' using errcode='55000';
 end if;
end
$verify$;
