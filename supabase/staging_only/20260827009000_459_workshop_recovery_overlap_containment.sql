-- STAGING ONLY 459: one historical overlap cannot take every Workshop station offline.
BEGIN;SET LOCAL lock_timeout='10s';SET LOCAL statement_timeout='120s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-459-workshop-recovery-overlap-containment',0));
DO $pre$ BEGIN
 ...
 OR NOT EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260827008000' AND name='458_all_inbox_action_queue_runtime')
 OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' AND version>'20260827008000')
 OR encode(extensions.digest(convert_to(pg_get_functiondef('public.recover_overdue_planned_workshop_bookings(text,timestamptz)'::regprocedure),'UTF8'),'sha256'),'hex')<>'af2d3d7986bbf6029eb2372e288c94a956b6569600e63d22aead369b5b950964'
 THEN RAISE EXCEPTION 'PDC_459_STAGING_HEAD_OR_FUNCTION_MISMATCH' USING errcode='55000';END IF;
END $pre$;
...
DECLARE
 ...
 v_start timestamptz:=date_trunc('minute',v_as_of);v_increment integer;v_bay record;v_stage text;v_repack jsonb;
 v_moved jsonb:='[]'::jsonb;v_skipped jsonb:='[]'::jsonb;v_count integer:=0;v_skipped_count integer:=0;v_error text;
...
BEGIN
 ...
 FOR v_bay IN SELECT DISTINCT b.bay_id FROM public.workshop_bookings b WHERE b.bay_id IS NOT NULL AND b.deleted_at IS NULL AND b.status::text='planned' AND b.scheduled_start_at<v_as_of ORDER BY b.bay_id LOOP
  ...
  EXCEPTION WHEN SQLSTATE '22023' THEN
   v_error:=SQLERRM;
   IF v_error NOT LIKE '%bay_overlap%' AND v_error NOT LIKE '%fixed_booking_conflict%' THEN RAISE;END IF;
   v_skipped_count:=v_skipped_count+1;v_skipped:=v_skipped||jsonb_build_array(jsonb_build_object('bay_id',v_bay.bay_id,'reason',CASE WHEN v_error LIKE '%fixed_booking_conflict%' THEN 'fixed_booking_conflict' ELSE 'bay_overlap' END));
   CONTINUE;
  END;
...
DO $post$ BEGIN
 IF pg_get_functiondef('public.recover_overdue_planned_workshop_bookings(text,timestamptz)'::regprocedure) NOT LIKE '%overdue_planned_recovered_with_conflicts%'
 OR pg_get_functiondef('public.recover_overdue_planned_workshop_bookings(text,timestamptz)'::regprocedure) NOT LIKE '%EXCEPTION WHEN SQLSTATE ''22023''%'
 THEN RAISE EXCEPTION 'PDC_459_POSTCONDITION_FAILED' USING errcode='55000';END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260827009000','459_workshop_recovery_overlap_containment',ARRAY['Overdue recovery skips only bays whose unchanged history conflicts, instead of failing every station snapshot','Other validation and security errors still raise; successful bays retain receipted future-only recovery','Production untouched']);
