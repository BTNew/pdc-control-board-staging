-- STAGING ONLY 451: Parts ordering must preserve notifications, not require an empty table.
BEGIN;
SET LOCAL lock_timeout='10s';
SET LOCAL statement_timeout='120s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-451-parts-order-notification-delta',0));
...
BEGIN
  ...
  IF current_user<>'postgres' OR session_user<>'postgres'
    ...
    OR v_head IS DISTINCT FROM '20260827000000'
    OR to_regprocedure('public.mark_pdc_parts_ordered_377(uuid,integer,uuid)') IS NULL
    OR NOT public.pdc_monitor_staging_guard() THEN
    RAISE EXCEPTION 'PDC_451_STAGING_HEAD_OR_DEPENDENCY_MISMATCH' USING errcode='55000';
  END IF;
END $pre$;

CREATE OR REPLACE FUNCTION public.mark_pdc_parts_ordered_377(p_vehicle_id uuid,p_expected_version integer,p_idempotency_key uuid)
...
BEGIN
 ...
 IF FOUND THEN
  ...
  IF NOT public.pdc_monitor_staging_guard() OR NOT coalesce(v_parts.parts_ordered,false)
  ...
  RETURN jsonb_set(v_receipt.response,'{replay}','true'::jsonb,false)
    ||jsonb_build_object('replay_containment_verified',true,'current_notification_count',(SELECT count(*) FROM public.vehicle_notifications));
 END IF;
 ...
 IF NOT public.pdc_monitor_staging_guard() THEN RAISE EXCEPTION 'PDC_377_RUNTIME_CONTAINMENT_MISMATCH' USING errcode='55000'; END IF;
 ...
 IF NOT coalesce(v_parts.parts_ordered,false) OR v_parts.worst_eta IS DISTINCT FROM v_eta OR v_vehicle.version<>p_expected_version+1
   OR (SELECT count(*) FROM public.vehicle_notifications)<>v_notifications THEN RAISE EXCEPTION 'PDC_377_ORDERED_READBACK_FAILED' USING errcode='55000'; END IF;
 ...
 v_response:=jsonb_build_object('ok',true,'code','parts_ordered','replay',false,'receipt_id',v_id,'request_sha256',v_sha,'vehicle_id',p_vehicle_id,
  ...
  'vehicle',to_jsonb(v_vehicle),'parts_update',to_jsonb(v_parts),'notification_count_before',v_notifications,
  'notification_count_after',(SELECT count(*) FROM public.vehicle_notifications),'notification_delta',0);
...
DECLARE d text;
BEGIN
 d:=pg_get_functiondef('public.mark_pdc_parts_ordered_377(uuid,integer,uuid)'::regprocedure);
 IF d ~ 'v_notifications\s*<>\s*0'
   OR d ~ 'vehicle_notifications\)\s*<>\s*0'
   OR d !~ 'vehicle_notifications\)\s*<>\s*v_notifications'
   OR position('notification_count_before' in d)=0
  ...
  RAISE EXCEPTION 'PDC_451_POSTCONDITION_FAILED' USING errcode='55000';
 END IF;
END $post$;

INSERT INTO supabase_migrations.schema_migrations(version,name,statements)
VALUES('20260827001000','451_parts_order_notification_delta',ARRAY[
 'Parts Mark ordered no longer requires the global notification table to be empty',
 'The mutation records notification count before and after and fails closed unless its own notification delta is exactly zero',
 'Existing immutable receipts remain replayable while unrelated later notifications do not invalidate their authoritative Parts readback'
]);
