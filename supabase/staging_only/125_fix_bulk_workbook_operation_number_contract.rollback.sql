-- Guarded rollback for staging-only migration 125.
begin;
...
begin
  if to_regclass('public.pdc_staging_environment_sentinel') is null
    ...
    raise exception 'PDC_BULK_125_ROLLBACK_STAGING_SENTINEL_MISMATCH';
  end if;
  if exists(select 1 from public.pdc_bulk_workbook_authorizations)
     or exists(select 1 from public.pdc_bulk_workbook_previews)
     or exists(select 1 from public.pdc_bulk_workbook_apply_receipts) then
    raise exception 'PDC_BULK_125_ROLLBACK_ACTIVITY_PRESENT_RECOVERY_REQUIRED';
  end if;
end;
$guard$;
...
delete from supabase_migrations.schema_migrations where version='125' and name='fix_bulk_workbook_operation_number_contract';
commit;
