-- STAGING ONLY 810: route pre796 authorization reads through the append-only
-- 809 renewal resolver. Preserve immutable 773 history and all fail-closed
-- evidence, role, actor, UID, Stock, replay and conflict semantics.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-810-pre796-renewal-resolver',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR v_head IS DISTINCT FROM '(20260830230000,809_historical_writer_authorization_renewal_successor)' OR to_regclass('public.pdc_historical_reconciliation_writer_authorizations_809') IS NULL OR owner_name IS DISTINCT FROM 'postgres' OR NOT secdef OR acl IS DISTINCT FROM '{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '720109d1320268a4a3e973b3687135ce578882755fa2b8d0b134bc82b4cccc2e' OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_810_CURRENT_HEAD_OR_PREFINAL_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
REVOKE ALL ON FUNCTION public.submit_pdc_historical_reconciliation_778_pre796(jsonb) FROM public,anon,authenticated,service_role,pdc_email_monitor; GRANT EXECUTE ON FUNCTION public.submit_pdc_historical_reconciliation_778_pre796(jsonb) TO postgres;
DO $post$
DECLARE v text; owner_name text; secdef boolean; acl text;
BEGIN
 SELECT p.proowner::regrole::text,p.prosecdef,p.proacl::text,p.prosrc INTO owner_name,secdef,acl,v FROM pg_proc p WHERE p.oid='public.submit_pdc_historical_reconciliation_778_pre796(jsonb)'::regprocedure;
 IF owner_name IS DISTINCT FROM 'postgres' OR NOT secdef OR acl IS DISTINCT FROM '{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '0ab2a18e16ec30608908763ee7bce54222a20f292e852bf78a1d8f343d9d9cd0' OR position('pdc_historical_writer_authorization_809_resolve' in v)=0 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 THEN RAISE EXCEPTION 'PDC_810_PREFINAL_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830231000','810_pre796_historical_authorization_renewal_resolver_successor',ARRAY['route pre796 authorization reads through exact 809 renewal resolver','preserve immutable 773 authorization IDs and expiry history','preserve ten material tuple conflicts and fail-closed security controls','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
