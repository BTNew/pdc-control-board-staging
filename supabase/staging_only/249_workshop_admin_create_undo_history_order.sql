-- Staging-only forward correction 249: write immutable Create Undo history
-- while the authoritative booking still exists, then delete it and restore
-- the complete prior vehicle state in the same transaction.
begin;
...
begin
 if to_regclass('public.pdc_staging_environment_sentinel') is null
    ...
    or not exists(select 1 from supabase_migrations.schema_migrations where version='248' and name='workshop_admin_create_undo_history_identity')
    or exists(select 1 from supabase_migrations.schema_migrations where version~'^[0-9]+$' and version::numeric>249)
    or exists(select 1 from supabase_migrations.schema_migrations where version='249' and name<>'workshop_admin_create_undo_history_order') then raise exception 'PDC_249_STAGING_OR_LEDGER_MISMATCH' using errcode='55000'; end if;
end $guard$;
...
insert into supabase_migrations.schema_migrations(version,name,statements) values('249','workshop_admin_create_undo_history_order',array['staging-only forward correction: immutable history before Create Undo booking delete']) on conflict(version) do update set name=excluded.name,statements=excluded.statements where supabase_migrations.schema_migrations.name=excluded.name;
do $verify$
declare d text:=pg_get_functiondef('public.undo_administrator_workshop_booking_move(uuid,integer,uuid)'::regprocedure);
begin
 if not exists(select 1 from supabase_migrations.schema_migrations where version='249' and name='workshop_admin_create_undo_history_order') then raise exception 'PDC_249_LEDGER_VERIFY_FAILED'; end if;
 if position('insert into public.workshop_booking_history' in d)=0 or position('delete from public.workshop_bookings' in d)=0 or position('insert into public.workshop_booking_history' in d)>position('delete from public.workshop_bookings' in d) then raise exception 'PDC_249_ORDER_VERIFY_FAILED'; end if;
end $verify$;
commit;
