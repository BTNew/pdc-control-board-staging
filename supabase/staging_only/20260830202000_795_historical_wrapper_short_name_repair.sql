-- STAGING ONLY 795: short-name wrapper parameter successor after 793.
-- PostgreSQL truncates identifiers above 63 bytes; use the exact deployed
-- 793 routine name and repair cloned v_request references to p_request.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-795-historical-wrapper-parameter',0));
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
DO $guard$
BEGIN
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830200000,793_historical_proposal_review_successor)' OR to_regprocedure('public.submit_pdc_historical_reconciliation_793_proposal_review_succes(jsonb)') IS NULL THEN RAISE EXCEPTION 'PDC_795_CURRENT_HEAD_GUARD_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
 ...
 IF position('p_request->' in lower(w))=0 OR position('v_request->' in lower(w))>0 OR position('submit_pdc_historical_reconciliation_793_proposal_review_succes' in lower(w))=0 OR position('historical_proposal_tuple_conflict' in lower(w))=0 OR position('limit 1 for update' in lower(w))=0 OR has_function_privilege('anon','public.submit_pdc_historical_reconciliation_778(jsonb)','execute') OR has_function_privilege('service_role','public.submit_pdc_historical_reconciliation_778(jsonb)','execute') OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL THEN RAISE EXCEPTION 'PDC_795_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $verify$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830202000','795_historical_wrapper_short_name_repair',ARRAY['Use the deployed PostgreSQL-truncated 793 private routine name explicitly','Repair cloned public-wrapper v_request references to p_request without changing proposal or business state','Preserve typed proposal conflicts, row locking, authenticated-only access, replay and staging controls']);
