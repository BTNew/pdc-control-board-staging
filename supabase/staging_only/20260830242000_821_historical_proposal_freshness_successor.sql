-- STAGING ONLY 821: gate shared proposal freshness by exact historical auth.
-- 820 correctly gates the canonical child intake freshness cutoff, but nested
-- auto-activation still applies its ordinary 30-day proposal source cutoff.
-- Preserve that cutoff for ordinary proposals and bypass it only when the
-- proposal's stored parent/UID/sender/auth/Stock tuple passes exact 773 auth.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-821-historical-proposal-freshness',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830241000,820_historical_importer_freshness_successor)' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM 'f735840fca5a249587b55b6a40984f5665e141d037c647da0f256c57bfec84ae' OR position('or v_proposal.source_received_at<clock_timestamp()-interval ''30 days'' then' in v)=0 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_821_CURRENT_HEAD_OR_HELPER_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
 ...
 IF owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'ebb25268ba1c7c09618ae03e2cc1bffd2a7c5282f74c7eda1114ee07e8a23d1f' THEN RAISE EXCEPTION 'PDC_821_HISTORICAL_PROPOSAL_FRESHNESS_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830242000','821_historical_proposal_freshness_successor',ARRAY['preserve ordinary 30-day proposal freshness rejection','allow only exact 773-authorized historical proposal tuples beyond 30 days','preserve activation identity evidence atomic rollback idempotency RLS/grants ten conflicts and zero drift','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
