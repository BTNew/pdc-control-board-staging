-- STAGING ONLY 820: gate the canonical importer freshness cutoff by historical auth.
-- The 819 sender/auth repair reaches the child importer, but frozen historical
-- rows are older than the normal 30-day intake freshness limit. Preserve the
-- normal 30-day rejection for unauthorised/stale imports and exempt only rows
-- already authorized by the exact 773 historical writer contract.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-820-historical-freshness',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830240000,819_historical_importer_auth_sender_final_successor)' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM 'ee743279ed1b3a9d4c46e2a12267ad054dacf59eb9b6eb1b9de216e25922947b' OR position('or v_intake.received_at<clock_timestamp()-interval ''30 days'' then' in v)=0 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_820_CURRENT_HEAD_OR_IMPORTER_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
 ...
 IF owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'20f492b534ec79e46fdb9164a36f5c56d119668baf7810bcc4f5f6c0bfb94f01' THEN RAISE EXCEPTION 'PDC_820_HISTORICAL_FRESHNESS_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830241000','820_historical_importer_freshness_successor',ARRAY['preserve the normal 30-day importer freshness rejection for unauthorised rows','allow only exact 773-authorized historical rows beyond 30 days','preserve sender auth, exact evidence, identity, atomic rollback, idempotency, RLS/grants, ten conflicts and zero drift','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
