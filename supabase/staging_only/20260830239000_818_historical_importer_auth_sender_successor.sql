-- STAGING ONLY 818: bind historical authorization fallback sender from intake.
-- 817 corrected the historical sender comparison but the first fallback still
-- passed v_sender before its later assignment, so exact 773 authorization was
-- evaluated with NULL. Replace only that argument with locked intake sender.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-818-historical-importer-auth-sender',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830238000,817_historical_importer_sender_binding_final_successor)' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '824118bfa90b509cf23eea078cb8f4a78bfe086df0535354e31471564b71040a' OR position('public.pdc_historical_writer_authorized_773(v_parent_hash,h.provider_uid,v_sender,v_auth' in v)=0 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_818_CURRENT_HEAD_OR_IMPORTER_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
 ...
 IF owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'ee743279ed1b3a9d4c46e2a12267ad054dacf59eb9b6eb1b9de216e25922947b' OR position('public.pdc_historical_writer_authorized_773(v_parent_hash,h.provider_uid,v_sender,v_auth' in v)>0 OR position('public.pdc_historical_writer_authorized_773(v_parent_hash,h.provider_uid,lower(coalesce(v_intake.sender_email,'')),v_auth' in v)=0 THEN RAISE EXCEPTION 'PDC_818_HISTORICAL_IMPORTER_AUTH_SENDER_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830239000','818_historical_importer_auth_sender_successor',ARRAY['replace only the remaining pre-assignment historical authorization sender argument with locked intake sender','preserve normal importer and second historical fallback behavior','preserve exact evidence, identity, atomic rollback, idempotency, RLS/grants, ten conflicts and zero drift','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
