-- STAGING ONLY 778 security successor.
-- non_job_card_sibling evidence is retained and never child-imported.
-- Append-only repair after observed live head 20260830170000.
begin;
...
begin
 if current_user<>'postgres' or session_user<>'postgres'
    ...
    or not exists(select 1 from supabase_migrations.schema_migrations where version='20260830170000' and name='778_historical_reconciliation_enqueue_adapter')
    ...
    or exists(select 1 from supabase_migrations.schema_migrations where version='20260830171000') then
   raise exception 'PDC_778_1710_PREDECESSOR_OR_CANONICAL_CONTRACT_MISMATCH' using errcode='55000';
 end if;
end
$guard$;
...
insert into supabase_migrations.schema_migrations(version,name,statements) values('20260830171000','778_historical_reconciliation_security_successor',array[
...
begin
 if to_regclass('public.pdc_production_environment_sentinel') is not null
    ...
    or has_function_privilege('service_role','public.submit_pdc_historical_reconciliation_778(jsonb)','execute') then
  raise exception 'PDC_778_1710_POSTCONDITION_FAILED' using errcode='55000';
 end if;
end
$verify$;
