-- Craig requested applying routing rules to the existing New Vehicles queue.
-- STAGING only. Review projections only: no source/staff/vehicle/workflow rows change.
DO $guard$ BEGIN
 ...
 IF md5(pg_get_functiondef('public.pdc_qc_operation_lines_379(uuid)'::regprocedure)) <> '14769021fb3cba1b89910e42839ddffb' THEN RAISE EXCEPTION 'qc_projection_changed_review_required'; END IF;
END $guard$;
...
DECLARE
 ...
 times numeric[]; effective numeric;
BEGIN
 ...
 IF stage='ELECTRICAL' AND coalesce((p_line->>'estimated_hours')::numeric,0)=0
  ...
  ELSE
   effective:=CASE WHEN cardinality(times)=1 THEN round(times[1],2) ELSE 1.5 END;
   result:=result||jsonb_build_object('estimated_hours',effective,'effective_estimated_hours',effective,
    'hours_provenance',CASE WHEN cardinality(times)=1 THEN 'explicit_description_time' ELSE 'craig_electrical_default_1_5_hours' END);
  END IF;
 END IF;
 RETURN result;
END $function$;
...
AS $function$
 ...
 SELECT coalesce(jsonb_agg(CASE WHEN review.status='pending' AND review_vehicle.visible_on_board IS NOT TRUE AND review_vehicle.deleted_at IS NULL AND review_vehicle.lifecycle_state='active' AND review_vehicle.qc_completed_at IS NULL AND saved.adjustment_id IS NULL AND NOT coalesce(c.completed,false) AND NOT coalesce(r.active,false) AND NOT EXISTS(SELECT 1 FROM public.workshop_bookings wb WHERE wb.vehicle_id=p_vehicle_id AND wb.deleted_at IS NULL) THEN public.pdc_pending_work_category_20260912(jsonb_build_object('line_identity',l.line_identity,'source_kind',l.source_kind,'source_line_id',l.source_line_id,
  ...
  || CASE WHEN ps.operation_id IS NOT NULL THEN jsonb_build_object('source_contract','pilbara_service_open_jobcards_v1','source_evidence_id',ps.raw_evidence_id,'department',ps.department,'original_line_number',ps.original_line_number,'operation_code',coalesce(public.pdc_tune_approved_operation_source_20260912(ps.operation_id)->>'operation_code',(SELECT h.immutable_snapshot->>'operation_code' FROM public.pdc_pilbara_service_operation_history h WHERE h.operation_id=ps.operation_id AND nullif(h.immutable_snapshot->>'operation_code','') IS NOT NULL ORDER BY h.created_at DESC LIMIT 1),ps.operation_code),'proposed_station',ps.proposed_station) ELSE '{}'::jsonb END||jsonb_build_object('source_estimated_hours',coalesce(ps.source_estimated_hours,l.estimated_hours))) ELSE jsonb_build_object('line_identity',l.line_identity,'source_kind',l.source_kind,'source_line_id',l.source_line_id,
  'operation_no',l.operation_no,'description',l.description,'job_card_number',l.job_card_number,'estimated_hours',public.pdc_standard_operation_hours_20260910(l.description,l.estimated_hours),
  'stage_code',l.stage_code,'active',l.active,'completed',coalesce(c.completed,false),'completed_by',c.completed_by,'completed_at',c.completed_at,
  'line_version',coalesce(c.version,0),'rejected',coalesce(r.active,false),'rejection_reason',case when r.active then r.reason else null end,
  'rejected_by',case when r.active then r.rejected_by else null end,'rejected_at',case when r.active then r.rejected_at else null end,
