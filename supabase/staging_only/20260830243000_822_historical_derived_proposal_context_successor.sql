-- STAGING ONLY 822: carry verified historical auth into derived child proposals.
-- The canonical child proposal uses a derived source hash/UID, so the shared
-- activation helper cannot identify its original historical tuple from proposal
-- columns alone. The historical child path sets a transaction-local context
-- only after exact 773 validation; pre310 requires matching derived UID/actor
-- and revalidates the full stored tuple before bypassing 30-day expiry.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-822-historical-derived-context',0));
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
DO $guard$
DECLARE c text; h text;
BEGIN
 SELECT p.prosrc INTO c FROM pg_proc p WHERE p.oid='public.import_pdc_jobcard_attachment_canonical(uuid,uuid,text,text,jsonb,jsonb,jsonb,jsonb)'::regprocedure;
 SELECT p.prosrc INTO h FROM pg_proc p WHERE p.oid='public.pdc_auto_apply_ai_intake_activation_internal_pre310(uuid,uuid,text,boolean)'::regprocedure;
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830242000,821_historical_proposal_freshness_successor)' OR encode(extensions.digest(convert_to(c,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '20f492b534ec79e46fdb9164a36f5c56d119668baf7810bcc4f5f6c0bfb94f01' OR encode(extensions.digest(convert_to(h,'UTF8'),'sha256'),'hex') IS DISTINCT FROM 'ebb25268ba1c7c09618ae03e2cc1bffd2a7c5282f74c7eda1114ee07e8a23d1f' OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_822_CURRENT_HEAD_OR_FUNCTION_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
DO $post$
DECLARE c text; h text; co text; ho text; cs boolean; hs boolean; ca text; ha text;
BEGIN
 SELECT p.prosrc,p.proowner::regrole::text,p.prosecdef,p.proacl::text INTO c,co,cs,ca FROM pg_proc p WHERE p.oid='public.import_pdc_jobcard_attachment_canonical(uuid,uuid,text,text,jsonb,jsonb,jsonb,jsonb)'::regprocedure;
 SELECT p.prosrc,p.proowner::regrole::text,p.prosecdef,p.proacl::text INTO h,ho,hs,ha FROM pg_proc p WHERE p.oid='public.pdc_auto_apply_ai_intake_activation_internal_pre310(uuid,uuid,text,boolean)'::regprocedure;
 IF encode(extensions.digest(convert_to(c,'UTF8'),'sha256'),'hex')<>'4b9c6e3e9daab98b10f28bc965dfb8dab8ae5264c87b12fbadafbde3f0f90513' OR encode(extensions.digest(convert_to(h,'UTF8'),'sha256'),'hex')<>'9c2cfe3d1effc0083ecd0b0ed0917e5019d823918470c1e6e44ea613bcc7c2ee' OR co<>'postgres' OR ho<>'postgres' OR NOT cs OR NOT hs OR ca<>'{postgres=X/postgres}' OR ha<>'{postgres=X/postgres}' THEN RAISE EXCEPTION 'PDC_822_HISTORICAL_CONTEXT_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830243000','822_historical_derived_proposal_context_successor',ARRAY['child sets transaction-local historical context only after exact 773 validation','pre310 bypasses 30-day proposal expiry only for matching derived UID actor and revalidated 773 tuple','ordinary proposal freshness, security, identity, evidence, atomic rollback, idempotency and RLS/grants remain unchanged','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
