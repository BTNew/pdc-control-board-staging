-- STAGING ONLY 827: preserve unknown operation hours in receipt aggregate.
-- Historical 1:134 has valid operation lines whose per-line hours are NULL.
-- Keep those line values NULL and coalesce only the NOT NULL aggregate sum.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-827-operation-hours',0));
...
BEGIN
 ...
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830247000,826_historical_vehicle_freshness_successor)' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '4b9c6e3e9daab98b10f28bc965dfb8dab8ae5264c87b12fbadafbde3f0f90513' OR position('select sum((x->>''estimated_hours'')::numeric) into v_hours_sum' in lower(v))=0 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_writer_authorizations_809 WHERE active AND expires_at>clock_timestamp())<>5 OR (SELECT count(*) FROM public.pdc_historical_reconciliation_778_receipts)<>0 OR (SELECT count(*) FROM public.pdc_historical_provider_observations_778)<>0 OR (SELECT count(*) FROM public.monitored_mailboxes WHERE active)<>0 THEN RAISE EXCEPTION 'PDC_827_CURRENT_HEAD_OR_OPERATION_PRESTATE_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
begin
  ...
  select coalesce(sum((x->>'estimated_hours')::numeric),0) into v_hours_sum from jsonb_array_elements(v_canonical_lines) x;
...
BEGIN
 ...
 IF encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex')<>'ccdd559262b3c8c180f1fb374863c487d3c45d99f161ca904bebbe9d740bdd7d' OR owner_name<>'postgres' OR NOT secdef OR acl<>'{postgres=X/postgres}' THEN RAISE EXCEPTION 'PDC_827_OPERATION_HOURS_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830248000','827_historical_operation_hours_successor',ARRAY['coalesce only the receipt estimated_hours_sum aggregate when all operation-line hours are unknown','preserve NULL per-line estimated_hours and existing validation identity evidence atomic rollback idempotency and RLS/grants','preserve exact ten conflict boundary zero-mailbox containment and no prohibited operations','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
