-- STAGING ONLY 507: stabilize the 504 reconcile lineage fence.
-- Forward-only repair; no applied migration row is rewritten and no
-- operational runtime is enabled.
BEGIN;
SET LOCAL lock_timeout='10s';
SET LOCAL statement_timeout='120s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-507-contained-email-runtime-reconcile-lineage',0));
...
BEGIN
  IF current_user<>'postgres' OR session_user<>'postgres'
     ...
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' AND version>'20260827056000')<>0
     OR to_regprocedure('public.reconcile_pdc_monitor_contained_binding_504(uuid,text,text,text,text,text,text)') IS NULL
     OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='20260827057000')<>0
  THEN RAISE EXCEPTION 'PDC_507_STAGING_PREDECESSOR_OR_COLLISION_MISMATCH' USING errcode='55000'; END IF;
END
$guard$;
...
BEGIN
  ...
  IF (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='20260827056000')<>1
...
BEGIN
  ...
  THEN RAISE EXCEPTION 'PDC_507_RECONCILE_FUNCTION_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END
$post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements)
VALUES('20260827057000','507_stabilize_contained_email_runtime_reconcile_lineage',ARRAY[
  'Require the exact applied timestamped 506 repair and immutable 503 through 504/505/506 lineage rows',
  'Prevent the reconcile RPC from rejecting its own current repair head while retaining exact lineage checks',
