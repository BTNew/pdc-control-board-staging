-- STAGING ONLY 792: deterministic vehicle identity readback successor after 791.
-- Replace invalid aggregate-over-UUID identity selection with count plus an
-- ordered single-row read, preserving duplicate identity rejection.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-792-historical-vehicle-identity',0));
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
DO $guard$
BEGIN
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard() OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830192000,791_historical_manifest_compatibility_successor)' OR to_regprocedure('public.submit_pdc_historical_reconciliation_791_manifest_compatibility_base(jsonb)') IS NULL OR to_regprocedure('public.submit_pdc_historical_reconciliation_792_vehicle_identity_successor(jsonb)') IS NOT NULL THEN RAISE EXCEPTION 'PDC_792_CURRENT_HEAD_GUARD_FAILED' USING errcode='55000'; END IF;
END $guard$;
CREATE OR REPLACE FUNCTION public.submit_pdc_historical_reconciliation_792_vehicle_identity_successor(p_request jsonb)
...
REVOKE ALL ON FUNCTION public.submit_pdc_historical_reconciliation_792_vehicle_identity_successor(jsonb) FROM public,anon,authenticated,service_role,pdc_email_monitor;
GRANT EXECUTE ON FUNCTION public.submit_pdc_historical_reconciliation_792_vehicle_identity_successor(jsonb) TO postgres;
...
v_request_hash:=encode(extensions.digest(convert_to(v_canonical_request,'UTF8'),'sha256'),'hex');
 ...
 v_result:=public.submit_pdc_historical_reconciliation_792_vehicle_identity_successor(p_request);
...
BEGIN
 SELECT pg_get_functiondef('public.submit_pdc_historical_reconciliation_792_vehicle_identity_successor(jsonb)'::regprocedure) INTO d;
 SELECT pg_get_functiondef('public.submit_pdc_historical_reconciliation_778(jsonb)'::regprocedure) INTO w;
 IF position('select count(*) into v_attachment_count' in lower(d))=0 OR position('order by v.id limit 1' in lower(d))=0 OR position('max(v.id)' in lower(d))>0 OR position('submit_pdc_historical_reconciliation_792_vehicle_identity_successor' in lower(w))=0 OR position('historical_proposal_tuple_conflict' in lower(w))=0 OR position('limit 1 for update' in lower(w))=0 OR has_function_privilege('authenticated','public.submit_pdc_historical_reconciliation_792_vehicle_identity_successor(jsonb)','execute') OR has_function_privilege('anon','public.submit_pdc_historical_reconciliation_778(jsonb)','execute') OR has_function_privilege('service_role','public.submit_pdc_historical_reconciliation_778(jsonb)','execute') OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL THEN RAISE EXCEPTION 'PDC_792_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $verify$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830193000','792_historical_vehicle_identity_successor',ARRAY['Replace invalid UUID max aggregation with deterministic count and ordered identity readback','Preserve duplicate vehicle identity conflict and exact source/proposal/binding controls','Forward the typed 790 wrapper to the 792 private base with no proposal or business-state mutation']);
