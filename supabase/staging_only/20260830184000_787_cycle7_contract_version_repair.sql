-- STAGING ONLY 787: align every historical contract-version assertion.
-- 786 corrected the inserted observation row tag but the copied 782 base still
-- used the 782.1 tag in its digest, observation readback, response and receipt paths.
-- The existing 778 observation and receipt tables both require 778.1.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-787-cycle7-contract-version-repair',0));
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
DO $guard$
BEGIN
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard()
    OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL
    OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830183000,786_cycle7_contract_repair)'
    OR to_regprocedure('public.submit_pdc_historical_reconciliation_782_base(jsonb)') IS NULL
    OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260830184000')
 THEN RAISE EXCEPTION 'PDC_787_CURRENT_HEAD_GUARD_FAILED' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
 ...
 IF position('782.1' in d)>0 OR position('778.1' in d)=0 OR position('v_request_hash,v_observation_sha' in d)=0 OR has_function_privilege('authenticated','public.submit_pdc_historical_reconciliation_782_base(jsonb)','execute') THEN RAISE EXCEPTION 'PDC_787_HISTORICAL_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
 IF has_function_privilege('authenticated','public.get_vehicle_workshop_detail(uuid)','execute') OR has_function_privilege('service_role','public.get_vehicle_workshop_detail(uuid)','execute') OR NOT has_function_privilege('authenticated','public.get_vehicle_workshop_detail_scoped(uuid,text)','execute') OR has_function_privilege('anon','public.get_vehicle_workshop_detail_scoped(uuid,text)','execute') OR has_function_privilege('service_role','public.get_vehicle_workshop_detail_scoped(uuid,text)','execute') THEN RAISE EXCEPTION 'PDC_787_PLANNER_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
 IF to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL THEN RAISE EXCEPTION 'PDC_787_PRODUCTION_SENTINEL_PRESENT' USING errcode='55000'; END IF;
END $verify$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830184000','787_cycle7_contract_version_repair',ARRAY[
 'Align all historical observation, receipt, response and digest contract-version assertions with the immutable 778.1 table contract',
 'Preserve distinct request_sha256 and observation_sha256, required/unique observation digest and private base execution boundary',
 'Preserve dealer-scoped planner wrapper, no direct intake SELECT, replay/old-mail/atomic/no-side-effect and Production/outbound/task controls'
]);
