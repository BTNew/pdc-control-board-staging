-- STAGING ONLY 815: bind historical importer sender from the locked intake row.
-- The 812 historical importer exception reaches the historical observation
-- fallback before its later v_sender assignment, so h.sender_email=v_sender
-- compared against NULL and rejected a valid child. Replace only that one
-- historical fallback comparison; preserve normal active-mailbox behavior,
-- exact 673/809 evidence, atomic rollback, RLS/grants and all conflicts.
BEGIN;
SET LOCAL lock_timeout='15s'; SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-815-historical-importer-sender',0));
...
BEGIN
 ...
 IF owner_name IS DISTINCT FROM 'postgres' OR NOT secdef OR acl IS DISTINCT FROM '{postgres=X/postgres}' OR encode(extensions.digest(convert_to(v,'UTF8'),'sha256'),'hex') IS DISTINCT FROM '824118bfa90b509cf23eea078cb8f4a78bfe086df0535354e31471564b71040a' OR position('h.sender_email=lower(coalesce(v_intake.sender_email,''))' in v)=0 OR position('and h.sender_email=v_sender' in v)>0 THEN RAISE EXCEPTION 'PDC_815_HISTORICAL_IMPORTER_SENDER_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830236000','815_historical_importer_sender_binding_successor',ARRAY['replace only historical fallback sender comparison with the locked intake sender before later v_sender assignment','preserve normal importer, exact evidence, identity, atomic rollback, idempotency, RLS/grants and ten conflicts','no historical Apply outbox mailbox task outbound or Production operation']);
COMMIT;
