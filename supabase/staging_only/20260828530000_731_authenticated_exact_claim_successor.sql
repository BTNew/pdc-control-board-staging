-- STAGING ONLY 731: exact authenticated PMB monitor claim successor.
...
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-731-exact-authenticated-claim',0));
...
BEGIN
  IF current_user<>'postgres' OR session_user<>'postgres'
     ...
     OR to_regprocedure('public.claim_pdc_email_intake_authenticated_exact_731(integer,text)') IS NOT NULL
     OR (SELECT encode(extensions.digest(convert_to(p.prosrc,'UTF8'),'sha256'),'hex') FROM pg_proc p WHERE p.oid='public.claim_pdc_email_intake_batch(integer,text)'::regprocedure)<>'082c5ec909c1b1c1f3023af72c5e66e68358be0a98e225b98515ce4eea6deefb'
  ...
  THEN RAISE EXCEPTION 'PDC_731_EXACT_PREDECESSOR_OR_SCOPE_PRESTATE_MISMATCH' USING errcode='55000';
  END IF;
END
$guard$;

CREATE OR REPLACE FUNCTION public.claim_pdc_email_intake_authenticated_exact_731(p_limit integer,p_gateway_instance_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.claim_pdc_email_intake_authenticated_exact_731(integer,text) FROM public,anon,service_role,pdc_email_monitor;
...
VALUES('20260828530000','731_authenticated_exact_claim_successor',ARRAY[
