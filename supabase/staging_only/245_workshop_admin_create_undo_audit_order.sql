-- Staging-only forward correction 245: preserve immutable create-Undo audit
-- action while physically deleting the newly-created booking.
begin;
...
begin
 if to_regclass('public.pdc_staging_environment_sentinel') is null
    ...
    or not exists(select 1 from supabase_migrations.schema_migrations where version='244' and name='workshop_admin_authority_intent_receipt_undo')
    or exists(select 1 from supabase_migrations.schema_migrations where version~'^[0-9]+$' and version::numeric>245)
    or exists(select 1 from supabase_migrations.schema_migrations where version='245' and name<>'workshop_admin_create_undo_audit_order') then
   raise exception 'PDC_245_STAGING_OR_LEDGER_MISMATCH' using errcode='55000';
 end if;
end $guard$;
...
begin
 ...
 values(case when r.operation_type='create' then null else r.booking_id end,
...
values('245','workshop_admin_create_undo_audit_order',array['staging-only forward correction: create Undo deletes booking and retains immutable history via receipt-bound metadata'])
...
do $verify$
begin
 if not exists(select 1 from supabase_migrations.schema_migrations where version='245' and name='workshop_admin_create_undo_audit_order') then raise exception 'PDC_245_LEDGER_VERIFY_FAILED'; end if;
 if position('case when r.operation_type=''create'' then null else r.booking_id end' in pg_get_functiondef('public.undo_administrator_workshop_booking_move(uuid,integer,uuid)'::regprocedure))=0
    or position('delete from public.workshop_bookings where id=r.booking_id' in pg_get_functiondef('public.undo_administrator_workshop_booking_move(uuid,integer,uuid)'::regprocedure))=0 then
   raise exception 'PDC_245_UNDO_BODY_VERIFY_FAILED';
 end if;
end $verify$;
commit;
