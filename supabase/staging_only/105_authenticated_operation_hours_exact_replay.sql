-- Staging-only migration 105: exact-replay no-op for authenticated job-card hours.
-- Preserves migration 104's typed contract and returns before all DML when every line/hour/work item already matches.
begin;
...
begin
  if to_regclass('public.pdc_staging_environment_sentinel') is null
    ...
    raise exception 'PDC_MIGRATION_104_STAGING_OR_DEPENDENCY_MISMATCH';
  end if;
end;
$guard$;
...
begin
     ...
     or exists(
       ...
       where jsonb_typeof(x)<>'object'
          or (select array_agg(k order by k) from jsonb_object_keys(x) k)
             is distinct from array['description','estimated_hours','operation_no','work_key']::text[]
  ...
  if not exists(
    ...
    return public.navision_backend_response(true,'operation_lines_and_hours_already_imported',jsonb_build_object(
      ...
      'estimated_hours_added',0,'required_work_added',0,'resulting_revision',v_resulting_revision,
      'booking_created',false,'completed_work_reopened',false));
  end if;
  ...
  for v_item in select value from jsonb_array_elements(v_operations) loop
    ...
    if v_line_before.operation_line_id is null then
      insert into public.pdc_authenticated_email_operation_lines(
        import_receipt_id,vehicle_id,source_hash,source_uid,operation_no,work_key,description,estimated_hours,operation_fingerprint
      ) values(v_receipt.receipt_id,v_receipt.vehicle_id,v_source_hash,v_source_uid,
        v_operation_no,v_work_key,v_description,v_estimated_hours,v_fingerprint)
      ...
      values('insert','pdc_authenticated_email_operation_lines',v_line_after.operation_line_id,v_receipt.vehicle_id,v_actor_id,v_actor_email,null,
        jsonb_build_object('operation_no',v_operation_no,'work_key',v_work_key,'description',v_description,'estimated_hours',v_estimated_hours),
        jsonb_build_object('source','pdc_authenticated_email_operation_hours_105','source_hash',v_source_hash,'no_booking',true));
    elsif v_line_before.estimated_hours is null and v_estimated_hours is not null then
      update public.pdc_authenticated_email_operation_lines set estimated_hours=v_estimated_hours
      ...
      values('update','pdc_authenticated_email_operation_lines',v_line_after.operation_line_id,v_receipt.vehicle_id,v_actor_id,v_actor_email,
        ...
        jsonb_build_object('source','pdc_authenticated_email_operation_hours_105','source_hash',v_source_hash,
          'estimated_hours_added',true,'no_booking',true,'completed_work_reopened',false));
    end if;
  ...
  return public.navision_backend_response(true,
    ...
    jsonb_build_object(
      ...
      'estimated_hours_added',v_hours_added,'required_work_added',v_work_added,
...
comment on function public.import_pdc_authenticated_email_operations_with_hours(text,text,jsonb) is
  'Staging-only enrolled-Viewer typed import of bounded authenticated job-card OP lines with explicit write-once estimated hours; never books or completes work or Parts.';
...
after update of estimated_hours on public.pdc_authenticated_email_operation_lines
...
comment on function public.get_pdc_email_vehicle_location_snapshot() is
  'Staging authenticated-email vehicle snapshot including latest 50 bounded typed operation lines and explicit estimated hours; read-only and no booking authority.';
