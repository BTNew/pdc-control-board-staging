-- Staging-only append fix for Migration 162 authenticated RPC runtime compilation.
-- SQLSTATE 42702 exposed local/column conflicts in lazily compiled PL/pgSQL functions.
-- Local email/id variables are renamed and qualified SQL aliases are resolved as columns.
begin;
do $guard$
begin
 if not public.pdc_monitor_staging_guard() then raise exception 'PDC_163_STAGING_ONLY'; end if;
 if not exists(select 1 from supabase_migrations.schema_migrations where version='162' and name='manager_approved_workbook_canonical_activation')
    or exists(select 1 from supabase_migrations.schema_migrations where version>'162') then raise exception 'PDC_163_PREDECESSOR_MISMATCH'; end if;
end $guard$;
...
declare uid uuid:=auth.uid();actor_email text:=lower(btrim(coalesce(auth.jwt()->>'email','')));x public.pdc_user_roles%rowtype;old jsonb;fresh jsonb;expected_confirmation text;
begin
 ...
 select r.* into x from public.pdc_user_roles r where r.auth_user_id=uid and lower(r.email)=actor_email and r.active and r.account_status='approved' and r.role='administrator';
 ...
 if p_active then
  ...
  values(p_user_id,true,uid,actor_email,clock_timestamp(),null,null,null)
  ...
  update public.pdc_pmb_canonical_manager_authorities set active=false,revoked_by=uid,revoked_by_email=actor_email,revoked_at=clock_timestamp()
 ...
 values('update','pdc_pmb_canonical_manager_authorities',p_user_id,uid,actor_email,old,fresh,
...
create or replace function public.manager_approve_pdc_pmb_canonical_activation(
...
#variable_conflict use_column
declare uid uuid:=auth.uid();actor_email text:=lower(btrim(coalesce(auth.jwt()->>'email','')));pr public.pdc_pmb_workbook_previews%rowtype;
 pair public.pdc_pmb_workbook_pair_reviews%rowtype;c jsonb;old public.pdc_pmb_canonical_manager_approvals%rowtype;h text;result_id uuid;
begin
 if not public.pdc_monitor_staging_guard() or uid is null or actor_email='' or p_confirmation<>'MANAGER APPROVE CANONICAL BOARD ACTIVATION'
 ...
 perform 1 from public.pdc_user_roles r where r.auth_user_id=uid and lower(r.email)=actor_email and r.role='operator'
 ...
 values(p_preview_id,p_pair_id,pr.workbook_sha256,pr.payload_sha256,pr.preview_hash,pair.pair_hash,pair.source_hash,p_expected_backend_record_id,
  p_expected_backend_record_version,p_expected_vehicle_id,p_expected_vehicle_version,p_expected_action,btrim(p_reason),p_confirmation,h,uid,actor_email)
 returning approval_id into result_id;
 return public.navision_backend_response(true,'manager_approval_recorded',jsonb_build_object('approval_id',result_id,'approval_hash',h,'action',p_expected_action));
end $manager$;

create or replace function public.administrator_countersign_pdc_pmb_canonical_activation(
...
#variable_conflict use_column
declare uid uuid:=auth.uid();actor_email text:=lower(btrim(coalesce(auth.jwt()->>'email','')));m public.pdc_pmb_canonical_manager_approvals%rowtype;
 old public.pdc_pmb_canonical_admin_countersignatures%rowtype;c jsonb;h text;result_id uuid;
begin
 if not public.pdc_monitor_staging_guard() or uid is null or actor_email='' or p_confirmation<>'ADMINISTRATOR COUNTERSIGN CANONICAL BOARD ACTIVATION'
...
or nullif(c->>'target_vehicle_id','')::uuid is distinct from m.target_vehicle_id
 ...
 perform 1 from public.pdc_user_roles r where r.auth_user_id=uid and lower(r.email)=actor_email and r.role='administrator'
   and r.active and r.account_status='approved' for share;
 if not found then return public.navision_backend_response(false,'administrator_required');end if;
 ...
 insert into public.pdc_pmb_canonical_admin_countersignatures(manager_approval_id,preview_id,pair_id,manager_approval_hash,reason,confirmation,
  countersignature_hash,countersigned_by,countersigned_email) values(m.approval_id,m.preview_id,m.pair_id,m.approval_hash,btrim(p_reason),p_confirmation,h,uid,actor_email)
 returning countersignature_id into result_id;
 return public.navision_backend_response(true,'administrator_countersignature_recorded',jsonb_build_object('countersignature_id',result_id,'countersignature_hash',h));
end $admin$;

create or replace function public.authorize_pdc_pmb_canonical_activation_apply(
...
#variable_conflict use_column
declare uid uuid:=auth.uid();actor_email text:=lower(btrim(coalesce(auth.jwt()->>'email','')));pr public.pdc_pmb_workbook_previews%rowtype;
 old public.pdc_pmb_canonical_apply_authorizations%rowtype;rev bigint;cnt integer;set_hash text;h text;result_id uuid;
begin
 if not public.pdc_monitor_staging_guard() or uid is null or actor_email='' or p_confirmation<>'AUTHORIZE MANAGER APPROVED CANONICAL ACTIVATIONS'
 ...
 values(pr.preview_id,pr.workbook_sha256,pr.payload_sha256,pr.preview_hash,rev,cnt,set_hash,p_confirmation,h,uid,actor_email,clock_timestamp()+interval '119 minutes')
 returning authorization_id into result_id;
 return public.navision_backend_response(true,'canonical_apply_authorized',jsonb_build_object('authorization_id',result_id,'authorization_hash',h,
 ...
 for m in select m.* from public.pdc_pmb_canonical_manager_approvals m join public.pdc_pmb_workbook_pair_reviews p on p.pair_id=m.pair_id
  ...
  if m.action='reactivate_complete_board_purge' then
   ...
   update public.navision_board_activations set active=true,activation_source='manual',activated_stock_number=r.normalized_data->>'batch',
    activated_at=clock_timestamp(),activated_by=uid,activated_by_email=actor_email,canonical_vehicle_id=v.id,
   ...
   values(r.id,'manual',r.normalized_data->>'batch',uid,actor_email,true) returning * into activation;
...
insert into supabase_migrations.schema_migrations(version,name,statements) values('163','canonical_activation_runtime_ambiguity_fix',array['Resolve authenticated canonical activation PL/pgSQL variable conflicts.']);
