...
BEGIN
 IF current_user<>'postgres' OR session_user<>'postgres'
   ...
   OR public.pdc_acceptance_protected_digest_375() IS DISTINCT FROM jsonb_build_object('rows',1413,'sha256','28476c8fac93aa03707b20a84b4b836b4268c96fa6710bf1238f0f6ebb265f11') THEN
...
DECLARE v_actor uuid:=auth.uid();v_email text:=lower(btrim(coalesce(auth.jwt()->>'email','')));v_line text:=lower(btrim(coalesce(p_line_identity,'')));
 ...
 v_response jsonb;v_id uuid;v_before jsonb;v_work_key text;v_all_complete boolean;
BEGIN
 ...
 IF v_line LIKE 'source:%' THEN
  ...
  SELECT coalesce(a.stage_code,public.workshop_stage_code_for_work_key(ol.work_key)),coalesce(a.active,true) INTO v_stage,v_active
 ...
 ELSE
  ...
  SELECT a.stage_code,a.active INTO v_stage,v_active FROM public.vehicle_workshop_line_adjustments a WHERE a.adjustment_id=v_source_id AND a.vehicle_id=p_vehicle_id AND a.source_kind='manual';
 END IF;
 ...
 SELECT NOT EXISTS(SELECT 1 FROM jsonb_array_elements(public.pdc_qc_operation_lines_379(p_vehicle_id)) l WHERE l->>'stage_code'=v_stage AND coalesce((l->>'active')::boolean,false) AND NOT coalesce((l->>'completed')::boolean,false)) INTO v_all_complete;
...
BEGIN
 IF NEW.qc_completed_at IS NOT NULL AND OLD.qc_completed_at IS NULL AND (
  ...
  OR EXISTS(SELECT 1 FROM jsonb_array_elements(public.pdc_qc_operation_lines_379(NEW.id)) l WHERE coalesce((l->>'active')::boolean,false) AND NOT coalesce((l->>'completed')::boolean,false))
