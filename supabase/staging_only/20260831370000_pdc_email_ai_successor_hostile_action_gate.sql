-- STAGING ONLY: reject hostile action types before any source lookup or dispatch.
BEGIN;
SET LOCAL lock_timeout='10s'; SET LOCAL statement_timeout='180s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-email-ai-successor-hostile-action-gate-20260831370000',0));
DO $guard$
BEGIN
 ...
 OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version='20260831360000' AND name='pdc_email_ai_successor_owner_credential_rotation')<>1
 OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260831370000')
 THEN RAISE EXCEPTION 'PDC_EMAIL_AI_SUCCESSOR_3700_STAGING_PRECONDITION_FAILED' USING errcode='55000'; END IF;
END $guard$;
CREATE OR REPLACE FUNCTION public.apply_pdc_email_ai_transaction_successor__pre_hostile_gate(p_plan jsonb)
...
AS $gate$
BEGIN
 IF jsonb_typeof(p_plan)='object' AND jsonb_typeof(p_plan->'instructions')='array'
    AND EXISTS(SELECT 1 FROM jsonb_array_elements(p_plan->'instructions') x WHERE x->>'action_type' NOT IN('activate_from_navision','location_set','workgroup_requirement_set','operation_upsert','parts_eta_set','parts_ordered','parts_complete','notes_append','job_card_upsert','sublet_booking_upsert','rft_transfer','rft_collect'))
...
END $gate$;
REVOKE ALL ON FUNCTION public.apply_pdc_email_ai_transaction_successor__pre_hostile_gate(jsonb) FROM public,anon,authenticated,service_role;
...
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260831370000','pdc_email_ai_successor_hostile_action_gate',ARRAY['Unknown action types fail closed before source lookup or canonical dispatch','Reviewed successor command contract remains the only canonical dispatcher','Production sentinel and service-role runtime execution remain denied']);
