-- STAGING ONLY 367: correct canonical Sublet-history ordering in wrapper state digests/readback.
BEGIN;
SET LOCAL lock_timeout='10s';
SET LOCAL statement_timeout='120s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-367-overnight-sublet-history-key',0));
...
BEGIN
 IF NOT public.pdc_monitor_staging_guard()
   ...
   OR NOT EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260825030000' AND name='366_overnight_wrapper_postgrest_argument_names')
   OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version>'20260825030000' AND version~'^[0-9]{14}$')
  ...
  RAISE EXCEPTION 'PDC_367_STAGING_HEAD_OR_CONTAINMENT_MISMATCH' USING errcode='55000'; END IF;
END $guard$;
...
BEGIN
 ...
 SELECT jsonb_build_object('ok',true,'run_id',p_run_id,'vehicle_id',p_vehicle_id,
  'vehicles',coalesce(jsonb_agg(jsonb_build_object('scenario_no',r.scenario_no,'scenario_name',r.scenario_name,
   ...
   'sublet_history',coalesce((SELECT jsonb_agg(to_jsonb(h) ORDER BY h.history_id) FROM public.pdc_sublet_booking_instance_history h WHERE h.vehicle_id=v.id),'[]'::jsonb),
...
DO $post$
BEGIN
 IF pg_get_functiondef('public.pdc_hermes_test_apply_365(text,uuid,integer,uuid,integer,uuid,text,jsonb)'::regprocedure) NOT LIKE '%ORDER BY x.history_id%'
   OR pg_get_functiondef('public.read_pdc_hermes_test_mutation_state_365(text,uuid)'::regprocedure) NOT LIKE '%ORDER BY h.history_id%'
   OR pg_get_functiondef('public.pdc_hermes_test_apply_365(text,uuid,integer,uuid,integer,uuid,text,jsonb)'::regprocedure) LIKE '%jsonb_agg(to_jsonb(x) ORDER BY x.id) FROM public.pdc_sublet_booking_instance_history%'
  ...
  RAISE EXCEPTION 'PDC_367_FUNCTION_OR_ACL_POSTCONDITION' USING errcode='55000'; END IF;
END $post$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements)
VALUES('20260825040000','367_overnight_sublet_history_primary_key',array[
 'Exact migration 366 head and staging containment',
 'Use pdc_sublet_booking_instance_history.history_id in failed-action target digests',
 'Use history_id in authoritative synthetic mutation readback without changing ACLs'
]);
...
DO $final$ BEGIN
 IF NOT public.pdc_monitor_staging_guard() OR (SELECT count(*) FROM public.vehicle_notifications)<>0
   OR EXISTS(SELECT 1 FROM public.monitored_mailboxes WHERE active)
   OR EXISTS(SELECT 1 FROM public.pdc_monitor_stage_activation_writers WHERE active AND revoked_at IS NULL) THEN
  RAISE EXCEPTION 'PDC_367_FINAL_CONTAINMENT_MISMATCH' USING errcode='55000'; END IF;
END $final$;
COMMIT;
