-- STAGING ONLY 786: repair the observation contract-version and narrow the old detail RPC.
-- 783 correctly separated request/observation digests but retained the 782
-- version tag in the existing 778 observation table, which is rejected by its
-- immutable constraint. This successor corrects that tag, seals the observation
-- digest field and removes the unscoped browser detail execute grant.
BEGIN;
SET LOCAL lock_timeout='15s';
SET LOCAL statement_timeout='300s';
SELECT pg_advisory_xact_lock(hashtextextended('pdc-staging-786-cycle7-contract-repair',0));
LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
DO $guard$
BEGIN
 IF current_user<>'postgres' OR session_user<>'postgres' OR NOT public.pdc_monitor_staging_guard()
    OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL
    OR (SELECT (version,name)::text FROM supabase_migrations.schema_migrations WHERE version~'^[0-9]{14}$' ORDER BY version::bigint DESC LIMIT 1) IS DISTINCT FROM '(20260830182000,785_narrow_authenticated_contracts)'
    ...
    OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260830183000')
 THEN RAISE EXCEPTION 'PDC_786_CURRENT_HEAD_GUARD_FAILED' USING errcode='55000'; END IF;
END $guard$;
ALTER TABLE public.pdc_historical_provider_observations_778 ALTER COLUMN observation_sha256 SET NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS pdc_historical_observation_sha256_unique_786 ON public.pdc_historical_provider_observations_778(observation_sha256);
...
REVOKE ALL ON FUNCTION public.get_vehicle_workshop_detail(uuid) FROM public,anon,authenticated,service_role;
...
BEGIN
 ...
 IF position('observation_sha256' in lower(d))=0 OR position('v_request_hash,v_observation_sha' in lower(d))=0 OR has_function_privilege('authenticated','public.submit_pdc_historical_reconciliation_782_base(jsonb)','execute') THEN RAISE EXCEPTION 'PDC_786_HISTORICAL_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
 IF has_function_privilege('authenticated','public.get_vehicle_workshop_detail(uuid)','execute') OR has_function_privilege('service_role','public.get_vehicle_workshop_detail(uuid)','execute') OR NOT has_function_privilege('authenticated','public.get_vehicle_workshop_detail_scoped(uuid,text)','execute') OR has_function_privilege('anon','public.get_vehicle_workshop_detail_scoped(uuid,text)','execute') OR has_function_privilege('service_role','public.get_vehicle_workshop_detail_scoped(uuid,text)','execute') THEN RAISE EXCEPTION 'PDC_786_PLANNER_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
 IF NOT EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='pdc_historical_provider_observations_778' AND column_name='observation_sha256' AND is_nullable='NO') OR NOT EXISTS(SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='pdc_historical_observation_sha256_unique_786') OR to_regclass('public.pdc_production_environment_sentinel') IS NOT NULL THEN RAISE EXCEPTION 'PDC_786_SCHEMA_POSTCONDITION_FAILED' USING errcode='55000'; END IF;
END $verify$;
INSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('20260830183000','786_cycle7_contract_repair',ARRAY[
 'Correct the existing 778 observation contract version while retaining separate request_sha256 and observation_sha256 fields',
 'Seal observation_sha256 as required and unique for one observation per immutable attachment',
 'Revoke the unscoped browser get_vehicle_workshop_detail execute grant and retain only the dealer-scoped authenticated wrapper',
 'Preserve forced RLS, historical atomic rollback, exact replay, old-mail, identity, Production and outbound-email controls'
]);
