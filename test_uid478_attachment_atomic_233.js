...
assert(sql.includes("v_canonical_source_hash:=public.pdc_233_length_prefixed_sha256(array[''pdc-attachment-canonical-source'',''233.1'',p_intake_id::text,p_attachment_id::text,v_parent_hash,v_attachment_hash])"), 'canonical identity uses the explicit cross-runtime byte contract while retaining parent evidence');
assert(sql.includes("v_payload:=v_payload||int4send(octet_length(v_bytes))||v_bytes") && sql.includes("convert_to(v_value,'UTF8')"), 'canonical bytes are UTF-8 values with unsigned 32-bit big-endian lengths');
assert(sql.includes("public.pdc_233_length_prefixed_sha256(array['pdc-uid478-message-aggregate','233.1',p_intake_id::text,'1','478']||v_ids::text[])"), 'aggregate uses the same explicit byte contract');
...
assert((sql.match(/coalesce\(v_receipt\.canonical_source_hash,v_receipt\.parent_source_hash\)/g) || []).length >= 4, 'historical immutable receipts read through parent hash fallback');
