...
assert.ok(version, 'app.js must define APP_VERSION');
...
for (const label of ['Parts', 'Tint', 'Bus 4x4', 'Hoist', 'Fitting', 'Fab', 'Elec', 'Tyre', 'Sublet']) {
  assert.ok(app.includes(label), `Missing full stage label: ${label}`);
}
assert.ok(app.includes("const rowOrder = ['parts', 'tint', 'bus4x4', 'hoist', 'fitting', 'fabrication', 'electrical', 'tyre', 'sublet']"), 'Work-status rows must show Parts first, Bus 4x4 third and Sublet last; PIT is not a work-status column');
