...
(async () => {
  ...
  try {
    for (const viewport of [{ width: 360, height: 800 }, { width: 390, height: 844 }, { width: 768, height: 1024 }]) {
      ...
      const file = { name: 'craig-mobile-qc.jpg', mimeType: 'image/jpeg', buffer: Buffer.from([1, 2, 3, 4]) };
      await input.setInputFiles(file);
      await page.locator('.qc-save-feedback.is-error').waitFor();
      assert.ok((await page.locator('.qc-save-feedback.is-error').textContent()).includes('staging photo upload failed'));
      assert.ok((await page.locator('.qc-photo-preview').textContent()).includes('craig-mobile-qc.jpg'));
      ...
      assert.deepStrictEqual(await page.evaluate(() => window.__qcStubCalls[0]), { kind: 'upload', vehicleId: 'd777b071-a2b0-5367-893b-aa83a07fcfce', version: 34, cycleId: '245974d0-2e8f-5215-bd85-3e8e10fe9a0e', name: 'craig-mobile-qc.jpg', type: 'image/jpeg' });
      await page.evaluate(() => window.__resolveQcUpload({ ok: true, code: 'qc_retest_photo_accepted', data: { photo_receipt_id: 'fresh-photo-receipt', sha256: 'a'.repeat(64), original_filename: 'craig-mobile-qc.jpg', byte_length: 4, original_byte_length: 4, image_width: 100, image_height: 100 } }));
      ...
      const calls = await page.evaluate(() => window.__qcStubCalls); assert.strictEqual(calls[1].kind, 'upload'); assert.strictEqual(calls[2].kind, 'finalize'); assert.strictEqual(calls[2].receiptId, 'fresh-photo-receipt'); assert.deepStrictEqual(errors, []); assert.strictEqual(external.length, 0);
