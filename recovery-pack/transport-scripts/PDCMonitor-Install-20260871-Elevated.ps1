...
$Installer=Join-Path $Root 'installer\install_pdc_monitor_successor_20260871.ps1'
...
try {
  $output=@(& $Installer -InstallRoot $InstallRoot -BundleRoot $Bundle -ExpectedManifestSha256 $ExpectedManifest -ExpectedParentManifestSha256 $ExpectedParent -EnableAutomation 2>&1)
  $exit=$LASTEXITCODE
  ...
  if($result.ok -ne $true -or $result.task_enabled -ne $true){ throw 'PDC_MONITOR_071_ELEVATED_INSTALL_ASSERTION_FAILED' }
  $result | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $Receipt -Encoding utf8
  exit 0
} catch {
  [ordered]@{ok=$false;error='PDC_MONITOR_071_ELEVATED_INSTALL_FAILED';task_enabled=$false;task_started=$false;mailbox_contacted=$false;production_contacted=$false;secrets_printed=$false}|ConvertTo-Json -Compress|Set-Content -LiteralPath $Receipt -Encoding utf8
  exit 1
}
