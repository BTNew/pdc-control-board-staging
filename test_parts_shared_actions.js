...
(async () => {
  ...
  ok(/async function markVehiclePartsComplete[\s\S]*?__emailVehicleServerAuthoritative[\s\S]*?markPartsComplete[\s\S]*?receipt-backed/.test(appSource), 'manual Mark Complete refuses local fallback and reconciles receipt-backed state');
