...
assert.strictEqual(vm.runInContext('typeof pdcAuditorBindOperationOrigin',context),'undefined','receipt binder must remain private to gateway closure');
assert.strictEqual(vm.runInContext('pdcAuditorOperationOriginCurrent',context)(proposal,{url:'https://gateway.staging.example/auditor',instance:'gw-staging-1'},'human-admin-token','human-admin-authority'),false,'synthetic receipt must not gain provenance');
...
(async()=>{
  ...
  context.fetch=async(url,options)=>{ request={url,options}; return {ok:true,json:async()=>proposal}; };
  context.app.pdcAuditorPendingOperation=await vm.runInContext("callPdcAuditorOperationGateway('status')",context);
  vm.runInContext('renderPdcAuditorPendingOperation()',context);
  ...
  const undoPending={state:'undo_available',instance_id:'gw-staging-1',run_id:'aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee',run_revision_after:'e'.repeat(64)};
  context.fetch=async(url,options)=>{ request={url,options}; return {ok:true,json:async()=>undoPending}; };
  context.app.pdcAuditorPendingOperation=await vm.runInContext("callPdcAuditorOperationGateway('status')",context);
