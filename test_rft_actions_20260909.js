...
test('boot loads new scripts; no mailto fallback drops the attachment',()=>{const code=fs.readFileSync('pdc-rft-actions.js','utf8');const boot=fs.readFileSync('canonical-entry.js','utf8');assert.match(boot,/pdc-rft-actions\.js/);assert.match(boot,/pdc-rft-actions\.css/);assert.doesNotMatch(code,/mailto:|sendMail|graph\.microsoft|\/send\b/);assert.match(code,/download\(prepared\); \/\/ Single Email button/);assert.match(code,/readRftTransportDraft739/);assert.match(code,/60000/);});
