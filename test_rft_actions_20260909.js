...
const v={__emailVehicleServerAuthoritative:true,__emailVehicleId:'synthetic-vehicle',__emailVehicleVersion:16,pdcQcComplete:true,pdcQcCompleteAt:'2026-09-09',rftConfirmedAt:'2026-09-10',rftTransferredAt:'2026-09-09',pdcLocation:'RFT',lifecycleState:'rft'};
...
test('RFT is a single read-only state icon, not two checked controls',()=>{const html=controls(v,options);assert.equal(ready(v),true);assert.doesNotMatch(html,/<input/);assert.equal((html.match(/<svg /g)||[]).length,4);assert.match(html,/QC signed off/);assert.match(html,/Mark collected/);assert.doesNotMatch(html,/data-rft-draft-key|aria-pressed|[⇩↗☑]/);});
...
test('untrusted or unsigned QC cannot be presented as ready',()=>{assert.equal(ready({...v,__emailVehicleServerAuthoritative:false}),false);assert.equal(ready({...v,pdcQcComplete:false}),false);const html=controls({...v,pdcQcComplete:false},options);assert.match(html,/Awaiting QC/);assert.equal((html.match(/ disabled/g)||[]).length,3);});
...
test('RFT boot uses direct Outlook compose without an email-file download',()=>{const code=fs.readFileSync('pdc-rft-actions.js','utf8');const boot=fs.readFileSync('canonical-entry.js','utf8');assert.match(boot,/pdc-rft-actions\.js\?v=2026\.09\.10\.04/);assert.match(code,/openOutlook\(prepared\); \/\/ Same direct compose/);assert.doesNotMatch(code,/link\.download|createObjectURL|sendMail|graph\.microsoft|\/send\b/);assert.match(code,/readRftTransportDraft739/);assert.match(code,/Copy QC photo/);});
