/* Cross-station vehicle availability. One elapsed hour between jobs. */
(() => {
 'use strict';
 ...
 async function warn(changedSince){
  try{
   ...
   window.alert('Vehicle booking warning — check the next station\n\n'+result.warnings.map(w=>`Stock ${w.stock}: ${w.first_stage} Bay ${w.first_bay} finishes ${label(w.end_at)}; ${w.next_stage} Bay ${w.next_bay} starts ${label(w.next_start)}. ${w.overlap?'Bookings overlap.':'Less than the 1-hour buffer.'}`).join('\n\n')+'\n\nMove the later booking using Best slot or choose a suitable time.');
