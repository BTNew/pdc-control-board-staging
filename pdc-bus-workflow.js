...
(function (root) {
  ...
  function addWeekdays(value, days) {
    if (!validDate(value)) return null;
    const d = new Date(Date.parse(value) + 8 * 3600000);
    while (days > 0) { d.setUTCDate(d.getUTCDate() + 1); if (d.getUTCDay() !== 0 && d.getUTCDay() !== 6) days--; }
    return new Date(d.getTime() - 8 * 3600000).toISOString();
  }
  function planningHints(plan, now = Date.now()) {
    const f = plan?.forecasts || {}, hints = [];
    const two = addWeekdays(f.mechanical_complete, 2), three = addWeekdays(f.mechanical_complete, 3);
    ...
    if (validDate(f.vehicle_ready) && !['requested','booked','passed','not_required'].includes(plan?.pit_status)) {
      const remaining = (Date.parse(f.vehicle_ready) - now) / 3600000;
      hints.push({tone:remaining < 48 ? 'risk' : remaining <= 72 ? 'review' : 'info',code:'pit_notice',text:remaining < 0 ? 'Vehicle-ready forecast has passed. Confirm readiness and the pit inspection plan.' : remaining < 48 ? 'Pit request needs attention: fewer than 48 hours remain before the vehicle-ready forecast.' : remaining <= 72 ? 'Pit request due: contact Ron now for the 48–72 hour notice window.' : `Plan the pit request from ${formatDate(new Date(Date.parse(f.vehicle_ready) - 72 * 3600000).toISOString())}.`});
  ...
  function panelHtml(plan,{identity='',editable=false,busy=false,message='',dirty=false,stale=false,retry=false}={}) {
    ...
    <details class="bus-section" open><summary>Forecasts and downstream planning</summary><div class="bus-fields">${date('forecast_mechanical_complete','Mechanical completion forecast (Perth)',f.mechanical_complete,disabled)}${date('forecast_electrical_complete','Electrical completion forecast (Perth)',f.electrical_complete,disabled)}${date('forecast_vehicle_ready','Vehicle ready for pit — after QA (Perth)',f.vehicle_ready,disabled)}${date('forecast_delivery','Confirmed delivery / transport (Perth)',f.delivery,disabled)}</div><p class="bus-help">The 2–3 working-day buffer skips weekends; check holidays and workshop availability. Pit reminders use elapsed 48–72 hours. Forecast changes require a downstream review and do not move bookings.</p>${plan.downstream_review_required===true?`<label class="bus-checkbox"><input type="checkbox" name="downstream_review_acknowledged" ${disabled}> I reviewed the downstream electrical, QA, pit, rustproofing, wash and delivery arrangements against the saved forecasts.</label><p class="bus-help">Save changed forecasts first, then confirm the downstream review. Existing bookings remain unchanged.</p>`:''}</details>
