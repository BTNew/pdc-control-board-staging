...
- Workshop queue cards have a direct `Schedule` action in addition to drag/drop. Dropping a new queue card into a bay fills the earliest continuous non-overlapping slot at or after the requested time; if that workday is full, the sequence advances to the next Monday-to-Friday workday. When a planned booking is dragged onto an occupied time, the planner asks whether to move that booking to the next open slot in the selected bay rather than dead-ending; it never moves the existing booking or silently cascades work. The search horizon is 260 workdays. The direct form still allows a manual bay/date/time and suggests the first open cross-day slot. Back-to-back bookings are valid, overlaps remain blocked, and selected bookings can be extended with +15m, +30m or +1h controls.
