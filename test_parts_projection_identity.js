...
assert.ok(action.includes('const { service, vehicle: sharedVehicle, vehicleId, expectedVersion } = sharedTarget;') && action.includes('service.markPartsOrdered(vehicleId, expectedVersion)'), 'Mark Ordered uses the projected canonical id/version');
