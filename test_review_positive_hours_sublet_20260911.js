...
const row=()=>({status:'pending',vehicle_id:'vehicle',operations:[{line_identity:'source:1',source_line_id:'1',description:'Supplied kit',estimated_hours:0,stage_code:'SUBLET',active:true,completed:false}]});
test('zero, missing and malformed hours cannot release a review; positive entered hours preserve source',()=>{
  ...
  assert.equal(api.stationGroups(r,{},draft).find(x=>x.code==='SUBLET').hours,0.25);
