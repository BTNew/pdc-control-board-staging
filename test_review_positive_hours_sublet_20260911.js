...
test('Sublet needs no hours, preserves source and responds to station changes',()=>{
 ...
 const r=row();assert.ok(api.problems(r).length);assert.deepEqual(api.problems(r,{'source:1':'SUBLET'}),[]);r.operations[0].stage_code='SUBLET';assert.ok(api.problems(r,{'source:1':'FITTING'}).length);r.operations[0].department='138';assert.ok(api.problems(r,{'source:1':'SUBLET'}).some(issue=>issue.includes('hours')));assert.equal(api.assignmentsFor(r,{'source:1':'SUBLET'})[0].stage_code,'BUS_4X4');
});
