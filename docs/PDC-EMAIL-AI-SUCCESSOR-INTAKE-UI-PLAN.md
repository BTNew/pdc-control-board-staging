...
Status: implementation authorized and in progress; source/live backend contract exists, UI/read projection/identity commission remain to be completed.
...
Fallback: current `.68` Email Monitor repair runtime/task remains untouched and disabled state remains the rollback reference.
...
5. Expand to show complete typed plan, model/prompt/instruction/taxonomy/action versions, every action/RPC, old/requested/result values, blocked/ambiguous instructions, retries/quarantine, immutable receipt references/digests and authoritative readback.
6. Provide Refresh, loading, synchronized-empty, unavailable/error and Realtime revision states. Never expose raw body, attachment bytes, secrets, access tokens or Windows logs.
...
- Every action includes `action_type`, `instruction_id`, `canonical_rpc`, disposition, reason, before/requested/after, expected/actual verification, evidence refs, retry/quarantine fields and action receipt ID/key.
- Transaction details include immutable transaction/source/evidence/plan digests, plan, independent model/prompt/taxonomy/rule/action/transport versions, aggregate disposition, readback and parity.
...
- Existing legacy `pdc-ai-intake-service.js` and `.68` proposal handlers remain present but are not used by the successor panel.
...
B. Red/green rendering: parent/vehicle split, two vehicles under one email, received metadata, UID/attachment summary, before→requested→result, verification badges, details for every action/version/retry/quarantine/readback, safe escaping.
...
D. Red/green responsive/browser: compact phone layout, desktop layout, accessible details/labels, no horizontal overflow, no console/resource errors, no non-staging requests.
...
2. Provision one dedicated authenticated STAGING runtime identity plus versioned planner binding through the protected connector. It must not be Administrator, service-role, direct DML/SQL or browser authority. Keep `.68` fallback untouched.
...
5. Prove exact replay has zero effects and unrelated vehicle remains unchanged. Record Stock, intake UID, typed plan/instruction version, actions, resulting state and readback. State explicitly that Windows made no business classification decision.
