...
- Follow-up screenshot regression for dashboard `20260831_100550_0dcd0a`: before release the authenticated live DOM showed `PradoSubletBooked` and `.pdc-station-sublet.is-required`; after release the vehicle/model column contains no `.incoming-card-sublet`, while exactly one canonical active `pdcSubletBookings` entry renders `.pdc-station-sublet.is-required.is-ordered` with accessible `Sublet booked` semantics. The mapper now derives active count only from canonical booking rows, so a stale redundant count cannot suppress the orange state. No-booking, cancelled, returned and completed cases remain non-orange; completed remains green. Exact Stock `13080534` was used.
