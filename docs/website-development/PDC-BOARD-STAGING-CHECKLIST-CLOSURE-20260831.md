...
- [x] 4. Relevant pages have in-place vehicle/workshop Refresh with no whole-page reload. Live diagnosis identified the revoked unscoped `get_workshop_snapshot` call as the dashboard Refresh failure; the deployed fix skips that call on dashboard while station planners use scoped reads. `test_operational_refresh_class.js` and the full 162-file staging Node suite passed; the deployed asset carries the closure marker and no `location.reload()` path exists.
...
- Follow-up screenshot regression for dashboard `20260831_100550_0dcd0a`: before release the authenticated live DOM showed `PradoSubletBooked` and `.pdc-station-sublet.is-required`; after release the vehicle/model column contains no `.incoming-card-sublet`, while exactly one canonical active `pdcSubletBookings` entry renders `.pdc-station-sublet.is-required.is-ordered` with accessible `Sublet booked` semantics. The mapper now derives active count only from canonical booking rows, so a stale redundant count cannot suppress the orange state; the DTO service cache is explicitly busted in `index.html`. No-booking, cancelled, returned and completed cases remain non-orange; completed remains green. Exact Stock `13080534` was used.
...
- Focused Node regressions: Admin 15h, atomic cascade, refresh coordinator, Stock 13017855, Sublet, Sublet workgroup active/history DOM states, vehicle booking pills, lifecycle history, Parts orange projection, operation removal, RFT/QC/delete and Workshop projection — PASS.
- Full staging JavaScript contract suite: `node --test test_*.js` — 162 passed, 0 failed.
...
- Live authenticated UI: final staging project/administrator session showed Stock `13080534`, Customer Sublet, exact dates, vehicle/model text `Prado` only, orange Sublet workgroup state, completed Parts and no Parts Risk warning; the final deployed asset also contains the canonical active-count and vehicle/Stock/bay booking-navigation fixes. No Production request was observed.
