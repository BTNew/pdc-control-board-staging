...
||| `supabase/staging_only/20260830170000_778_historical_reconciliation_enqueue_adapter.sql`, `20260830171000_778_historical_reconciliation_security_successor.sql` | Staging database / canonical email importer | UUID-free exact 773-derived enqueue, immutable per-attachment evidence, 24-hour expiry, replay protection, old-mail protection, valid-sibling continuation and ambiguous/multi-vehicle fail-closed handling | Live staging head `20260830171000`; authenticated-only execute; receipts/observations remain zero before owner-controlled retry. |
||| `pdc_historical_778_caller.py`, `scripts/apply_migration_778_staging.py`, `scripts/apply_migration_778_successor_1710_staging.py` | Staging caller/controllers | Frozen-manifest-only caller, current actor/gateway/release binding, fresh outbox, exact staging target and migration approval/readback | Python syntax and focused caller/controller tests passed; no IMAP rescan or valid historical Apply. |
