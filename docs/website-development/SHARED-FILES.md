...
| `backend/pdc_email_ai_successor_executor.py`, `supabase/staging_only/20260831300000_pdc_email_ai_transaction_successor.sql` | Separate staging email successor | One typed command RPC, fixed canonical dispatch, immutable receipts, RLS/grants and independent Board readback | SQL/Python contract tests; live apply requires exact 863 staging predecessor and protected connector. |
...
| `scripts/apply_pdc_email_ai_successor_staging.py`, `scripts/verify_pdc_email_ai_successor_staging.py` | STAGING installation/readback | Exact protected connector, live-head guard, catalog/function/ACL/forced-RLS readback, Production exclusion | Apply/read-only verifier passed against live 863→successor ledger; no runtime identity provisioned, no mailbox/task/outbound/UID514 activity. |
