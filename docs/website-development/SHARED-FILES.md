...
| `scripts/verify_pdc_email_ai_successor_inbox_staging.py` | Successor STAGING read verification | Read-only migration/RPC/typed-plan/Realtime/ACL/RLS/receipt-count/Production-sentinel proof | Live verifier returned `ok=true`; no runtime identity or transaction/action receipt provisioned. |
