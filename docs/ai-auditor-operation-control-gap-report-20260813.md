...
| Batch Review and Apply | Migration 253 plans first, requires a separate exact signed Apply delivery, and the staging website exposes a separately configured gateway panel displaying immutable proposal bindings. | Gateway deployment/configuration and scoped credentials remain activation blockers. The browser never holds the bot token/HMAC key and has no direct typed-RPC fallback. |
...
- Gateway-signature boundary: **implemented in migration 253/runtime source** with exact canonical envelope, global Telegram reservation, nonce/delivery replay control and shared Python/SQL vectors. Separate key custody, scoped credentials and live acceptance remain activation blockers.
- Realtime operation-control consumer: **implemented in source** against migration 229's shared run-revision publication, with typed and legacy event validation, two-consumer invalidation tests and legacy-stream preservation in forward containment 254.
- Separate Apply/Undo UX: **implemented fail-closed in source** through an HTTPS signed-gateway boundary with exact proposal/run bindings and Administrator confirmation. It remains disabled until the separate gateway and scoped identity credentials are configured and accepted live.
