...
assert.strictEqual(resolverCallSites.length, 6, 'PMB transfer, Ready for QC, PIT movement, QC sign-off, legacy RFT transfer and collection are the only lifecycle consumers');
assert.strictEqual(resolvedGuards.length, 6, 'every lifecycle consumer must require an explicit resolved outcome');
assert((app.match(/ref\.isArchived/g) || []).length >= 6, 'every lifecycle mutation must reject archived vehicles');
console.log('PASS 4: all six lifecycle consumers fail closed on outcome and archive state');
