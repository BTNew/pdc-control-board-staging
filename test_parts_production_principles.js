...
(function(){
  ...
  assert(!vehicleReadyForQualityControl(pitPendingVehicle), 'Outstanding Pit Inspection must block entry to the QC bucket');
  assert(vehicleRftGateIssues(pitPendingVehicle).some(issue => /outstanding jobs:.*pit/i.test(issue)), 'Pit Inspection must be reported as a QC/RFT gate issue');
  assert(vehicleRftGateIssues({ ...pitPendingVehicle, pdcQcComplete: true }).some(issue => /outstanding jobs:.*pit/i.test(issue)), 'QC metadata must not bypass outstanding Pit Inspection');
